# 🚀 EXECUTAR SQL AGORA - 2 MINUTOS!

---

## ⚠️ Situação Atual

O Supabase bloqueia SQL arbitrário por segurança via API.  
**Solução**: Você copia e cola nos 3 passos abaixo. ⏱️ ~2 minutos!
...
## 🎯 PASSO 2: Colar e Executar SQL 1 (FASE 6-8)
...
2. Clique no editor SQL (area branca)
3. **Ctrl+A** (seleciona tudo)
4. **Ctrl+V** (cola)
5. Clique botão azul "**Run**"
6. Aguarde: **✅ "Query executed successfully"**

---

## 🎯 PASSO 3: Colar e Executar SQL 2 (FASE 9-11)
...
**O que fazer**:
1. Clique "**+ New Query**" (botão verde) novamente
2. Clique no editor SQL
3. **Ctrl+A** (seleciona)
4. **Ctrl+V** (cola)
5. Clique "**Run**"
6. Aguarde: **✅ "Query executed successfully"**
...
Para confirmar que deu certo, execute estas 3 queries (uma por uma):
...
```sql
SELECT COUNT(*) as total_views FROM information_schema.tables 
WHERE table_schema = 'public' AND table_type = 'VIEW' AND table_name LIKE 'vw_%';
```

**Resultado esperado**: `3` (ou mais) ✅
...
WHERE trigger_schema = 'public' AND (trigger_name LIKE '%receivable%' OR trigger_name LIKE '%appointment%');
```

**Resultado esperado**: `2` (ou mais) ✅
...
Próximo: FASE 12-17 (próxima sessão)
```
...
```
Copiar SQL 1:   30 segundos
Cola + Run:     30 segundos
Copiar SQL 2:   30 segundos
Cola + Run:     30 segundos
Validações:     1 minuto
────────────────────────────
TOTAL:          ~3 minutos
...
**🚀 VAI! Abra Supabase agora: https://app.supabase.com/project/gvdkdjyupktlflwurike/sql**
