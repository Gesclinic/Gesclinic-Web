...
-- This script will create cbhpm_procedures table with 50 procedures
-- ============================================================

-- Step 1: Create cbhpm_procedures table
...
CREATE POLICY IF NOT EXISTS cbhpm_procedures_select_own_clinic ON cbhpm_procedures
  FOR SELECT USING (
    clinic_id IN (
...
CREATE POLICY IF NOT EXISTS cbhpm_procedures_insert_own_clinic ON cbhpm_procedures
...
CREATE POLICY IF NOT EXISTS cbhpm_procedures_update_own_clinic ON cbhpm_procedures
  FOR UPDATE USING (
    clinic_id IN (
      SELECT clinic_id FROM user_roles WHERE user_id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS cbhpm_procedures_delete_own_clinic ON cbhpm_procedures
  FOR DELETE USING (
    clinic_id IN (
      SELECT clinic_id FROM user_roles WHERE user_id = auth.uid()
    )
...
-- Step 3: Insert 50 procedures for the first clinic
WITH clinic_data AS (
...
SELECT COUNT(*) as total_procedures_created FROM cbhpm_procedures WHERE ativo = true;
