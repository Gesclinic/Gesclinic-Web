-- ============================================
-- CLEANUP + RESET (Executar primeiro)
-- ============================================

-- Drop se existirem (ordem importa - drop dependências primeiro)
DROP TRIGGER IF EXISTS trg_financial_transactions_timestamp ON financial_transactions;
DROP FUNCTION IF EXISTS update_financial_transactions_timestamp();
DROP VIEW IF EXISTS view_dre_summary;
DROP VIEW IF EXISTS view_financial_summary;
...
-- ============================================
-- AGORA EXECUTE ISTO APÓS O CLEANUP ACIMA
-- ============================================
...
RAISE NOTICE 'Contas criadas com sucesso!';

ELSE
  RAISE NOTICE 'Nenhuma clinica encontrada.';
END IF;
...
CREATE TYPE transaction_type AS ENUM ('revenue', 'expense', 'cost', 'deduction', 'adjustment', 'transfer');
CREATE TYPE transaction_status AS ENUM ('pending', 'scheduled', 'processed', 'paid', 'canceled');
CREATE TYPE transaction_category AS ENUM ('appointment', 'medical_service', 'exam', 'procedure', 'surgery', 'telemedicine', 'payroll', 'materials', 'maintenance', 'utilities', 'rent', 'tax', 'commission', 'marketing', 'software', 'equipment', 'other');
...
SELECT
  clinic_id, account_id, type, category,
...
-- VALIDACAO
SELECT 'OK!' as status;
SELECT COUNT(*) as contas_criadas FROM financial_accounts;
SELECT COUNT(*) as transacoes FROM financial_transactions;
