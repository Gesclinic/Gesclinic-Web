-- =============================================================================
-- PATCH: Permissão RPC de simulação com fallback para tabela users
-- =============================================================================
-- Cenário:
-- - Alguns ambientes usam user_clinic_roles
-- - Outros usam users(clinic_id, role)
--
-- Este patch mantém a validação por user_clinic_roles e adiciona fallback por users.
-- =============================================================================
...
begin
  ...
  -- Modelo principal: user_clinic_roles
  select exists (
    select 1
  ...
  -- Fallback: tabela users (alguns tenants usam apenas este vínculo)
  if not v_allowed then
    select exists (
      ...
      where u.id = p_actor_id
        ...
        and lower(coalesce(u.role, '')) in ('admin', 'gestor', 'financeiro', 'director')
    ) into v_allowed;
  end if;
