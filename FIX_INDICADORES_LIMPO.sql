-- ============================================
-- CORREÇÃO: INDICADORES DA AGENDA
-- Executa apenas as views e funções necessárias
-- ============================================
...
appointments_data AS (
  ...
  WHERE a.clinic_id IS NOT NULL
    AND a.scheduled_date >= dr.date_start
    AND a.scheduled_date < dr.date_end
),
slot_data AS (
  SELECT 
    a.clinic_id,
...
-- ============================================
-- 4. FUNÇÃO RPC: Indicadores Consolidados
-- ============================================
    ...
    COALESCE(AVG(
      CASE 
        WHEN a.end_time IS NOT NULL THEN 
          EXTRACT(EPOCH FROM (
            (a.scheduled_date + a.end_time::interval) - 
            (a.scheduled_date + a.scheduled_time::interval)
          )) / 60
        ELSE NULL
      END
    ), 0) as avg_duration,
