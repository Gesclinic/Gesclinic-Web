...
import { generateFinancialPrioritySuggestions, logFinancialSuggestionAction } from '../../../lib/financialPriorityApi';
...
export function useFinancialPrioritySuggestions(clinicId, date, options = {}) {
  ...
  const executeSuggestion = useCallback(async (suggestion, appointmentId, executedBy) => {
  ...
  const createAppointmentFromSuggestion = useCallback(async (suggestion, appointmentData, executedBy) => {
    try {
  ...
  const stats = {
    ...
    averageScore: suggestions.length > 0 ? Math.round(suggestions.reduce((sum, s) => sum + s.score_financeiro, 0) / suggestions.length) : 0
  };
