...
export function useAgendaStore() {
  ...
  const [filters, setFilters] = useState({
    ...
    // ID da sala
    status: null,
    // Confirmado, A confirmar, Faltou, Encaixe, Disponível
    payer: null,
    // ID do convênio/pagador
    service: null,
    // ID do serviço
  ...
  const filteredAppointments = useMemo(() => {
    ...
    if (filters.searchText) {
      ...
      result = result.filter(apt => apt.patient_name?.toLowerCase().includes(search) || apt.service_name?.toLowerCase().includes(search));
    }
    return result;
