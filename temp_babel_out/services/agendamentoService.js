...
export async function salvarAgendamento(payload) {
  ...
  const {
    data: conflitos
  } = await supabase.from("appointments").select("id").eq("clinic_id", clinicId).neq("id", id || "00000000-0000-0000-0000-000000000000").lt("start_time", end_time).gt("end_time", start_time);
  if (conflitos?.length) {
