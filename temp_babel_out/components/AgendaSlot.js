...
export default function AgendaSlot({
  ...
  if (columnType === 'professional' && groupId) {
    ...
    const schedules = professionalSchedules.filter(s => s.professional_id === groupId && parseInt(s.day_of_week, 10) === dayOfWeek);
  ...
  const actionLabel = isClinicalFlow ? normalizedStatus === SERVICE_STATUSES.AWAITING_PROFESSIONAL ? '📝 Prontuário' : '👨‍⚕️ Atendimento' : '📋 Check-in';
  const actionTitle = isClinicalFlow ? normalizedStatus === SERVICE_STATUSES.AWAITING_PROFESSIONAL ? 'Abrir prontuário do paciente' : 'Abrir atendimento em andamento' : 'Abrir check-in';
  ...
  if (isAvailable) {
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "absolute inset-0 bg-black/5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center gap-1 backdrop-blur-sm"
      ...
      className: "absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 bg-gray-900 text-white text-xs px-2 py-1 rounded whitespace-nowrap z-50"
    }, "Clique para agendar"));
  }
  ...
  }, appointment ? appointment.patient_name || 'Paciente' : 'Paciente'), size === 'standard' && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "text-xs text-gray-700 truncate"
  }, appointment.professional_name && `👨‍⚕️ ${appointment.professional_name}`), /*#__PURE__*/React.createElement("div", {
    className: "text-xs text-gray-600 truncate"
  }, appointment.service_name))), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between gap-1 pt-1"
  }, /*#__PURE__*/React.createElement("span", {
    className: `inline-block px-1.5 py-0.5 text-xs font-bold rounded ${colors.badge}`
  }, getStatusIcon(status), " ", getStatusLabel(status))), /*#__PURE__*/React.createElement("div", {
    className: "absolute inset-0 bg-black/60 rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center gap-1 backdrop-blur-sm p-2"
  }, onCheckin && ['recepcao', 'gestor', 'admin'].includes(userRole?.toLowerCase?.()) && /*#__PURE__*/React.createElement("button", {
    onClick: e => {
      e.stopPropagation();
  ...
  }, "\u2715 Cancelar")), showTooltip && appointment && /*#__PURE__*/React.createElement("div", {
    className: "absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 bg-gray-900 text-white text-xs px-3 py-2 rounded whitespace-nowrap z-50 shadow-lg"
  }, /*#__PURE__*/React.createElement("div", {
    className: "font-semibold"
  }, appointment.patient_name), appointment.professional_name && /*#__PURE__*/React.createElement("div", null, "\uD83D\uDC68\u200D\u2695\uFE0F ", appointment.professional_name), appointment.room_name && /*#__PURE__*/React.createElement("div", null, "\uD83C\uDFE5 ", appointment.room_name), appointment.service_name && /*#__PURE__*/React.createElement("div", null, "\uD83D\uDCCB ", appointment.service_name), /*#__PURE__*/React.createElement("div", {
