...
export default function ModalCriarAgendamento({
        ...
        try {
          const {
            ...
            error
              ...
              payers (id, name),
              plans (id, name, code)
            `).eq('id', appointmentIdToEdit).maybeSingle();
          if (error) {
  ...
  useEffect(() => {
    if (clinicId && (!rooms || rooms.length === 0)) {
      listRooms(clinicId).then(setRoomsList).catch(() => setRoomsList([]));
  ...
  return /*#__PURE__*/React.createElement(AppointmentUnitedModal, {
    isOpen: isModalOpen,
    onClose: handleClose,
    mode: determinedMode,
