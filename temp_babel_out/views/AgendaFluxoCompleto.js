...
import { APPOINTMENT_STATUS, AGENDA_MODE, getAgendaModeForRole, getVisibleStatusByRole } from "@/lib/appointmentStatusEnums";
...
export default function AgendaFluxoCompleto() {
  ...
  const loadAppointments = useCallback(async () => {
    try {
      ...
      if (currentRole?.toLowerCase?.() === 'profissional' && user?.email) {
        const {
          supabase
        } = await import('@/lib/customSupabaseClient');
        const {
          data: profData
        } = await supabase.from('professionals').select('id').eq('email', user.email).eq('clinic_id', clinicId).maybeSingle();
        if (profData?.id) {
  ...
  if (!clinicId) {
    ...
    }, "Cl\xEDnica n\xE3o configurada"), /*#__PURE__*/React.createElement("p", {
