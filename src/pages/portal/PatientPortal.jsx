...
import { Calendar, Key, Loader2, Pill, Video, Download, LogIn, FileText, Stethoscope } from "lucide-react";
  ...
  <Card className="hover:shadow-lg transition-shadow duration-300">
    <CardHeader>
      <CardTitle className="text-base font-semibold flex items-center justify-between">
        <span><Stethoscope className="inline-block mr-2 text-primary" />{item.professional_name}</span>
        <span className="text-sm font-normal text-muted-foreground">{new Date(item.start_time).toLocaleDateString('pt-BR')}</span>
      </CardTitle>
  ...
  <Card className="hover:shadow-lg transition-shadow duration-300 border-sky-100">
    <CardHeader>
      <CardTitle className="text-base font-semibold flex items-center justify-between gap-3">
        <span><FileText className="inline-block mr-2 text-sky-600" />{item.title}</span>
        <span className="text-sm font-normal text-muted-foreground">
          {item.exam_date ? new Date(item.exam_date).toLocaleDateString('pt-BR') : new Date(item.created_at).toLocaleDateString('pt-BR')}
        </span>
      </CardTitle>
  ...
  const fetchPortalData = async (patientId) => {
    ...
    try {
      ...
      setLaudos(laudosData || []);
       ...
       toast({ title: "Erro ao carregar portal", description: error.message, variant: "destructive" });
    } finally {
        setLoading(false);
    }
  };
      ...
      <div className="min-h-screen bg-slate-50 p-4 sm:p-8">
        <div className="max-w-4xl mx-auto space-y-8">
            ...
            <Card className="max-w-md mx-auto shadow-xl">
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><Key className="text-primary"/> Acesso Seguro</CardTitle>
              </CardHeader>
              <form onSubmit={handleLogin}>
                <CardContent className="space-y-4">
                  <div className="space-y-1">
                      <label htmlFor="cpf" className="text-sm font-medium">CPF</label>
                      <Input id="cpf" placeholder="000.000.000-00" type="text" value={credentials.cpf} onChange={e => setCredentials({...credentials, cpf: e.target.value})} required/>
                  </div>
                  <div className="space-y-1">
                      <label htmlFor="birth" className="text-sm font-medium">Data de Nascimento</label>
                      <Input id="birth" type="date" value={credentials.birthDate} onChange={e => setCredentials({...credentials, birthDate: e.target.value})} required/>
                  </div>
                </CardContent>
                <CardFooter>
            ...
            <main className="space-y-8">
              ...
              {!loading ? (
                <section className="space-y-4">
                  <div>
                    <h2 className="text-lg font-semibold text-slate-800">Historico de atendimentos</h2>
                    <p className="text-sm text-muted-foreground">Consultas, receitas e registros relacionados ao seu cuidado.</p>
                  </div>
                  {history.length === 0 ? (
                    <p className="text-center text-muted-foreground">Nenhum historico encontrado.</p>
              ...
              {!loading ? (
                <section className="space-y-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Resultados e pareceres publicados pela clinica para consulta online.</p>
                  </div>
                  {laudos.length === 0 ? (
                    <Card>
