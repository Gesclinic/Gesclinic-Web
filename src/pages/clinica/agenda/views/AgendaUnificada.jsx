...
function AgendaUnificada() {
  ...
  const carregarAgendamentos = async () => {
    ...
    console.log('[AgendaUnificada] Carregando agenda para:', { clinic_id: clinic.id, date, start: dayStartUTC.toISOString(), end: dayEndUTC.toISOString() });
    ...
    try {
      ...
      console.log('[AgendaUnificada] Resultado de listAppointments:', { result, length: result?.length });
  ...
  useEffect(() => {
    ...
    if (clinic?.id) {
      ...
      listRooms(clinic.id).then(setRooms).catch(() => setRooms([]));
    }
  }, [clinic?.id, date, currentRole, user?.id]);

  // Filtros únicos
  // Para Radix Select, precisamos arrays de objetos {id, name}
  const profissionais = useMemo(() => Array.from(new Set(agendamentos.map(a => a.professionalId && a.professional ? JSON.stringify({ id: a.professionalId, name: a.professional }) : null).filter(Boolean))).map(str => JSON.parse(str)), [agendamentos]);
  const salas = useMemo(() => Array.from(new Set(agendamentos.map(a => a.roomId && a.sala ? JSON.stringify({ id: a.roomId, name: a.sala }) : null).filter(Boolean))).map(str => JSON.parse(str)), [agendamentos]);
  const pacientes = useMemo(() => Array.from(new Set(agendamentos.map(a => a.patientId && a.paciente ? JSON.stringify({ id: a.patientId, name: a.paciente }) : null).filter(Boolean))).map(str => JSON.parse(str)), [agendamentos]);
  ...
  const disponibilidadeProfissional = useMemo(() => {
    if (professionalId && professionalId !== 'all') {
      ...
      const disp = schedules.filter(s => String(s.professional_id) === String(professionalId) && s.day_of_week === dayOfWeek && s.active !== false && !s.blocked);
      if (!disp || disp.length === 0) return false;
    }
    return true;
  ...
  function getHorariosDisponiveis(profId, dateStr) {
    ...
    const profSchedules = schedules.filter(s => String(s.professional_id) === String(profId) && s.day_of_week === diaSemana && s.active !== false && !s.blocked);
    if (profSchedules.length === 0) return null; // agenda bloqueada
    let slots = [];
    profSchedules.forEach(sch => {
      ...
      while (curr < fim) {
        ...
        let mins = h * 60 + m + dur;
        let nh = String(Math.floor(mins / 60)).padStart(2, '0');
        let nm = String(mins % 60).padStart(2, '0');
        curr = `${nh}:${nm}`;
        if (curr >= fim) break;
      }
    });
  ...
  const linhas = useMemo(() => {
    ...
    return horarios.map(horario => {
      const agendamento = agendamentos.find(a => {
        ...
        const filtroProfId = professionalId && professionalId !== 'all' ? String(professionalId) : null;
        if (filtroProfId) {
      ...
      if (agendamento) {
        if (
          (professionalId && professionalId !== 'all' && String(agendamento.professionalId || agendamento.professional_id) !== String(professionalId)) ||
      ...
      <div style={{ padding: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <button style={{ padding: '6px 14px', background: '#1976d2', color: '#fff', border: 0, borderRadius: 4, fontWeight: 600, cursor: 'pointer' }} onClick={() => setNovoAgendamento(true)}>Novo Agendamento</button>
            <input
              type="date"
              value={date}
        ...
        <p style={{ textAlign: 'center', color: '#b00', padding: 32, fontWeight: 600, background: '#fff0f0' }}>Agenda bloqueada para este profissional neste dia</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ 
        ...
        🔍 Debug: Clinic ID: {clinic?.id} | Date: {date} | Appointments loaded: {agendamentos.length} | {loading ? '⏳ Loading...' : 'Done'}
      </div>
      ...
      <div style={{ background: '#eef', color: '#333', padding: 8, margin: '8px 0', fontSize: 13, border: '1px solid #dde', borderRadius: 4 }}>
        <b>Schedules carregados:</b> {JSON.stringify(schedules)}<br/>
        <b>Dia da semana atual:</b> {new Date(date).getDay()}<br/>
        {professionalId && professionalId !== 'all' && (
          <div style={{ background: '#ffe', color: '#333', padding: 8, margin: '8px 0', fontSize: 13, border: '1px solid #eed', borderRadius: 4 }}>
            <b>Horários disponíveis para o profissional selecionado:</b> {JSON.stringify(getHorariosDisponiveis(professionalId, date))}
          </div>
        )}
      </div>
      {loading ? (
        <p style={{ textAlign: 'center', color: '#888', padding: 24 }}>Carregando agendamentos...</p>
      ) : (professionalId && professionalId !== 'all' && (!horarios || horarios.length === 0)) ? (
        <p style={{ textAlign: 'center', color: '#b00', padding: 32, fontWeight: 600, background: '#fff0f0' }}>Agenda bloqueada para este profissional neste dia</p>
      ) : linhas[0]?.bloqueada ? (
        <p style={{ textAlign: 'center', color: '#888', padding: 24 }}>Nenhum horário disponível.</p>
      ) : (
        <div style={{ overflowX: 'auto', border: '1px solid #d0d0d0', borderRadius: '6px', backgroundColor: '#f9f9f9', width: '100%' }}>
          {/* Agenda Unificada: cada linha é um card horizontal interativo */}
          <div style={{ display: 'flex', flexDirection: 'column', minWidth: '1450px', background: '#fff' }}>
            <div className="bg-gray-100 text-xs font-bold" style={{ display: 'flex', fontWeight: 700, borderBottom: '2px solid #999', padding: '12px 0', position: 'sticky', top: 0, zIndex: 10, backgroundColor: '#f0f0f0', color: '#333' }}>
              <div style={{ width: 90, paddingRight: 12, paddingLeft: 12, textAlign: 'center', flexShrink: 0, fontSize: '11px', fontWeight: 700 }}>HORÁRIO</div>
              <div style={{ width: 140, paddingRight: 12, textAlign: 'left', flexShrink: 0, fontSize: '11px', fontWeight: 700 }}>PACIENTE</div>
              <div style={{ width: 40, paddingRight: 8, textAlign: 'left', flexShrink: 0, fontSize: '11px', fontWeight: 700 }}>CONTATO</div>
              <div style={{ width: 160, paddingRight: 12, textAlign: 'left', flexShrink: 0, fontSize: '11px', fontWeight: 700 }}>SERVIÇO</div>
              <div style={{ width: 50, paddingRight: 8, textAlign: 'left', flexShrink: 0, fontSize: '11px', fontWeight: 700 }}>CONVÊNIO</div>
              <div style={{ minWidth: 120, paddingRight: 12, flex: 1, textAlign: 'left', fontSize: '11px', fontWeight: 700 }}>PLANO/OBS</div>
              <div style={{ width: 130, paddingRight: 12, textAlign: 'left', flexShrink: 0, fontSize: '11px', fontWeight: 700 }}>PROFISSIONAL</div>
              <div style={{ width: 200, paddingRight: 12, paddingLeft: 8, textAlign: 'center', flexShrink: 0, fontSize: '11px', fontWeight: 700 }}>STATUS</div>
            </div>
