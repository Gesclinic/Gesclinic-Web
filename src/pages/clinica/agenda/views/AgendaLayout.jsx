...
export default function AgendaLayout({ mode, professionalId, roomId }) {
  const { clinic, professionals: ctxProfessionals, loadingClinic } = useClinicContext();
  ...
  useEffect(() => {
    if (clinic?.id) {
      setFilters((prev) => ({ ...prev, clinicId: clinic.id }));
    }
  }, [clinic]);
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      <div className="flex-1 overflow-hidden bg-white/80 backdrop-blur border shadow-inner rounded-xl">
        {renderView}
      </div>
