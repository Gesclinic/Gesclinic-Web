...
export default function AgendaPorProfissional({
  ...
  console.log(
    `🎭 [AgendaPorProfissional] Perfil: ${userRole} | isProfissional: ${isProfissional} | Abas: ${accessibleTabs.join(', ')}`,
  );
  ...
  useEffect(() => {
    ...
    if (appointments && appointments.length > 0) {
      console.log(
        `✅ [useEffect] Setando agendamentos com ${appointments.length} itens:`,
        appointments.map((a) => ({
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([profs, svcs, payers_, scheds, rms]) => {
        ...
        setRooms(rms || []);
        console.log(
          `✅ [Agenda] Carregada: ${profs?.length} prof, ${scheds?.length} schedules, ${rms?.length} rooms`,
        );
      })
  ...
  const profissionaisFiltrados = useMemo(() => {
    ...
    console.log(
      `📍 [profissionaisFiltrados] Nenhum filtro - retornando todos os ${profissionais.length} profissionais`,
    );
  ...
  const agendamentosFiltrados = useMemo(() => {
    ...
    if (isProfissional && user?.professional_id) {
      ...
      console.log(
        `🎭 [agendamentosFiltrados] Profissional: ${user.professional_id} | Filtrado: ${filtered.length}/${agendamentos.length}`,
      );
    ...
    console.log(
      `🎭 [agendamentosFiltrados] Admin/Gestor - retornando todos os ${agendamentos.length}`,
    );
  ...
  const horariosPorProfissional = useMemo(() => {
    ...
    console.log(
      `📍 [horariosPorProfissional] Data: ${date} | dayOfWeek: ${dayOfWeek} | Total schedules: ${schedules.length}`,
    );
  ...
  const renderDiaView = () => {
    ...
    console.log(
      `🔍 [renderDiaView] DEBUG: isProfissional=${isProfissional} | userRole='${userRole}' | user.professional_id=${user?.professional_id}`,
    );
          ...
          <tbody>
            {(() => {
              ...
              profissionaisFiltrados.forEach((prof) => {
                ...
                if (disp) {
                  disp.forEach((h) => allHorarios.add(h));
                }
              });
                ...
                return (
                  <tr>
                    <td
                      ...
                      style={{ textAlign: 'center', padding: 48, color: '#6c757d', fontSize: 15 }}
                    >
                  ...
                  {profissionaisFiltrados.map((prof) => {
                    const apt = agendamentosFiltrados.find((a) => {
                      ...
                      const matches =
                        aptDate === date &&
                        aptTime === horario &&
                        String(aptProfId) === String(prof.id);
    ...
    if (allHorarios.size === 0) {
      agendamentosFiltrados.forEach((apt) => {
        if (apt.scheduled_time) {
          allHorarios.add(apt.scheduled_time);
        }
      });
    }
            ...
            horariosOrdenados.map((horario) => (
              ...
              >
                ...
                {daysOfWeek.map((day) => {
                  ...
                  const apt = agendamentosFiltrados.find(
                    (a) => a.scheduled_date === dayStr && a.scheduled_time === horario,
                  );
                  const isLivre = !apt;
                  let cellBg = '#fafaf9';
  ...
  const renderSemanaView = () => {
    ...
    console.log(
      `  isProfissional: ${isProfissional}, user.professional_id: ${user?.professional_id}`,
    );
    console.log(`  agendamentosFiltrados.length: ${agendamentosFiltrados.length}`);
    console.log(`  schedules.length: ${schedules?.length || 0}`);
    console.log(
      '  horariosPorProfissional keys:',
      Object.keys(horariosPorProfissional || {}).slice(0, 3),
    );
    if (agendamentosFiltrados.length > 0) {
      console.log(
        '  Exemplos:',
        agendamentosFiltrados.slice(0, 3).map((a) => ({
          patient: a.patient_name,
          date: a.scheduled_date,
          time: a.scheduled_time,
      ...
      console.log(
        '  TODOS da semana (05/03 a 11/03):',
        agendamentosFiltrados
          .filter((a) => a.scheduled_date >= '2026-03-02' && a.scheduled_date <= '2026-03-08')
          .map((a) => ({
            patient: a.patient_name,
            date: a.scheduled_date,
            time: a.scheduled_time,
    ...
    console.log(
      `  Semana: ${format(weekStart, 'dd/MM')} a ${format(addDays(weekStart, 6), 'dd/MM')}`,
    );
    console.log(
      `  weekStart: ${weekStart}, daysOfWeek[0]: ${format(daysOfWeek[0], 'yyyy-MM-dd')}, daysOfWeek[4]: ${format(daysOfWeek[4], 'yyyy-MM-dd')}`,
    );
              ...
              {daysOfWeek.map((day) => {
                ...
                const apt = agendamentosFiltrados.find((a) => {
                  ...
                  const matches =
                    aptDate === dayStr &&
                    aptTime === horario &&
                    (isProfissional || !targetProfId || String(aptProfId) === String(targetProfId));

                  // DEBUG detalhado - mostrar todos os agendamentos da quinta (05/03)
                  if (dayStr === '2026-03-05') {
                    if (horario === '09:00' || horario === '09:30') {
                      console.log(
                        `  🔍 [${dayStr} ${horario}] ${a.patient_name?.substring(0, 10)}: aptDate=${aptDate}, aptTime=${aptTime}, dayMatch=${aptDate === dayStr}, timeMatch=${aptTime === horario}, isProfissional=${isProfissional}, profMatch=${String(aptProfId) === String(targetProfId)}, MATCHES=${matches}`,
                      );
                    }
                  }
                  ...
                  if (matches) {
                    console.log(
                      `✅ [renderSemanaView] FOUND: ${dayStr} ${horario} → ${a.patient_name}`,
                    );
                  }
                ...
                if (!isAvailable && horario === '08:00') {
                  console.log(
                    `  ❌ NOT AVAILABLE: targetProfId=${targetProfId}, dayOfWeek=${dayOfWeek}, horario=${horario}, schedules?.length=${schedules?.length || 0}`,
                  );
                }
                ...
                if (apt) {
                  ...
                  console.log(
                    `✅✅✅ [AGENDAMENTO ENCONTRADO] ${dayStr} ${horario} → ${apt.patient_name}, status=${apt.status}, color=${color}`,
                  );
          ...
          <div className="grid grid-cols-7 gap-0">
            {calendarDays.map((day) => {
              ...
              if (profissionaisFiltrados && profissionaisFiltrados.length > 0) {
                ...
                isDayAvailable = schedules.some(
                  (s) => s.day_of_week === supabaseDayOfWeek && s.active !== false && !s.blocked,
                );
              }
              ...
              return (
                ...
                >
                  {/* Número do dia */}
                  <div className="text-sm font-bold mb-2 text-gray-900">{format(day, 'd')}</div>
      ...
      <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap items-end gap-2">
          ...
          <button
            ...
            disabled={
              isProfissional ? !user?.professional_id : !professionalId || professionalId === 'all'
            }
          >
            Novo Agendamento
          </button>
