 ...
 */
export default function ProfessionalColumnHeader({ groupId, group, columnType, totalSlots }) {
  // Calcular métricas
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-center gap-2 mb-3 pb-3 border-b border-gray-200">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-bold text-gray-900 text-sm truncate">{group.name}</div>
          {group.specialty && (
            <div className="text-xs text-gray-500 truncate">{group.specialty}</div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        ...
        <div className="bg-white rounded-lg p-2.5 border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="flex items-baseline gap-1.5">
            <span className="text-xl font-bold text-blue-600">{metrics.totalAppointments}</span>
            <span className="text-xs text-gray-500">
              agendado{metrics.totalAppointments !== 1 ? 's' : ''}
            </span>
          </div>
        </div>
      </div>
      ...
      {/* Badge de disponibilidade */}
      <div
        className={`inline-block px-3 py-1.5 rounded-lg text-xs font-bold text-center w-full border-2 ${
          metrics.availableSlots > 5
