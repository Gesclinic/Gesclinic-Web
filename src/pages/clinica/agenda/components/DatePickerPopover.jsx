...
export default function DatePickerPopover({ selectedDate, onChange, agendaSummary = {} }) {
  ...
  const dayButtons = useMemo(() => {
    ...
    return days.map((date) => {
      ...
      const displayMeta = isFeriadoDay ? { status: 'holiday', label: feriado } : meta;
      ...
      {open &&
        createPortal(
          <>
            ...
            >
              {/* Header com setas de navegação e selects lado a lado */}
                ...
                >
                  {Array.from({ length: 12 }).map((_, i) => (
                    <option key={i} value={i}>
                      {new Date(2020, i).toLocaleString('pt-BR', { month: 'long' })}
                    </option>
                  ))}
                </select>
              ...
              <div className="border-t border-gray-100 pt-3">
                ...
                {/* Legenda - lado a lado */}
                <div className="text-xs text-gray-600 flex items-center justify-start gap-4 flex-wrap">
