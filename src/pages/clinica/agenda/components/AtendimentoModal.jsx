...
export default function AtendimentoModal({
  ...
  useEffect(() => {
    if (isOpen && appointment) {
      console.log(
        '✅ [AtendimentoModal] Carregando dados frescos do appointment ID:',
        appointment.id,
      );
      ...
      (async () => {
        try {
          ...
          console.log(
            '🔄 [AtendimentoModal] Buscando dados frescos do Supabase para ID:',
            appointment.id,
          );
          ...
          console.log(
            '✅ [AtendimentoModal] Dados mapeados (snake_case → camelCase):',
            mappedAppointment,
          );
    ...
    if (agendamentoData.professionalId && isOpen) {
      const loadSchedules = async () => {
        try {
          setLoadingProfessionalSchedules(true);
          console.log(
            '⏰ [AtendimentoModal] Carregando schedules quando profissional muda:',
          ...
          setProfessionalSchedules(schedules || []);
          console.log(
            '✅ [AtendimentoModal] Schedules do profissional carregados:',
            schedules?.length,
          );
        } catch (err) {
          console.warn('⚠️ [AtendimentoModal] Erro ao carregar schedules:', err.message);
  ...
  const loadPatientData = async () => {
    try {
      ...
      console.log(
        '🔄 [AtendimentoModal] Carregando dados mais recentes do appointment com relacionamentos...',
      );
      ...
      if (appointmentError) {
        ...
        console.warn(
          '⚠️ [AtendimentoModal] Usando dados do appointment original (não conseguiu recarregar):',
      ...
      const missing = requiredFields
        .filter(
          ([field, label]) => !patientData[field] || patientData[field].toString().trim() === '',
        )
      ...
      const freshAuthNumber =
        appointmentFresh?.authorization_number || appointment.authorization_number || '';
      const freshAuthExpiry =
        appointmentFresh?.authorization_expiry || appointment.authorization_expiry || '';
      ...
      setFaturamentoData({
        ...
        requesting_doctor:
          freshBillingData?.requesting_doctor || appointment.requesting_doctor || '',
        responsible_doctor:
          freshBillingData?.responsible_doctor || appointment.professionals?.name || '',
        ...
        discount: parseFloat(
          freshBillingData?.discount !== undefined
            ? freshBillingData.discount
            : appointmentFresh?.discount || appointment.discount || 0,
        ), // ✅ Sempre carregar desconto do appointmentFresh do banco
        discount_reason:
          appointmentFresh?.discount_reason ||
          appointment.discount_reason ||
          freshBillingData?.discount_reason ||
          '', // ✅ Carregar motivo do desconto
        discount_authorized_by:
          appointmentFresh?.discount_authorized_by || appointment.discount_authorized_by || null, // ✅ Carregar quem autorizou o desconto
        discount_authorized_at:
          appointmentFresh?.discount_authorized_at || appointment.discount_authorized_at || null, // ✅ Carregar quando foi autorizado
      ...
      setPagamentoData({
        ...
        change: '',
        receipt_number:
          appointmentFresh?.authorization_number || appointment.authorization_number || '', // ✅ Carregar Nº Autorização do banco
        ...
        card_installments:
          appointmentFresh?.card_installments?.toString() ||
          appointment.card_installments?.toString() ||
          '1',
        // Específico para PIX
        pix_identifier: '',
        pix_transaction_id: '',
      ...
      // 3️⃣ Se é CONVÊNIO FATURADO (precisa de guia TISS, autorização):
        ...
        // 3b. Se tem auth mas sem guide e cadastrais completos → Faturamento (próxima aba)
          ...
          console.log(
            '✅ Convênio Faturado com auth e cadastrais completos → Faturamento (próxima aba)',
          );
  ...
  useEffect(() => {
    ...
    const missing = requiredFields
      .filter(
        ([field, label]) => !cadastralData[field] || cadastralData[field].toString().trim() === '',
      )
      .map(([field, label]) => label);
  ...
  useEffect(() => {
    const autoSaveTimer = setTimeout(async () => {
      if (!pagamentoData.payment_method || !appointment?.id) {
        return;
      }
      ...
      try {
        ...
        const updatePayload = {
          ...
          card_installments: pagamentoData.card_installments
            ? parseInt(pagamentoData.card_installments)
            : 1,
    ...
    pagamentoData.card_brand,
    pagamentoData.card_last_digits,
    pagamentoData.card_installments,
    appointment?.id,
  ]);
  ...
  useEffect(() => {
    const autoSaveTimer = setTimeout(async () => {
      ...
      if (!faturamentoData.discount && !faturamentoData.notes) {
        return;
      }
  ...
  useEffect(() => {
    const autoSaveTimer = setTimeout(async () => {
      ...
      if (!liberacaoData.auth_number && !liberacaoData.auth_expiry && !liberacaoData.card_number) {
        return;
      }
  ...
  const handleSaveAgendamento = async (e) => {
    ...
    try {
      ...
      console.log(
        '💾 [handleSaveAgendamento] scheduled_time value being sent:',
      ...
      if (data && data.length > 0) {
        ...
        console.warn(
          '⚠️ UPDATE executado mas sem retorno de dados (possível RLS). Considerando sucesso.',
        );
        if (onSuccess) {
  ...
  const handleSaveFaturamento = async (e) => {
    try {
      ...
      showSuccessNotification(
        '✅ Guia TISS + Desconto salvo com sucesso! Avançando para resumo...',
      );
  ...
  const handleSavePagamento = async (e) => {
    try {
      ...
      try {
        if (arValue > 0) {
          ...
          if (pagamentoSplits && pagamentoSplits.length > 0) {
            ...
            console.log(
              `💳 Múltiplos pagamentos detectados. Total pago: R$ ${totalPago.toFixed(2)}, Saldo aberto: R$ ${saldoAberto.toFixed(2)}`,
            );
            ...
            try {
              await logAppointmentFinancialAudit({
                ...
                context: {
                  ...
                  total_paid: pagamentoSplits.reduce(
                    (sum, split) => sum + (parseFloat(split.amount) || 0),
                    0,
                  ),
            ...
            console.log(
              `💳 Criando Conta a Receber (valor com desconto: R$ ${arValue.toFixed(2)})...${isInstalledCard ? ` com ${installments} parcelas` : ''}`,
            );
            ...
            for (let i = 0; i < installments; i++) {
              ...
              console.log(
                `✅ ${isInstalledCard ? `Parcela ${i + 1}/${installments}` : 'Conta a Receber'} criada:`,
                {
                  id: arResult.id,
      ...
      const paymentSplitsJson =
        pagamentoSplits && pagamentoSplits.length > 0 ? JSON.stringify(pagamentoSplits) : null;
      ...
      const updatePayload = {
        ...
        notes: paymentSplitsJson
          ? `Múltiplos pagamentos: ${paymentSplitsJson}`
          : pagamentoData.notes || null,
        status: 'confirmed',
        updated_at: new Date().toISOString(),
        card_number: liberacaoData.card_number || null,
        authorization_number: isConvenioFaturado
          ? liberacaoData.auth_number || null
          : pagamentoData.receipt_number || null,
      ...
      const totalPaid = pagamentoSplits.reduce(
        (sum, split) => sum + (parseFloat(split.amount) || 0),
        0,
      );
      const saldoAberto =
  ...
  const validateFinancialData = () => {
    ...
    if (!cadastralData.name?.trim()) {
      errors.push('Nome do paciente incompleto');
    }
    if (!cadastralData.email?.trim()) {
      errors.push('Email do paciente não preenchido');
    }
    if (!cadastralData.phone?.trim() && !cadastralData.cell_phone?.trim()) {
      errors.push('Telefone não preenchido');
    }
    ...
    if (isParticular || isConvenioParticular) {
      ...
      if (saldoAberto > 0.01) {
        errors.push(
          `Valor em aberto: R$ ${saldoAberto.toFixed(2)} (Preencha o valor faltante antes de liberar)`,
        );
      }
    }
  ...
  const handleCompleteCheckIn = async (e) => {
    try {
      ...
      if (!validation.valid) {
        alert(
          `❌ Dados incompletos para liberar:\n\n${validation.errors.map((e) => `• ${e}`).join('\n')}`,
        );
  ...
  const selectedDateHasAvailability =
    !agendamentoData.professionalId || !agendamentoData.date
      ...
      : schedulesForSelectedDate.length > 0 &&
        !isBlockedHolidayDate(parseLocalDate(agendamentoData.date));
  ...
  const buildAvailableSlots = (schedules, duration) => {
    ...
    (schedules || []).forEach((schedule) => {
      ...
      const slotDuration =
        Number(schedule.duration_minutes) || Number(duration) || fallbackDuration;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose(false)}>
      <DialogContent className="app-dialog-shell app-dialog-shell--content overflow-auto">
        <DialogHeader className="border-b border-gray-200 px-6 pb-4 pt-6 text-left">
          <DialogTitle className="flex items-center gap-3">
            ...
            <span className="text-sm font-normal text-gray-500">
              Senha:{' '}
              <span className="text-blue-600 font-bold">{arrivals[appointment.id]?.password}</span>
            </span>
          </DialogTitle>
        </DialogHeader>
          ...
          <div className="bg-green-50 border border-green-300 rounded-lg p-3 mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-green-700">
                Conta a Receber criada • ID: {registroData.receivableId.substring(0, 8)}... •
                Auditoria: ✓
              </p>
            </div>
            <div className="text-right">
              ...
              <p className="text-xs text-green-600">
                {formatPaymentMethod(registroData.paymentMethod)}
              </p>
            </div>
          </div>
        )}
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="text-sm text-violet-700 mt-1">
                  Status atual: {getStatusLabelOnly(normalizedAppointmentStatus)}. Use o prontuário
                  para registrar a evolução e, se preferir, o painel clínico para conduzir o
                  início/finalização.
                </p>
              </div>
        ...
        <div
          style={{ display: 'flex', flexDirection: 'column', height: '80vh', overflow: 'hidden' }}
        >
          {/* Área com scroll */}
            ...
            <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value) => {
                          console.log(
                            '💳 [Select Convênio] Estado anterior:',
                            agendamentoData.payerId,
                            '→ Novo valor:',
                            value,
                            'Payers disponíveis:',
                ...
                <div className="space-y-4">
                  <div className="bg-blue-50 border border-blue-300 rounded-lg p-3 mb-4">
                    ...
                    <p className="text-xs text-blue-700 mt-1">
                      Estes dados vêm do cadastro do paciente e são obrigatórios para emissão de
                      guia
                    </p>
                  </div>
                    ...
                    <div className="grid grid-cols-4 gap-4">
                      <div className="col-span-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setCadastralData({ ...cadastralData, name: e.target.value })
                          }
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-gray-700 mb-1">
                          ...
                          {cadastralData.document_id?.trim() && (
                            <span className="text-green-600 ml-1">✓</span>
                          )}
                        </label>
                        <Input
                          ...
                          onChange={(e) =>
                            setCadastralData({ ...cadastralData, document_id: e.target.value })
                          }
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          value={cadastralData.birthdate}
                          onChange={(e) =>
                            setCadastralData({ ...cadastralData, birthdate: e.target.value })
                          }
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setCadastralData({ ...cadastralData, gender: e.target.value })
                          }
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setCadastralData({ ...cadastralData, email: e.target.value })
                          }
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setCadastralData({ ...cadastralData, phone: e.target.value })
                          }
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setCadastralData({ ...cadastralData, cell_phone: e.target.value })
                          }
                    ...
                    <div className="grid grid-cols-4 gap-4">
                      <div className="col-span-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setCadastralData({ ...cadastralData, street: e.target.value })
                          }
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setCadastralData({ ...cadastralData, number: e.target.value })
                          }
                          placeholder="123"
                      ...
                      <div>
                        ...
                        <Input value="" onChange={() => {}} placeholder="Apto 101, sala 202, etc" />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-700 mb-1">
                          Bairro <span className="text-red-500">*</span>
                          {cadastralData.neighborhood?.trim() && (
                            <span className="text-green-600 ml-1">✓</span>
                          )}
                        </label>
                        <Input
                          ...
                          onChange={(e) =>
                            setCadastralData({ ...cadastralData, neighborhood: e.target.value })
                          }
                          placeholder="Bairro"
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setCadastralData({ ...cadastralData, city: e.target.value })
                          }
                          placeholder="Cidade"
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setCadastralData({ ...cadastralData, state: e.target.value })
                          }
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setCadastralData({ ...cadastralData, zip_code: e.target.value })
                          }
              ...
              {tabAtivo === 'liberacao' && (
                <div className="space-y-4">
                  <div className="bg-blue-50 border border-blue-300 rounded-lg p-3 mb-4">
                    <p className="text-sm font-semibold text-blue-900">
                      📋 Validação de Cobertura (Padrão TISS)
                    </p>
                    <p className="text-xs text-blue-700 mt-1">
                      Operadora: {liberacaoData.payer_name || 'Não definida'}
                    </p>
                  </div>
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <div className="col-span-1">
                        <label className="block text-xs font-semibold text-gray-700 mb-1">
                          Matrícula / Nº Carteirinha <span className="text-red-500">*</span>
                        </label>
                        <Input
                          value={liberacaoData.card_number}
                          onChange={(e) =>
                            setLiberacaoData({ ...liberacaoData, card_number: e.target.value })
                          }
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="flex gap-4">
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              onChange={(e) =>
                                setLiberacaoData({ ...liberacaoData, requires_auth: 'yes' })
                              }
                              className="w-4 h-4"
                            />
                          ...
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              onChange={(e) =>
                                setLiberacaoData({ ...liberacaoData, requires_auth: 'no' })
                              }
                              className="w-4 h-4"
                            />
                      ...
                      {liberacaoData.requires_auth === 'yes' && (
                        <>
                          <div>
                            <label className="block text-xs font-semibold text-gray-700 mb-1">
                              Nº de Autorização (TISS)
                            </label>
                            <Input
                              value={liberacaoData.auth_number}
                              onChange={(e) =>
                                setLiberacaoData({ ...liberacaoData, auth_number: e.target.value })
                              }
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setLiberacaoData({ ...liberacaoData, auth_expiry: e.target.value })
                              }
                            />
                          </div>
                        </>
                      ...
                      <div className="col-span-3">
                        <select
                          ...
                          onChange={(e) =>
                            setLiberacaoData({ ...liberacaoData, auth_status: e.target.value })
                          }
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
                        >
                  ...
                  <div className="flex items-center gap-3 p-3 bg-blue-100 border border-blue-400 rounded-lg">
                    <input
                      ...
                      onChange={(e) =>
                        setLiberacaoData({ ...liberacaoData, authorized: e.target.checked })
                      }
                      className="w-5 h-5 text-blue-600 cursor-pointer"
                    />
                    <label htmlFor="authorized" className="flex-1 cursor-pointer">
                      <p className="font-semibold text-gray-900">
                        Documentação validada e paciente autorizado
                      </p>
                      <p className="text-xs text-gray-600">
                        Marque para prosseguir com o faturamento
                      </p>
                    </label>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div className="col-span-2">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setFaturamentoData({ ...faturamentoData, guide_type: e.target.value })
                          }
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setFaturamentoData({ ...faturamentoData, code_type: e.target.value })
                          }
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
                        >
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setFaturamentoData({ ...faturamentoData, service_date: e.target.value })
                          }
                        />
                      </div>
                      <div>
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div className="col-span-2">
                        <label className="block text-xs font-semibold text-gray-700 mb-1">
                          Nº Sequencial da Guia TISS <span className="text-red-500">*</span>
                        </label>
                        <Input
                          value={faturamentoData.guide_number}
                          onChange={(e) =>
                            setFaturamentoData({ ...faturamentoData, guide_number: e.target.value })
                          }
                          placeholder="Ex: 000000001"
                          className="font-mono text-lg font-bold"
                        />
                        <p className="text-xs text-gray-600 mt-1">
                          Será componente do arquivo XML para faturamento
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          step="0.01"
                          value={
                            faturamentoData.authorized_value || faturamentoData.estimated_value
                          }
                          onChange={(e) =>
                            setFaturamentoData({
                              ...faturamentoData,
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setFaturamentoData({ ...faturamentoData, notes: e.target.value })
                      }
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <p className="text-sm font-semibold text-blue-900">
                      ✓ Guia TISS{' '}
                      {faturamentoData.guide_number?.trim() ? 'pronta para XML' : 'incompleta'}
                    </p>
                    {faturamentoData.guide_number?.trim() && (
                      <p className="text-xs text-blue-700 mt-1">
                        Operadora: {liberacaoData.payer_name} | Arquivo será estruturado conforme
                        padrão
                      </p>
                    )}
                  </div>
                ...
                <div className="space-y-4">
                  <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mb-4">
                    ...
                    <p className="text-xs text-orange-700 mt-1">
                      Preencha os dados específicos da forma de pagamento
                    </p>
                  </div>
                    ...
                    <div className="bg-gradient-to-br from-red-50 to-orange-50 border-2 border-red-300 rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              if (
                                ...
                                alert(
                                  '❌ Você não tem permissão para autorizar descontos. Apenas Administrador ou Gerente Financeiro podem fazer isso.',
                                );
                                return;
                              }
                          ...
                          />
                          <p className="text-xs text-gray-600 mt-1">
                            Deixe em branco ou 0 para sem desconto
                          </p>
                        </div>
                        <div>
                          <label className="block text-xs font-semibold text-gray-700 mb-1">
                        ...
                        <div className="mt-4">
                          <button
                            ...
                            onClick={async () => {
                              if (
                                !faturamentoData.discount_reason ||
                                faturamentoData.discount_reason.trim() === ''
                              ) {
                                alert(
                                  '⚠️ Por favor, selecione um motivo válido para o desconto antes de solicitar',
                                );
                                return;
                              }
                              ...
                              try {
                                ...
                                showSuccessNotification(
                                  '📋 Solicitação enviada com sucesso! Aguardando aprovação na página de Autorizações...',
                                );
                                setIsDiscountSectionOpen(false); // Fechar seção após solicitar
                              } catch (error) {
                                console.error('Erro ao solicitar autorização:', error);
                                alert(
                                  '❌ Erro ao enviar solicitação de autorização: ' + error.message,
                                );
                              } finally {
                                setLoading(false);
                              }
                          ...
                          <p className="text-xs text-gray-600 mt-2 text-center">
                            O desconto será enviado para aprovação do Administrador/Gerente
                            Financeiro na página de Autorizações
                          </p>
                        </div>
                      )}
                      ...
                      {faturamentoData.discount > 0 && faturamentoData.discount_authorized_by && (
                        <div className="bg-green-50 border-l-4 border-green-600 p-3 mt-4 rounded">
                          <p className="text-xs font-semibold text-green-800">
                            ...
                            {discountAuthorizedByName || faturamentoData.discount_authorized_by}
                          </p>
                          {faturamentoData.discount_authorized_at && (
                            <p className="text-xs text-green-700 mt-1">
                              Em:{' '}
                              {new Date(faturamentoData.discount_authorized_at).toLocaleString(
                                'pt-BR',
                              )}
                            </p>
                          )}
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (totalPago + amount > valorComDesconto + 0.01) {
                                alert(
                                  `⚠️ Valor excede o saldo. Saldo disponível: R$ ${(valorComDesconto - totalPago).toFixed(2)}`,
                                );
                                return;
                              }
                  ...
                  {pagamentoSplits.length > 0 && (
                    <div className="bg-gradient-to-r from-green-50 to-green-100 border-2 border-green-400 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="inline-block px-2 py-1 bg-green-200 text-green-900 text-xs font-bold rounded">
                          PRONTO
                        </span>
                      </div>
                      <div className="space-y-3">
                          ...
                          >
                            ...
                            <div className="flex justify-between items-center bg-green-100 p-3 text-sm">
                              <div>
                                ...
                                {split.details?.cardBrand && (
                                  <span className="text-xs text-green-700 ml-2">
                                    ({split.details.cardBrand})
                                  </span>
                                )}
                                {split.details?.pixIdentifier && (
                                  <span className="text-xs text-green-700 ml-2">
                                    {split.details.pixIdentifier}
                                  </span>
                                )}
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    setPagamentoSplits(
                                      pagamentoSplits.filter((s) => s.id !== split.id),
                                    );
                                    if (editingSplitId === split.id) {
                                      setEditingSplitId(null);
                                    }
                ...
                <div className="space-y-4">
                  <div className="bg-purple-50 border border-purple-300 rounded-lg p-4 mb-4">
                    <p className="text-lg font-bold text-purple-900 flex items-center gap-2">
                      <TrendingUp size={20} />
                      📊 Resumo Financeiro{' '}
                      {isConvenioFaturado ? '- Convênio/Faturamento' : '- Particular'}
                    </p>
                      ...
                      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                        ...
                        <div className="grid grid-cols-3 gap-4 text-sm">
                          ...
                          <div className="bg-white p-3 rounded border border-green-100">
                            ...
                            <p className="font-bold text-green-600">
                              R$ {registroData.arValue.toFixed(2)}
                            </p>
                          </div>
                          <div className="bg-white p-3 rounded border border-green-100">
                          ...
                          <div className="bg-white p-3 rounded border border-green-100">
                            ...
                            <p className="font-mono text-xs font-semibold text-gray-900">
                              {registroData.receivableId
                                ? registroData.receivableId.substring(0, 8) + '...'
                                : 'Aguardando'}
                            </p>
                          </div>
                        </div>
                        {registroData.receivableStatus === 'criada' && (
                          <div className="mt-3 p-2 bg-green-100 rounded border border-green-300 text-xs text-green-800">
                            ✓ Conta a Receber criada com sucesso e registrada na auditoria
                            financeira
                          </div>
                        )}
                      </div>
                  ...
                  <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div className="bg-white p-3 rounded border border-indigo-100">
                        ...
                        >
                          {registroData.cashFlowRegistered
                            ? '✓ Registrado'
                    ...
                    <ul className="space-y-2 text-sm">
                      <li className="flex items-center gap-2">
                        ...
                        <span className="text-gray-700">
                          Dados cadastrais validados conforme padrão TISS
                        </span>
                      </li>
                      <li className="flex items-center gap-2">
                        <span
                      ...
                      <li className="flex items-center gap-2">
                        <span className="text-green-600 font-bold">✓</span>
                        <span className="text-gray-700">
                          Auditoria financeira ativada e rastreando
                        </span>
                      </li>
                      <li className="flex items-center gap-2">
                        <span className="text-green-600 font-bold">✓</span>
                ...
                <div className="space-y-4">
                    ...
                    return validation.valid ? null : (
                      <div className="bg-red-50 border-l-4 border-red-600 p-4 rounded-lg">
                        <p className="text-sm font-bold text-red-900 mb-2">
                          ❌ Não é possível liberar o atendimento:
                        </p>
                    ...
                    <>
                      <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                        ...
                        <div className="grid grid-cols-3 gap-4 text-sm">
                          ...
                          <div>
                            <p className="text-xs text-gray-600 font-medium">Nº Autorização</p>
                            <p className="font-semibold text-gray-900 text-blue-600">
                              {liberacaoData.auth_number}
                            </p>
                          </div>
                          <div>
                  ...
                  {(isParticular || isConvenioParticular) && (
                    <div className="bg-orange-50 rounded-lg p-4 border border-orange-200">
                          ...
                          <div>
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-700 mt-1">
                                {faturamentoData.discount_reason || 'Sem motivo'}
                              </p>
                              {faturamentoData.discount_authorized_by && (
                                <p className="text-xs text-green-700">
                                  ✓ Autorizado:{' '}
                                  {discountAuthorizedByName ||
                                    faturamentoData.discount_authorized_by}
                                </p>
                              )}
                              {!faturamentoData.discount_authorized_by && (
                        ...
                        {pagamentoData.payment_method === 'DINHEIRO' && (
                          <div>
                            ...
                            <p className="font-semibold text-green-600">
                              R$ {parseFloat(pagamentoData.change || '0').toFixed(2)}
                            </p>
                          </div>
                        )}
                        {pagamentoData.payment_method === 'CARTAO' &&
                      ...
                      {pagamentoSplits && pagamentoSplits.length > 0 && (
                        <div className="mt-4 pt-4 border-t border-orange-300">
                          <p className="text-xs font-bold text-orange-900 mb-3">
                          ...
                          <div className="space-y-2">
                            {pagamentoSplits.map((split, idx) => (
                              ...
                              >
                                <span className="font-medium text-gray-700">
                                  {formatPaymentMethod(split.method)}:
                                </span>
                                <span className="text-gray-900">
                                  R$ {parseFloat(split.amount || '0').toFixed(2)}
                                </span>
          ...
          <div className="border-t border-gray-200 p-4 bg-white flex gap-2 justify-end flex-shrink-0">
              ...
              <>
                ...
                <Button
                  ...
                  title={
                    ...
                    !agendamentoData.professionalId ||
                    !agendamentoData.serviceId
                      ? 'Preencha Data, Hora, Profissional e Serviço'
            ...
            {tabAtivo === 'cadastrais' && (
              <>
                ...
                <Button
                  type="button"
                  onClick={() =>
                    window.open(`/clinica/pacientes/${appointment?.patientId}`, '_blank')
                  }
            ...
            {tabAtivo === 'liberacao' && (
              <>
                <Button type="button" variant="outline" onClick={() => setTabAtivo('cadastrais')}>
                  ← Voltar
                </Button>
                <Button
                  type="button"
                  ...
                  disabled={
                    !liberacaoData.card_number?.trim() || !liberacaoData.authorized || loading
                  }
                  className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold"
                >
                  {loading ? (
            ...
            {tabAtivo === 'faturamento' && (
              <>
                <Button type="button" variant="outline" onClick={() => setTabAtivo('liberacao')}>
                  ← Voltar
                </Button>
                <Button
                  type="button"
            ...
            {tabAtivo === 'financeiro' && (
              <>
                <Button type="button" variant="outline" onClick={() => setTabAtivo('pagamento')}>
                  ← Voltar
                </Button>
                <Button
                  type="button"
