            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                {editando ? (
                  ...
                  <div className="text-gray-900">{dados?.startTime ? format(new Date(dados.startTime), 'dd/MM/yyyy') : '—'}</div>
                )}
              </div>
              <div className="flex flex-col gap-1">
                ...
                {editando ? (
                  ...
                  <div className="text-gray-900">{dados?.startTime ? format(new Date(dados.startTime), 'HH:mm') : '—'}</div>
                )}
              </div>
                ...
                <div className="flex flex-col md:flex-row gap-2">
                  <div className="flex-1 flex flex-col">
                    ...
                    {editando && Array.isArray(pacientes) && pacientes.length > 0 ? (
                      ...
                      >
                        ...
                        {pacientes.map(p => (
                          <option key={p.id} value={p.id}>{p.full_name || p.nome}</option>
                        ))}
                      </select>
                    ) : (
                  ...
                  <div className="w-full md:w-40 flex flex-col">
                    ...
                    {editando ? (
                      ...
                      <div className="text-gray-900">{dados?.prontuario || dados?.patientRecord || dados?.prontuarioNumero || dados?.patient?.record_number || '—'}</div>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                {editando && Array.isArray(servicos) && servicos.length > 0 ? (
                  ...
                  >
                    {servicos.some(s => (s.id || s.value || s) === '') ? null : <option value="">Selecione</option>}
                    {servicos.map(s => (
              ...
              <div className="flex flex-col gap-1">
                ...
                {editando ? (
                    <select className="input px-2 py-1 rounded border border-gray-300" value={form?.payer || ''} onChange={e => setForm(f => ({ ...f, payer: e.target.value }))}>
                      <option value="">Selecione</option>
              ...
              <div className="flex flex-col gap-1">
                ...
                {editando ? (
                    <select
                      ...
                      disabled={!form?.payer}
                    >
                      {Array.isArray(planos) && planos.length > 0 && form?.payer && planos
                        .filter(p => !form?.payer || p.payerId === form.payer || p.convenioId === form.payer || p.payer === form.payer)
                        .some(p => (p.value || p.id || p) === '') ? null : <option value="">Selecione</option>}
                      {Array.isArray(planos) && planos.length > 0 && form?.payer && planos
                        .filter(p => !form?.payer || p.payerId === form.payer || p.convenioId === form.payer || p.payer === form.payer)
                        .map(p => (
              ...
              <div className="flex flex-col gap-1">
                ...
                {editando ? (
                  <input className="input px-2 py-1 rounded border border-gray-300" value={form?.serviceValue || ''} onChange={e => setForm(f => ({ ...f, serviceValue: e.target.value }))} />
                ) : (
                  <div className="text-gray-900">{dados?.serviceValue !== null && dados?.serviceValue !== undefined ? dados?.serviceValue : '—'}</div>
                )}
              </div>
              <div className="flex flex-col gap-1">
                <label className="font-semibold text-gray-700">Sala</label>
                {editando ? (
                    <select className="input px-2 py-1 rounded border border-gray-300" value={form?.room || ''} onChange={e => setForm(f => ({ ...f, room: e.target.value }))}>
                      {salas.some(s => (s.value || s.id || s) === '') ? null : <option value="">Selecione</option>}
                      {salas.map(s => <option key={s.value || s.id || s} value={s.value || s.id || s}>{s.label || s.name || s}</option>)}
                    </select>
                ) : (
              ...
              <div className="flex flex-col gap-1">
                ...
                {editando ? (
                    <select className="input px-2 py-1 rounded border border-gray-300" value={form?.professional || ''} onChange={e => setForm(f => ({ ...f, professional: e.target.value }))}>
                      {profissionais.some(p => (p.value || p.id || p) === '') ? null : <option value="">Selecione</option>}
                      {profissionais.map(p => <option key={p.value || p.id || p} value={p.value || p.id || p}>{p.label || p.name || p}</option>)}
                    </select>
                ) : (
              ...
              <div className="flex flex-col gap-1">
                ...
                {editando ? (
                  <select className="input px-2 py-1 rounded border border-gray-300" value={form?.status || ''} onChange={e => setForm(f => ({ ...f, status: e.target.value }))}>
                  ...
                  <div className="text-gray-900">
                    {dados?.status === 'scheduled' ? 'Agendado' :
                    dados?.status === 'confirmed' ? 'Confirmado' :
                    dados?.status === 'cancelled' ? 'Cancelado' :
                    dados?.status || '—'}
                  </div>
                )}
              </div>
              ...
              <div className="col-span-2 flex flex-col gap-1">
                ...
                {editando ? (
                  <input className="input px-2 py-1 rounded border border-gray-300" value={form?.notes || ''} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
                ) : (
              ...
              <div className="col-span-2 flex flex-col gap-1">
                ...
                {editando ? (
                  <input className="input px-2 py-1 rounded border border-gray-300" value={form?.internalNotes || ''} onChange={e => setForm(f => ({ ...f, internalNotes: e.target.value }))} />
                ) : (
