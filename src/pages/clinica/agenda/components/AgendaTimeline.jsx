    ...
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b-2 border-gray-300 bg-gradient-to-b from-gray-50 to-white sticky top-0 z-10">
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 w-20 sticky left-0 bg-gray-50 z-20">
                Horário
              </th>
          ...
          <tbody>
            {timeSlots.flatMap((time) => {
              ...
              return slotAppointments.map((apt, idx) => (
                ...
                >
                  {idx === 0 ? (
                    <td className="px-4 py-3 text-sm font-bold text-gray-700">{time}</td>
                  ) : (
                    <td></td>
                  )}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity flex-wrap">
                      ...
                      {/* Iniciar Atendimento: Para profissional quando status é pronto_atendimento */}
                      {apt.status === 'pronto_atendimento' && (
                        <button
                          onClick={() => onSlotClick({ ...apt, type: 'start-attendance' })}
