...
export function PaymentMethodFields({
  ...
  return (
    <div className={`bg-${config.color}-50 border border-${config.color}-300 rounded-lg p-4 space-y-4`}>
      {/* Header */}
      ...
      {/* Warning se houver */}
      {config.warning && (
        <div className={`bg-${config.color === 'yellow' ? 'orange' : config.color}-100 border-l-4 border-${config.color === 'yellow' ? 'orange' : config.color}-500 p-3 text-sm`}>
          <p className={`font-semibold text-${config.color === 'yellow' ? 'orange' : config.color}-900`}>⚠️ {config.warning}</p>
        </div>
      )}
            ...
            {field.type === 'select' && (
              ...
              >
                ...
                <SelectContent>
                  ...
                  {field.id === 'bank_account' && bankAccounts.map((account) => (
