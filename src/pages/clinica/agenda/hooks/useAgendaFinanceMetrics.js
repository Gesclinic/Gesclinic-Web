...
export function useAgendaFinanceMetrics(
  ...
  const metaDia = {
    ...
    receitaMeta: receitaPorHora * 3.5 * (professionals?.length || 1), // 3.5h úteis
 ...
 */
function calcularIndicadorSaude({ ocupacao, receita, agendamentos }) {
...
export function compararMetricas(metricsAnterior, metricsAtual) {
  return {
    receitaVariacao: (
      ((metricsAtual.totalReceita - metricsAnterior.totalReceita) /
        (metricsAnterior.totalReceita || 1)) *
      100
    ).toFixed(1),
    ...
    receitaHoraVariacao: (
      ((metricsAtual.receitaPorHora - metricsAnterior.receitaPorHora) /
        (metricsAnterior.receitaPorHora || 1)) *
      100
    ).toFixed(1),
  };
}
