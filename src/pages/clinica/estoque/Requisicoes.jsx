...
export default function Requisicoes() {
      ...
      const entries = await Promise.all(
        (data || []).map(async (it) => {
          ...
          const pending = Math.max(
            0,
            (parseFloat(it.qty) || 0) - (parseFloat(it.delivered_qty) || 0),
          );
  ...
  const handleReject = async (id) => {
    if (!confirm('Confirmar rejeição desta requisição?')) {
      return;
    }
    try {
  ...
  const handleCancel = async (id) => {
    if (!confirm('Confirmar cancelamento desta requisição?')) {
      return;
    }
    try {
  ...
  const openSaida = (id) => {
    setSaidaReqId(id);
    setSaidaInitialForm({ notes: `REQ:${id}` });
    setSaidaOpen(true);
  };
  ...
  return (
    ...
    >
                  ...
                  .map((r) => {
                    ...
                    return (
                      <tr key={r.id} className="border-t hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`px-2 py-1 rounded text-xs font-semibold ${statusClass(r.status)}`}
                          >
                        ...
                        <td className="px-4 py-3">
                          {r.purpose === 'paciente'
                            ...
                            : r.purpose === 'reposicao_sala'
                              ? 'Reposição de sala'
                              : r.purpose === 'consumo_admin'
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                ...
                                {r.status === 'approved' && (
                                  <>
                                    <DropdownMenuSeparator />
                                    <DropdownMenuItem onClick={() => handleFulfill(r.id)}>
                                      Atender
                                    </DropdownMenuItem>
                                    <DropdownMenuItem onClick={() => openSaida(r.id)}>
                                      Saída
                                    </DropdownMenuItem>
                                  </>
                                )}
                ...
                {items.map((it) => (
                  <tr key={it.id} className="border-t">
                    ...
                    <td className="px-3 py-2 text-right">
                      {Math.max(0, (parseFloat(it.qty) || 0) - (parseFloat(it.delivered_qty) || 0))}
                    </td>
                    <td className="px-3 py-2 text-right">
                      {balances[it.item?.id || it.item_id] ?? '-'}
                    </td>
                    <td className="px-3 py-2 text-right">
                      <input
                        type="number"
                        ...
                        onChange={(e) =>
                          changeDeliverQty(it.item?.id || it.item_id, e.target.value)
                        }
                      />
                    </td>
      ...
      <Dialog open={approveOpen} onOpenChange={setApproveOpen}>
        <DialogContent className="app-dialog-shell app-dialog-shell--content app-dialog-shell--wide">
          ...
          <div className="space-y-2">
            ...
            <input
              className="border rounded px-3 py-2 text-sm w-full"
              value={user?.email || user?.user_metadata?.name || ''}
              disabled
            />
          </div>
      ...
      <StockRequestFulfillDialog
        ...
        onCompleted={async () => {
          toast({ title: 'Requisição atendida' });
          await loadRequests();
        }}
      />
