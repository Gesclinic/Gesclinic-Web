...
export default function Transferencias() {
  ...
  const { toast } = useToast();
      ...
      (async () => {
        try {
          const { data, error } = await supabase
            ...
            .eq('id', savedId)
            .single();
          if (!error && data) setDefaultDest({ id: data.id, name: data.name });
        } catch {}
      })();
    }, []);
    ...
    const saveDefaultDest = () => {
      if (!destFilter.id) {
        toast({ variant: 'destructive', title: 'Selecione um destino', description: 'Escolha o local de destino para salvar como padrão.' });
        return;
      }
    ...
    const applyMyPendings = () => {
      ...
      if (destFilter.id) {
        const ok = window.confirm('Deseja usar o destino selecionado como padrão para "Meus Pendentes"?');
        if (ok) {
          saveDefaultDest();
          setStatusFilter('pendente');
        }
      ...
      toast({ variant: 'destructive', title: 'Destino padrão não definido', description: 'Selecione um destino ou salve um padrão para usar "Meus Pendentes".' });
    };
  ...
  const handleSubmit = async (payload) => {
    try {
      ...
      const token = (globalThis.crypto && crypto.randomUUID) ? crypto.randomUUID() : Math.random().toString(36).slice(2, 10);
      ...
      toast({ variant: "destructive", title: "Erro ao registrar transferência", description: error.message });
    }
  };
  ...
  const loadTransfers = async () => {
    ...
    try {
      ...
      for (const r of data || []) {
        ...
        if (!groups.has(token)) groups.set(token, { date: r.move_date, items: new Map(), note: r.notes || '' });
        const g = groups.get(token);
        if (!g.items.has(key)) g.items.set(key, { item: r.item?.name || '-', item_id: r.item_id, qty: r.qty, exit: null, entry: null });
        const rec = g.items.get(key);
        if (r.type === 'exit') rec.exit = r; else if (r.type === 'entry') rec.entry = r;
        if (!g.date || (r.move_date && r.move_date > g.date)) g.date = r.move_date;
      ...
      for (const [, g] of groups) {
        for (const [, rec] of g.items) {
          ...
          table.push({
            ...
            notes: (g.note || '').replace(tokenRe, '').replace(/\s-\s*/, '').trim(),
      ...
      toast({ variant: 'destructive', title: 'Erro ao carregar transferências', description: err.message });
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => { loadTransfers(); }, [clinicId]);

  const filteredRows = useMemo(() => {
    return rows.filter(r => {
      if (startDate && (!r.date || r.date < startDate)) return false;
      if (endDate && (!r.date || r.date > endDate)) return false;
      if (originFilter.id && r.origemId !== originFilter.id) return false;
      if (destFilter.id && r.destinoId !== destFilter.id) return false;
    ...
    try {
      if (!row.origemId || !row.destinoId) {
        toast({
          ...
          description: 'Não foi possível identificar origem e destino desta transferência (registro antigo).'
        });
        return;
      }
  ...
  const acceptTransfer = async (row) => {
    if (!row.destinoId) {
      toast({ variant: 'destructive', title: 'Destino não identificado', description: 'Não foi possível identificar o local de destino para esta transferência.' });
      return;
    }
    const ok = window.confirm(`Aceitar transferência de ${row.qty} do item ${row.item} para o destino?`);
    if (!ok) return;
    try {
      ...
      toast({ variant: 'destructive', title: 'Erro ao aceitar transferência', description: err.message });
    }
  };
  ...
  return (
    <PageLayout
      ...
      actions={
        <Button className="bg-purple-600 text-white flex items-center" onClick={() => setDialogOpen(true)}>
      ...
      <Card className="p-6 mt-6">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-2">
          <div>
            ...
            <input type="date" className="border rounded px-3 py-2 w-full text-sm" value={startDate} onChange={e=>setStartDate(e.target.value)} />
          </div>
          <div>
            <label className="text-xs block mb-1">Data final</label>
            <input type="date" className="border rounded px-3 py-2 w-full text-sm" value={endDate} onChange={e=>setEndDate(e.target.value)} />
          </div>
          <div>
          ...
          <div>
            ...
            <select className="border rounded px-3 py-2 w-full text-sm" value={statusFilter} onChange={(e)=>setStatusFilter(e.target.value)}>
        ...
        {(startDate || endDate || originFilter.id || destFilter.id || statusFilter) && (
          <div className="mb-4">
            <Button variant="outline" size="sm" onClick={()=>{setStartDate("");setEndDate("");setOriginFilter({id:"",name:""});setDestFilter({id:"",name:""});setStatusFilter("");}}>Limpar filtros</Button>
          </div>
        )}
        {loading ? (
            ...
            <tbody>
              {filteredRows.map((r, idx) => (
                <tr key={idx} className="border-t hover:bg-gray-50">
                  <td className="px-4 py-3">{r.date ? format(new Date(r.date + 'T00:00:00'), 'dd/MM/yyyy') : '-'}</td>
                  ...
                  <td className="px-4 py-3">
                    {r.status === 'pendente' ? (
                      <span className="inline-flex items-center px-2 py-1 rounded bg-yellow-100 text-yellow-800 text-xs">Pendente de aceite</span>
                    ) : (
                      <span className="inline-flex items-center px-2 py-1 rounded bg-green-100 text-green-800 text-xs">Aceita</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-2">
                      {r.status === 'pendente' && (
                        <Button size="sm" className="bg-blue-600 text-white" onClick={()=>acceptTransfer(r)} disabled={!r.destinoId}>Aceitar</Button>
                      )}
                      <Button size="sm" variant="outline" onClick={()=>revertTransfer(r)} disabled={!r.origemId || !r.destinoId} title={!r.origemId || !r.destinoId ? 'Transferência antiga sem origem/destino completos' : undefined}>Reverter</Button>
                    </div>
                  </td>
                </tr>
              ))}
