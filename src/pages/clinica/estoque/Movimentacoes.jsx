...
export default function Movimentacoes() {
  ...
  const loadMovements = async () => {
    ...
    try {
      ...
      if (filters.itemId) queryFilters.itemId = filters.itemId;
  ...
  const getTypeIcon = (type) => {
    if (type === "entry") return <ArrowUpCircle className="w-4 h-4 text-green-500" />;
    if (type === "exit") return <ArrowDownCircle className="w-4 h-4 text-red-500" />;
                ...
                movements.map((mov) => (
                  <tr key={mov.id} className="border-t hover:bg-gray-50">
                    <td className="px-4 py-3">
                      {mov.move_date ? format(new Date(mov.move_date + 'T00:00:00'), 'dd/MM/yyyy') : '-'}
                    </td>
                    <td className="px-4 py-3">
