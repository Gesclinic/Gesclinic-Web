...
export default function RepassesRulesManager() {
  ...
  const regimesTaxRates = {
    simples_3: { name: "Simples Nacional (Anexo III - Serviços)", rate: 0.18, description: "Alíquota única Anexo III" },
    simples_5: { name: "Simples Nacional (Anexo V - Tecnologia)", rate: 0.15, description: "Alíquota única Anexo V" },
    simples_r: { name: "Simples com Fator R", rate: 0.1493, description: "PIS 3.65% + COFINS 7.65% + ISS 5%" },
    presumido: { name: "Lucro Presumido - Normal", rate: 0.2325, description: "IR 1.6% + CSLL 8% + COFINS 7.6% + PIS 1.65% + ISS 5%" },
    presumido_hospitalar: { name: "Lucro Presumido - Equiparação Hospitalar", rate: 0.1560, description: "IR 0.8% + CSLL 3.2% + COFINS 7.6% + PIS 1.6% + ISS 2.4%" },
    real_conservador: { name: "Lucro Real - Conservador", rate: 0.2330, description: "COFINS 7.65% + PIS 1.65% + ISS 5% + IR+CSLL 9%" },
    real_otimizado: { name: "Lucro Real - Otimizado (Equip.)", rate: 0.1750, description: "COFINS 7.65% + PIS 1.65% + ISS 2.4% + IR+CSLL 5.8%" },
  };
    ...
    try 
{
  ...
  const ruleStats = useMemo(() => {
    return {
      ...
      averagePercentual: rules.length > 0
  ...
  return (
    <>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-3xl font-bold text-blue-600 dark:text-blue-400">{ruleStats.total}</div>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-3xl font-bold text-green-600 dark:text-green-400">{ruleStats.active}</div>
              </div>
      ...
      <div className="flex justify-end">
        <Button onClick={() => handleOpenDialog()} className="gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 shadow-lg">
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                  Crie uma regra para controlar automaticamente o percentual de
                  repasse por profissional e serviço
                </p>
              </div>
              <Button
            ...
            >
              ...
              <CardContent className="space-y-3">
                  ...
                  {rule.service_type ? (
                    <>
                      <div className="px-2 py-1 rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 font-medium">
                        👥 Grupo: {rule.service_type === 'consultas' ? 'Consultas' : rule.service_type === 'exames' ? 'Exames' : rule.service_type === 'cirurgias' ? 'Cirurgias' : 'Procedimentos'}
                      </div>
                    </>
                  ) : (
                    <>
                ...
                <div className="space-y-1">
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">Serviço</p>
                ...
                <div className="bg-gradient-to-br from-blue-100 to-blue-50 dark:from-blue-900/30 dark:to-blue-800/20 rounded-lg p-3 border border-blue-200/50 dark:border-blue-800/30 text-center">
                  <p className="text-xs font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wide mb-0.5">Repasse</p>
                  <p className="text-3xl font-bold text-blue-700 dark:text-blue-300">
                    {rule.percentual || 0}<span className="text-base ml-1">%</span>
                  </p>
                </div>
        ...
        <DialogContent className="w-[90vw] h-[90vh] max-w-none p-0 flex flex-col gap-0">
            ...
            <div className="h-full overflow-y-auto">
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() => setFormData({ ...formData, rule_type: "individual", service_type: null })}
                  ...
                  <button
                    type="button"
                    onClick={() => setFormData({ ...formData, rule_type: "group", service_id: "", service_type: "consultas" })}
                    className={`flex-1 px-4 py-3 rounded-lg border-2 transition font-medium text-sm ${
              ...
              <div className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900/30 dark:to-slate-800/30 rounded-lg p-5 space-y-5 border border-slate-200/50 dark:border-slate-700/50">
                ...
                <div className="space-y-3">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setFormData({ ...formData, tipo_base: value })
                    }
                  >
                    ...
                    <SelectContent className="z-[99999] overflow-visible">
                      <SelectItem value="BRUTO">💰 Sobre valor bruto (sem deduções)</SelectItem>
                      <SelectItem value="LIQUIDO">📊 Sobre valor líquido (recomendado - com deduções)</SelectItem>
                    </SelectContent>
                  </Select>
                  <p className="text-xs text-slate-500 dark:text-slate-400 italic">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 italic">
                    {formData.iss_customizado ? (
                      <span className="text-orange-600 dark:text-orange-400 font-semibold">
                        ✏️ Customizado: {formData.iss_customizado}% (Padrão: {(regimesTaxRates[formData.regime_code]?.rate * 100).toFixed(2)}%)
                      </span>
                    ) : (
                      <>ISS padrão: {(regimesTaxRates[formData.regime_code]?.rate * 100).toFixed(2)}%</>
                    )}
                  </p>
                </div>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-950/20 dark:to-blue-900/20 border border-blue-300 dark:border-blue-700/50 rounded-lg p-4 flex gap-3">
                ...
                <div className="flex-1 text-sm text-blue-900 dark:text-blue-100">
                  ...
                  <p className="leading-relaxed text-xs mb-2">
                    <span className="font-semibold">{getProfessionalName(formData.professional_id) || "O profissional"}</span> receberá{" "}
                    ...
                    do valor <span className="font-semibold capitalize">{formData.tipo_base === "BRUTO" ? "bruto" : "líquido"}</span> pelo serviço{" "}
                    <span className="font-semibold">{getServiceName(formData.service_id) || "selecionado"}</span>
                  </p>
                  <p className="text-xs opacity-90 border-t border-blue-200 dark:border-blue-800 pt-2">
                    Regime: <span className="font-semibold">{regimesTaxRates[formData.regime_code]?.name || "Regime"}</span>
                  </p>
                </div>
              </div>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-700/50 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        <label className="text-xs font-semibold text-slate-600 dark:text-slate-400">Valor Bruto (R$):</label>
                        <Input
                          type="number"
                          min={0.01}
                          step={10}
                    ...
                    <div className="space-y-2 text-xs">
                      ...
                      <div className="flex items-center justify-between p-2 bg-orange-50 dark:bg-orange-900/10 rounded border border-orange-200 dark:border-orange-700/30">
                        <span className="text-orange-700 dark:text-orange-300">
                          🏛️ Impostos ({(effectiveRate * 100).toFixed(2)}%{formData.iss_customizado ? " ✏️ customizado" : ""}):
                        </span>
          ...
          <div className="bg-gradient-to-r from-slate-100 to-slate-50 dark:from-slate-900/40 dark:to-slate-800/40 px-6 py-4 border-t border-slate-200 dark:border-slate-700/50 flex gap-3 justify-end flex-shrink-0">
            ...
            <Button
              ...
              disabled={!formData.professional_id || (!formData.service_id && !formData.service_type)}
            >
        ...
        <AlertDialogContent className="border-0">
          ...
          <AlertDialogHeader className="pt-6 pl-16">
            ...
            <AlertDialogDescription className="text-sm mt-2">
              A regra será permanentemente removida do sistema. Esta ação não
              pode ser desfeita e não afetará os repassos já calculados.
            </AlertDialogDescription>
          </AlertDialogHeader>
