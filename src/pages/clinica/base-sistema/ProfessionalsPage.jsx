...
export function ProfessionalsPage() {
  ...
  const [scheduleFormState, setScheduleFormState] = useState({ isEditing: false, hasUnsavedData: false });
  ...
  const { pageNum, pageSize, goToPage, prevPage, nextPage, totalPages, items: paginatedProfessionals } = usePagination(professionals || [], 10);
  ...
  const loadData = useCallback(async () => {
    if (!clinicId || !user) {
      console.warn('🔴 [ProfessionalsPage] Missing clinicId or user:', { clinicId, user: user?.email });
      return;
    }
    ...
    const loadAssociatedUser = async () => {
      ...
      try {
        ...
        if (user) {
          ...
          console.log('[DEBUG] ❌ Nenhum usuário associado encontrado para:', { email, cpf, clinicId });
          setAssociatedUser(null);
          setUserRole('profissional');
        }
      } catch (err) {
  ...
  React.useEffect(() => {
    const loadTabData = async () => {
      if (!editingListId || !clinicId) return;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingListId) {
        console.log('📝 [handleSubmit] Atualizando profissional:', editingListId, 'com user_id:', dataToSave.user_id);
      ...
      setSuccess(editingListId ? "✅ Profissional atualizado com sucesso!" : "✅ Profissional criado com sucesso!");
      setTimeout(() => {
        closeListForm();
        setSelectedDocType("");
        setSuccess(null);
  ...
  const handleEditInList = async (professional) => {
    setFormData({
      ...
      documents: professional.documents ? (typeof professional.documents === "string" ? JSON.parse(professional.documents) : professional.documents) : [],
  ...
  const handleCreateAssociatedUser = async () => {
    ...
    if (!dataToUse?.email || !dataToUse?.name) {
      console.error('❌ Email ou Name ausentes:', { email: dataToUse?.email, name: dataToUse?.name });
    ...
    try {
      ...
      if (cpf) {
        ...
        console.log('📋 res.data:', res.data, '| Tipo:', typeof res.data, '| Booleano:', !!res.data);
        existingUserByCpf = res.data;
      }

      console.log('🔍 Verificação duplicata:');
      console.log('  - existingUserByEmail:', existingUserByEmail, '| Booleano:', !!existingUserByEmail);
      ...
      console.log('  -> (existingUserByEmail || existingUserByCpf):', !!(existingUserByEmail || existingUserByCpf));

      if (existingUserByEmail || existingUserByCpf) {
        console.error('❌ ENTROU NO IF - Usuário duplicado encontrado');
        setError("Já existe um usuário com este e-mail ou CPF vinculado à clínica. Não é possível criar outro.");
  ...
  const handleLinkExistingUser = async (selectedUser) => {
    try {
      ...
      const dataToSave = {
        ...
        documents: updatedFormData.documents?.length > 0 ? JSON.stringify(updatedFormData.documents) : null,
      };
      ...
      {showListForm && (
        <div className="app-modal-overlay" key={`modal-${editingListId}-${refreshCounter}`}>
          <div className="app-modal-shell app-modal-shell--form app-modal-shell--medium">
            <Card key={`card-${editingListId}-${refreshCounter}`} className="app-modal-card shadow-2xl border-0">
              {/* Header */}
              <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4 flex items-center justify-between rounded-t-lg shadow-md" style={{flexShrink: 0}}>
                <div className="flex items-center gap-3">
              ...
              <div style={{flex: 1, overflowY: "auto", overflowX: "hidden", display: "flex", flexDirection: "column", minHeight: 0}}>
                {/* ABA: DADOS */}
                {activeTab === "dados" && (
                <form id="professional-form" onSubmit={handleSubmit} className="space-y-5" style={{padding: "24px"}}>
                  <div style={{width: "100%"}}>

                    {/* SEÇÃO 1: FOTO E NOME */}
                    <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-4">
                      <div className="border-b pb-3">
                        <h3 className="text-lg font-bold text-gray-900">👤 Foto e Identificação</h3>
                        <p className="text-sm text-gray-600 mt-1">Informações básicas do profissional</p>
                      </div>
                          ...
                          {formData.photo_url ? (
                            ...
                            <div className="w-32 h-40 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center gap-2 hover:border-blue-400 hover:bg-blue-50 transition cursor-pointer group">
                              ...
                              >
                                ...
                                <span className="text-xs text-gray-600 group-hover:text-blue-600 font-semibold">Arquivo</span>
                              </label>
                              <button
                                type="button"
                                onClick={() => setShowCameraModal(true)}
                        ...
                        <div className="flex-1 space-y-4">
                          <div>
                            ...
                            <input
                              ...
                              autoFocus
                            />
                            <p className="text-xs text-gray-500 mt-2">Nome completo do profissional</p>
                          </div>
                          <div>
                            <label className="block text-sm font-semibold text-gray-800 mb-2">
                              Especialização
                            ...
                            <input
                              ...
                              disabled={submitting}
                            />
                            <p className="text-xs text-gray-500 mt-2">Especialidade profissional</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-4">
                      <div className="border-b pb-3">
                        ...
                        <p className="text-sm text-gray-600 mt-1">Informações de contato e documentação</p>
                      </div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-4" key={`access-section-${refreshCounter}`}>
                      <div className="border-b pb-3">
                        <h3 className="text-lg font-bold text-gray-900">🔐 Perfil de Acesso</h3>
                        <p className="text-sm text-gray-600 mt-1">Usuário e permissões de sistema</p>
                      </div>

                      {associatedUser ? (
                        <div className="space-y-4" key={`associated-user-${associatedUser.id}`}>
                          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <p className="text-sm font-medium text-gray-700 mb-2">🔗 Usuário Associado</p>
                            <div className="text-sm text-gray-600 space-y-1">
                              <p><strong>Nome:</strong> {associatedUser.full_name || associatedUser.name}</p>
                              <p><strong>Email:</strong> {associatedUser.email}</p>
                              <p><strong>ID:</strong> {associatedUser.id.substring(0, 8)}...</p>
                              <p><strong>Papel:</strong> <span className="font-semibold text-blue-600">{userRole}</span></p>
                            </div>
                          </div>
                        ...
                        <div className="space-y-4" key="no-associated-user">
                          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                            <p className="text-sm font-medium text-gray-900 mb-2">⚠️ Nenhum usuário associado</p>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <button
                              ...
                              title={associatedUser ? "Usuário já vinculado. Use 'Desvincular' para criar novo." : ""}
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-4">
                      <div className="border-b pb-3">
                        ...
                        <p className="text-sm text-gray-600 mt-1">Registro profissional e licenças</p>
                      </div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-4">
                      <div className="border-b pb-3">
                        ...
                        <p className="text-sm text-gray-600 mt-1">Localização do consultório ou clínica</p>
                      </div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-4">
                      <div className="border-b pb-3">
                        ...
                        <p className="text-sm text-gray-600 mt-1">Anexe cópias de documentos importantes</p>
                      </div>
                      ...
                      <div className="flex gap-3">
                        <div className="flex-1">
                          ...
                          >
                            ...
                            <option value="certificado_graduacao">Certificado de Graduação</option>
                      ...
                      {formData.documents && Array.isArray(formData.documents) && formData.documents.length > 0 && (
                        <div className="space-y-2 mt-4 pt-4 border-t">
                          <p className="text-sm font-semibold text-gray-900 mb-3">✅ Documentos adicionados:</p>
                          {formData.documents.map((doc) => (
                            <div key={doc.id} className="flex items-center justify-between gap-2 p-3 bg-green-50 rounded-lg border border-green-200">
                              <div className="flex items-center gap-3 flex-1">
                                <span className="text-green-700 font-semibold">✓</span>
                                <div>
                                  <div className="text-sm font-medium text-gray-900">{doc.name}</div>
                                  <div className="text-xs text-gray-600">{doc.type.replace(/_/g, ' ').toUpperCase()}</div>
                                </div>
                              </div>
                              <button
                                type="button"
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-5">
                      ...
                      <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                        <label className="flex items-center gap-3 cursor-pointer">
                          ...
                          <span className="text-xs text-gray-500">Disponível para agendamentos</span>
                        </label>
                      </div>
                    </div>
              ...
              {/* Footer */}
              <div className="border-t bg-gradient-to-r from-gray-50 to-white px-6 py-3 flex items-center justify-end gap-3 shadow-md rounded-b-lg" style={{flexShrink: 0}}>
                <button
                  type="button"
                  onClick={() => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl max-w-md w-full p-6 space-y-4">
            <div className="text-center border-b pb-4">
              ...
              <p className="text-sm text-gray-600 mt-1">Altere o papel/perfil do usuário no sistema</p>
            </div>

            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-2">
              <p className="text-sm font-medium text-gray-700">Informações do Usuário</p>
              <div className="text-sm text-gray-600 space-y-1">
                <p><strong>Nome:</strong> {associatedUser.full_name}</p>
                <p><strong>Email:</strong> {associatedUser.email}</p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">Papel atual: <strong>{userRole}</strong></p>
            </div>
      ...
      {formData.photo_url && (
        <div className="flex items-center gap-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <div>
            ...
            <p className="text-xs text-gray-600 mt-1">📸 Clique em "Editar Profissional" para alterar</p>
          </div>
        </div>
      )}
    ...
    <div className="space-y-4 w-full">
      <div className="space-y-2">
        {healthInsurances.map((insurance) => (
          <label key={insurance.id} className="flex items-center gap-3 p-3 border rounded-lg hover:bg-gray-50 cursor-pointer">
            <input
              type="checkbox"
