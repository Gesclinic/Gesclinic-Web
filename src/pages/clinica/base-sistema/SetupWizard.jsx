...
// ============================================================
// SETUP WIZARD - Assistente de configura��o inicial
// ============================================================
// Guia o usu�rio atrav�s dos passos necess�rios para
// configurar a cl�nica, validando e bloqueando features
...
export function SetupWizard({ onComplete, compactMode = false }) {
  ...
  const [expandedCategory, setExpandedCategory] = useState('cadastros-estruturais');
        ...
        <CardContent className="pt-6">
          <div className="flex items-center justify-center py-8">
            ...
            <span className="text-sm text-red-700">
              Nenhuma cl�nica associada. Por favor, fa�a login.
            </span>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-amber-200 bg-amber-50">
        <CardContent className="pt-6">
          <div className="flex items-center justify-center py-8">
            ...
            <span className="text-sm text-amber-700">Carregando configura��o...</span>
          </div>
        </CardContent>
      </Card>
    );
  ...
  if (compactMode) {
    return (
      <Card
        className={isComplete ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50'}
      >
        <CardContent className="pt-6">
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center text-sm">
                <span
                  className={
                    isComplete ? 'text-green-700 font-semibold' : 'text-amber-700 font-semibold'
                  }
                >
                  {isComplete ? '? Configura��o Completa' : 'Configura��o da Cl�nica'}
                </span>
            ...
            <Button
              ...
              className="w-full"
            >
              {isComplete ? 'Ir para Base do Sistema' : 'Completar Configura��o'}
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent className="pt-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <Button onClick={loadWizardStatus} variant="outline" size="sm" className="mt-2">
                  Tentar Novamente
                </Button>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="mt-1">
                Verifique o que j� est� configurado e o que ainda precisa ser ajustado para liberar
                o sistema
              </CardDescription>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              <div className="bg-white rounded-lg p-3 shadow-sm">
                <p className="text-xs text-gray-600">Obrigat�rios</p>
                <p className="text-lg font-bold text-blue-600">
                  {
                    SETUP_WIZARD_STEPS.filter((s) => wizardStatus[s.id]?.completed && s.required)
                      .length
                  }
      ...
      {/* Categories - Modelo correto: diagn�stico + um bot�o por grupo */}
      {Object.entries(SETUP_WIZARD_CATEGORIES).map(([categoryId, category]) => {
        ...
        const completedRequired = categorySteps.filter(
          (s) => s.required && wizardStatus[s.id]?.completed,
        ).length;

        // Determinar status da categoria
        const isCategoryComplete = completedSteps === categorySteps.length;
        const isBlocked =
          categoryId !== 'cadastros-estruturais' &&
          !isWizardComplete(
            SETUP_WIZARD_CATEGORIES['cadastros-estruturais'].steps.reduce((acc, s) => {
        ...
        return (
          ...
          >
                ...
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div className="flex-1">
                    ...
                    <CardDescription className="text-xs mt-1">
                      ...
                      {!isCategoryComplete &&
                        !isBlocked &&
                        `${completedRequired}/${requiredSteps} obrigat�rios conclu�dos`}
                    </CardDescription>
                  </div>
                </div>
            ...
            {/* Conte�do: lista informativa (N�O clic�vel) */}
            <CardContent className="space-y-3">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isCompleted && (
                        <Badge
                          variant="outline"
                          className="bg-green-100 text-green-800 border-green-300 mt-1 text-xs"
                        >
                      ...
                      {!isCompleted && !step.required && (
                        <Badge variant="secondary" className="opacity-70 mt-1 text-xs">
                          Opcional
                        </Badge>
                      )}
                    </div>
              ...
              {/* ? UM �NICO BOT�O POR CATEGORIA (fonte de navega��o segura) */}
              <Button
                disabled={isBlocked}
                ...
                title={
                  isBlocked
                    ? 'Complete a categoria anterior'
                    : `Acessar ${category.title} no menu lateral`
                }
              >
      ...
      {/* ?? PR�XIMOS PASSOS - APENAS TEXTO (sem navega��o) */}
      {!isComplete && (
        <Card className="border-blue-200 bg-blue-50">
          <CardHeader>
            ...
            <CardDescription>Siga este roteiro para completar a configura��o</CardDescription>
          </CardHeader>
          <CardContent>
            <ol className="space-y-3">
              <li className="flex gap-3">
                ...
                <span className="text-sm text-gray-700">
                  Cadastre os <strong>servi�os</strong> que sua cl�nica oferece
                </span>
              </li>
              <li className="flex gap-3">
                <span className="flex-shrink-0 font-bold text-blue-600 min-w-6">2.</span>
                <span className="text-sm text-gray-700">
                  Cadastre os <strong>profissionais</strong> que prestam os servi�os
                </span>
              </li>
              <li className="flex gap-3">
                <span className="flex-shrink-0 font-bold text-blue-600 min-w-6">3.</span>
                <span className="text-sm text-gray-700">
                  Vincule os profissionais aos servi�os que podem realizar
                </span>
              </li>
              <li className="flex gap-3">
                <span className="flex-shrink-0 font-bold text-blue-600 min-w-6">4.</span>
                <span className="text-sm text-gray-700">
                  Configure as <strong>regras de agenda</strong> para cada servi�o
                </span>
              </li>
              <li className="flex gap-3">
                <span className="flex-shrink-0 font-bold text-blue-600 min-w-6">5.</span>
                <span className="text-sm text-gray-700">
                  Defina os <strong>pre�os</strong> e <strong>repasses</strong> financeiros
                </span>
              </li>
            </ol>
            <p className="text-xs text-gray-600 mt-4 p-3 bg-white rounded border border-blue-100">
              ? <strong>Dica:</strong> Use o menu lateral para acessar cada se��o. O assistente aqui
              mostra apenas o status de configura��o.
            </p>
          </CardContent>
        </Card>
      )}
        ...
        <Card className="border-green-200 bg-gradient-to-r from-green-50 to-emerald-50 shadow-lg">
          <CardContent className="pt-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <p className="text-base font-bold text-green-900">?? Configura��o Completa!</p>
                <p className="text-sm text-green-800 mt-2">
                  Todos os passos obrigat�rios foram conclu�dos. Sua cl�nica est� pronta para usar
                ...
                <div className="flex gap-3 mt-4">
                  ...
                  <Button onClick={() => navigate('/clinica/dashboard')} variant="outline">
                    Dashboard
                  </Button>
                </div>
              </div>
