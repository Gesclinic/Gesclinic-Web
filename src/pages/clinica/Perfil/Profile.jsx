  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Edite seus dados de perfil, incluindo nome, telefone e foto." />
      </Helmet>
        ...
        <Card className="shadow-lg border-gray-100/10">
          <CardHeader className="flex flex-col items-center text-center">
            ...
            <div className="relative mt-6">
              {preview ? (
                <img src={preview} alt="Avatar" className="h-28 w-28 rounded-full object-cover border-4 border-white shadow-lg" />
              ) : (
        ...
        <Card className="mt-6">
          ...
          <CardContent>
            <div className="space-y-2">
              <div><b>Nome:</b> {form.full_name || user?.user_metadata?.full_name || user?.name || "-"}</div>
              <div><b>Email:</b> {user?.email || "-"}</div>
              <div><b>Telefone:</b> {form.phone || user?.user_metadata?.phone || "-"}</div>
              <div><b>Função:</b> {authProfile?.role || user?.user_metadata?.role || "-"}</div>
            </div>
          </CardContent>
        </Card>
