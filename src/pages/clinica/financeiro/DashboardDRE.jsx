...
export default function DashboardDRE() {
  ...
  const loadData = async () => {
    try {
      ...
      const dreData = await financialAccountsApi.calculateDREForPeriod(clinicId, startDate, endDate);
      setDre(dreData);
  ...
  const dreLineData = [
    ...
    { stage: "Receita\nLíquida", value: dre?.receitaLiquida || 0, formatted: formatBRL(dre?.receitaLiquida || 0) },
    { stage: "Margem\nBruta", value: dre?.margemBruta || 0, formatted: formatBRL(dre?.margemBruta || 0) },
    { stage: "EBITDA", value: dre?.ebitda || 0, formatted: formatBRL(dre?.ebitda || 0) },
    { stage: "Lucro\nOperacional", value: dre?.lucroOperacional || 0, formatted: formatBRL(dre?.lucroOperacional || 0) },
    { stage: "Lucro\nLíquido", value: dre?.lucroLiquido || 0, formatted: formatBRL(dre?.lucroLiquido || 0) }
  ];
    ...
    <div className="space-y-6 pb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "Margem Operacional", value: `${metrics.margemOperacional}%`, icon: "📊", color: "blue" },
          { label: "Margem Líquida", value: `${metrics.margemLiquida}%`, icon: "💰", color: "purple" },
        ...
        <CardContent>
          <div className="space-y-1">
            ...
            <div className="flex justify-between items-center p-3 bg-green-50 dark:bg-green-950/20 rounded-lg border-l-4 border-green-500">
              <span className="font-semibold text-green-900 dark:text-green-100">Receita Bruta</span>
              <span className="text-lg font-bold text-green-700 dark:text-green-300">{formatBRL(dre?.receita || 0)}</span>
            </div>
            ...
            <div className="flex justify-between items-center p-3 bg-red-50 dark:bg-red-950/20 rounded-lg border-l-4 border-red-500 ml-6">
              <span className="text-slate-600 dark:text-slate-400">(-) Deduções (Impostos/Glosas)</span>
              <span className="text-red-700 dark:text-red-300 font-semibold">-{formatBRL(dre?.deducao || 0)}</span>
            </div>
            ...
            <div className="flex justify-between items-center p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg border-l-4 border-blue-500 font-bold my-2">
              ...
              <span className="text-xl text-blue-700 dark:text-blue-300">{formatBRL(dre?.receitaLiquida || 0)}</span>
            </div>
            ...
            <div className="flex justify-between items-center p-3 bg-orange-50 dark:bg-orange-950/20 rounded-lg border-l-4 border-orange-500 ml-6">
              <span className="text-slate-600 dark:text-slate-400">(-) Custos Diretos (Repasse/Materiais)</span>
              <span className="text-orange-700 dark:text-orange-300 font-semibold">-{formatBRL(dre?.custos || 0)}</span>
            </div>
            ...
            <div className="flex justify-between items-center p-3 bg-emerald-50 dark:bg-emerald-950/20 rounded-lg border-l-4 border-emerald-500 font-bold my-2">
              <span className="text-emerald-900 dark:text-emerald-100">= Margem Bruta ({(dre?.margemBrutaPct || 0).toFixed(1)}%)</span>
              <span className="text-xl text-emerald-700 dark:text-emerald-300">{formatBRL(dre?.margemBruta || 0)}</span>
            </div>
            ...
            <div className="flex justify-between items-center p-3 bg-slate-100 dark:bg-slate-800 rounded-lg border-l-4 border-slate-400 ml-6">
              <span className="text-slate-600 dark:text-slate-400">(-) Despesas Administrativas</span>
              <span className="text-slate-700 dark:text-slate-300 font-semibold">-{formatBRL(dre?.despesasAdmin || 0)}</span>
            </div>
            <div className="flex justify-between items-center p-3 bg-slate-100 dark:bg-slate-800 rounded-lg border-l-4 border-slate-400 ml-6">
              <span className="text-slate-600 dark:text-slate-400">(-) Despesas da Clínica</span>
              <span className="text-slate-700 dark:text-slate-300 font-semibold">-{formatBRL(dre?.despesasClinica || 0)}</span>
            </div>
            <div className="flex justify-between items-center p-3 bg-slate-100 dark:bg-slate-800 rounded-lg border-l-4 border-slate-400 ml-6">
              <span className="text-slate-600 dark:text-slate-400">(-) Despesas Comerciais</span>
              <span className="text-slate-700 dark:text-slate-300 font-semibold">-{formatBRL(dre?.despesasComercial || 0)}</span>
            </div>
            <div className="flex justify-between items-center p-3 bg-slate-100 dark:bg-slate-800 rounded-lg border-l-4 border-slate-400 ml-6">
              <span className="text-slate-600 dark:text-slate-400">(-) Despesas Financeiras</span>
              <span className="text-slate-700 dark:text-slate-300 font-semibold">-{formatBRL(dre?.despesasFinanceira || 0)}</span>
            </div>
            ...
            <div className="flex justify-between items-center p-3 bg-indigo-50 dark:bg-indigo-950/20 rounded-lg border-l-4 border-indigo-500 font-bold my-2">
              <span className="text-indigo-900 dark:text-indigo-100">= EBITDA ({(dre?.ebitdaPct || 0).toFixed(1)}%)</span>
              <span className="text-xl text-indigo-700 dark:text-indigo-300">{formatBRL(dre?.ebitda || 0)}</span>
            </div>

            {/* Depreciação */}
            <div className="flex justify-between items-center p-3 bg-slate-100 dark:bg-slate-800 rounded-lg border-l-4 border-slate-400 ml-6">
              <span className="text-slate-600 dark:text-slate-400">(-) Depreciação</span>
              <span className="text-slate-700 dark:text-slate-300 font-semibold">-{formatBRL(dre?.depreciacao || 0)}</span>
            </div>
            ...
            <div className="flex justify-between items-center p-3 bg-purple-50 dark:bg-purple-950/20 rounded-lg border-l-4 border-purple-500 font-bold my-2">
              <span className="text-purple-900 dark:text-purple-100">= Lucro Operacional ({(dre?.lucroOperacionalPct || 0).toFixed(1)}%)</span>
              <span className="text-xl text-purple-700 dark:text-purple-300">{formatBRL(dre?.lucroOperacional || 0)}</span>
            </div>
            ...
            <div className="flex justify-between items-center p-3 bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-950/30 dark:to-green-950/30 rounded-lg border-l-4 border-green-600 font-bold my-3">
              <span className="text-green-900 dark:text-green-100 text-lg">= Lucro Líquido ({(dre?.lucroLiquidoPct || 0).toFixed(1)}%)</span>
              <span className="text-2xl text-green-700 dark:text-green-300 font-extrabold">{formatBRL(dre?.lucroLiquido || 0)}</span>
            </div>
          </div>
        </CardContent>
      </Card>
