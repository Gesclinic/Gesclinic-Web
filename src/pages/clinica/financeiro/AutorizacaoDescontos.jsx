...
export default function AutorizacaoDescontos() {
  ...
  const loadDescontos = async () => {
    ...
    try {
      ...
      if (filterStatus === 'pendentes') {
        ...
        query = query
          .gt('discount', 0)
          .is('discount_authorized_by', null)
          .is('discount_rejected_at', null);
  ...
  return (
    <PageLayout
      title="⚙️ Autorização de Descontos"
      breadcrumbs={[
          ...
          <Card className="p-8 text-center">
            <p className="text-gray-500 text-lg">
              {filterStatus === 'pendentes'
                ? 'Nenhum desconto pendente de autorização'
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600">
                      📅 {new Date(desconto.scheduled_date).toLocaleDateString('pt-BR')} às{' '}
                      {desconto.scheduled_time}
                    </p>
                    ...
                    <p className="text-sm text-gray-600">
                      👤 {desconto.professionals?.name || 'Profissional não informado'} |{' '}
                      {desconto.payers?.name || 'Particular'}
                    </p>
                    ...
                    <div className="flex gap-4 mt-2 text-sm font-semibold">
                      <span className="text-gray-700">
                        ...
                        <span className="text-green-700">
                          R$ {desconto.value ? parseFloat(desconto.value).toFixed(2) : '0.00'}
                        </span>
                      </span>
                      <span className="text-yellow-700">
                        🔖 Desconto:{' '}
                        <span className="text-yellow-900">
                          R$ {desconto.discount ? parseFloat(desconto.discount).toFixed(2) : '0.00'}
                        </span>
                      </span>
                      <span className="text-blue-700">
                        ...
                        <span className="text-blue-900">
                          R${' '}
                          {(
                            parseFloat(desconto.value || 0) - parseFloat(desconto.discount || 0)
                          ).toFixed(2)}
                        </span>
                      </span>
                    </div>
                  ...
                  <div className="text-2xl">{expandedId === desconto.id ? '▼' : '▶'}</div>
                </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-200 space-y-3">
                        ...
                        <div className="space-y-1">
                          <p className="text-sm text-gray-900">
                            <span className="font-semibold">
                              ...
                              {desconto.discount_requested_by_name ||
                                '(Sem informação de quem solicitou)'}
                            </span>
                          </p>
                          <p className="text-sm text-gray-600">
                            📅{' '}
                    ...
                    {!desconto.discount_authorized_by && (
                      <div className="flex gap-2 pt-3 border-t border-gray-200">
                        <Button
                          onClick={() =>
                            autorizarDesconto(desconto.id, parseFloat(desconto.discount || 0))
                          }
                        ...
                        <Button
                          onClick={() =>
                            rejeitarDesconto(desconto.id, parseFloat(desconto.discount || 0))
                          }
                          className="flex-1 gap-2"
                          variant="outline"
                        >
