...
export default function FluxoCaixa() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        setInitialBalances(JSON.parse(raw));
      }
    } catch {}
  }, []);
  useEffect(() => {
    try {
      localStorage.setItem('cashflow_initial_balances', JSON.stringify(initialBalances));
    } catch {}
  }, [initialBalances]);
  ...
  const {
    data: cachedFluxoData,
    loading: fluxoLoading,
    refresh: refreshFluxo,
  } = useDataCache({
    key: cacheKeyFluxo,
    fetcher: async () => {
  ...
  useEffect(() => {
    if (cachedFluxoData) {
      ...
      setSummary(
        cachedFluxoData.summary || { entradas: 0, saidas: 0, resultado_liquido: 0, saldo_final: 0 },
      );
    }
  }, [cachedFluxoData]);
  ...
  const saveTransfer = async () => {
    try {
      ...
      toast({
        ...
        description: 'Entrada e saída vinculadas criadas.',
      });
    } catch (e) {
  ...
  const deleteRow = async (row) => {
    if (row.origin !== 'manual' || row.is_reconciled) {
      toast({
        ...
        description: 'Só é possível excluir lançamentos manuais não conciliados.',
        variant: 'destructive',
      });
      return;
    }
  ...
  const accountBalances = useMemo(() => {
    ...
    return items.map((it) => ({
      ...
      total: it.total,
      name:
        accounts.find((a) => a.id === it.id)?.name ||
        accounts.find((a) => a.id === it.id)?.bank_name ||
        (it.id === 'sem-conta' ? 'Sem Conta' : it.id),
    }));
  }, [rows, accounts]);

  // Running balance per row, considering selected account (or overall if none)
  const runningBalanceMap = useMemo(() => {
    const relevant = filters.accountId
      ? rows.filter((r) => r.account_id === filters.accountId)
      : rows;
      ...
      <Card className="p-6 mt-4">
        <div className="grid md:grid-cols-6 gap-3 mb-6">
          ...
          <div className="flex gap-2">
            <Button onClick={refreshFluxo} className="flex-1">
              Filtrar
            </Button>
        ...
        <div className="grid md:grid-cols-4 gap-3 mb-4">
          {costCenters.length > 0 && (
            <Select
              ...
              onValueChange={(v) =>
                setFilters((f) => ({ ...f, costCenterId: v === 'all' ? '' : v }))
              }
            >
              <SelectTrigger>
            ...
            <div className="flex items-end gap-2 md:col-span-2">
              <div className="flex-1">
                ...
                <Input
                  type="number"
                  step="0.01"
                  value={filters.accountId ? (initialBalances[filters.accountId] ?? '') : ''}
                  onChange={(e) =>
                    filters.accountId &&
                    setInitialBalances((b) => ({
                      ...b,
                      [filters.accountId]: Number(e.target.value),
                    }))
                  }
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setInitialBalances((b) => ({
                    ...b,
                    [filters.accountId]: Number(b[filters.accountId] || 0),
                  }));
                  toast({
                    title: 'Saldo inicial salvo',
                    description: 'Usado para o saldo diário da conta filtrada.',
                  });
                }}
              >
                Salvar saldo
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(v) => setFormManual((s) => ({ ...s, type: v }))}
                        >
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-6 space-y-5">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormManual((s) => ({ ...s, description: e.target.value }))
                        }
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-6 space-y-5">
                    <div className="flex items-center gap-3 border-b pb-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          Categoria e centro de custo (opcional)
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Select
                          value={formManual.category_id || 'none'}
                          onValueChange={(v) =>
                            setFormManual((s) => ({ ...s, category_id: v === 'none' ? '' : v }))
                          }
                        >
                          <SelectTrigger className="w-full px-3 py-2 border border-gray-300 rounded-lg">
                      ...
                      {costCenters.length > 0 && (
                        <div className="space-y-2">
                          ...
                          <Select
                            value={formManual.cost_center_id || 'none'}
                            onValueChange={(v) =>
                              setFormManual((s) => ({
                                ...s,
                      ...
                      {accounts.length > 0 && (
                        <div className="space-y-2">
                          ...
                          <Select
                            value={formManual.account_id || 'none'}
                            onValueChange={(v) =>
                              setFormManual((s) => ({ ...s, account_id: v === 'none' ? '' : v }))
                            }
                          >
                            <SelectTrigger className="w-full px-3 py-2 border border-gray-300 rounded-lg">
                              <SelectValue placeholder="Selecione..." />
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-purple-100 text-sm">
                          Transfira fundos entre as contas da clínica
                        </p>
                      </div>
                    </div>
                    <Button
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setFormTransfer((s) => ({ ...s, amount: e.target.value }))
                          }
                          placeholder="0,00"
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormTransfer((s) => ({ ...s, description: e.target.value }))
                        }
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Select
                          value={formTransfer.account_from}
                          onValueChange={(v) => setFormTransfer((s) => ({ ...s, account_from: v }))}
                        >
                          <SelectTrigger className="w-full px-3 py-2 border border-gray-300 rounded-lg">
                            <SelectValue placeholder="Selecione..." />
                          </SelectTrigger>
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          value={formTransfer.account_to}
                          onValueChange={(v) => setFormTransfer((s) => ({ ...s, account_to: v }))}
                        >
                          <SelectTrigger className="w-full px-3 py-2 border border-gray-300 rounded-lg">
                            <SelectValue placeholder="Selecione..." />
                          </SelectTrigger>
                    ...
                    {accounts.length < 2 && (
                      <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
                        <p className="text-sm text-amber-800">
                          ⚠️ É necessário cadastrar pelo menos 2 contas para realizar
                          transferências.
                        </p>
                      </div>
                    )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <Card
            className={`p-4 bg-gradient-to-br ${(summary.saldo_final || 0) >= 0 ? 'from-purple-50 to-purple-100 border-purple-200' : 'from-orange-50 to-orange-100 border-orange-200'}`}
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">Saldo final</p>
                <p
                  className={`text-2xl font-bold mt-1 ${(summary.saldo_final || 0) >= 0 ? 'text-purple-600' : 'text-orange-600'}`}
                >
                  {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(
                    summary.saldo_final || 0,
                  )}
                </p>
              </div>
              <Tag
                className={`w-8 h-8 ${(summary.saldo_final || 0) >= 0 ? 'text-purple-400' : 'text-orange-400'}`}
              />
            </div>
          </Card>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-2">
            {accountBalances.map((acc) => (
              <Card key={acc.id} className="p-3">
                ...
                <div
                  className={`text-lg font-bold ${acc.total >= 0 ? 'text-blue-600' : 'text-red-600'}`}
                >
                  {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(
                    acc.total,
                  )}
                      ...
                      {runningBalanceMap.has(r.id) ? (
                        <span className="font-semibold">
                          {new Intl.NumberFormat('pt-BR', {
                            style: 'currency',
                            currency: 'BRL',
                          }).format(runningBalanceMap.get(r.id))}
                        </span>
                      ) : (
