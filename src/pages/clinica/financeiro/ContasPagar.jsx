...
import { Plus, Search, Trash2, ChevronUp, ChevronDown, ChevronRight, Info, Pencil, Paperclip, AlertTriangle, Clock, CalendarDays, CircleDollarSign, Download, Filter, Check, AlertCircle } from "lucide-react";
import SummaryCards from '@/components/clinica/financeiro/SummaryCards';
import { formatBRL } from '@/utils/formatCurrency';
import { toDate, todayStart } from '@/utils/dateUtils';
...
import { listAPQuery, deleteAP, deleteAPBulk, updateAPBulk, payAccountsPayableBatch, listAccountPlans, listVendorNames, listPaymentMethods } from "@/lib/financeApi";
import { parseSearchGeneral, parcelLabel } from "@/utils/financeHelpers";
  ...
  <React.Fragment key={item.id}>
    <tr className={`${isOverdue(item) ? 'bg-red-50' : ''} border-b`}> 
      <td className="px-3 py-3 w-[26%] truncate text-left">
        <div className="inline-flex items-center gap-2">
          <button type="button" className="text-gray-600 hover:text-gray-800" onClick={() => toggleExpanded(item.id)}>
            {expanded.has(item.id) ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
          </button>
          {item.description ? (
            <button className="text-blue-600 hover:underline"
              onClick={() => { setViewItem(item); setViewDialogOpen(true); }}
            >{item.description}</button>
          ) : '-'}
        </div>
      </td>
      <td className="px-3 py-3 w-[20%] truncate text-left">{item.vendor_name || '-'}</td>
      <td className="px-3 py-3 w-[14%] hidden md:table-cell truncate text-left">{item.category_id ? (costCenterMap[item.category_id] || '—') : '—'}</td>
      <td className={`px-3 py-3 w-[12%] ${isOverdue(item) ? 'text-red-600' : ''} whitespace-nowrap text-left`}>{item.due_date ? new Date(item.due_date).toLocaleDateString('pt-BR') : '-'}</td>
      <td className="px-3 py-3 w-[12%] font-semibold whitespace-nowrap text-right">{formatBRL(item.amount)}</td>
      <td className="px-3 py-3 w-[10%] text-left"><StatusBadge status={isOverdue(item) ? 'overdue' : (item.status || 'open').toLowerCase()} /></td>
    ...
    <tr className="bg-gray-50 border-b" aria-expanded={expanded.has(item.id)}>
        <td colSpan="7" className="px-4 py-2">
          <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 text-sm overflow-hidden transition-all duration-300 ease-in-out ${expanded.has(item.id) ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
            <div>
              <div className="text-xs text-gray-500">Emissão</div>
              <div>{item.issue_date ? new Date(item.issue_date).toLocaleDateString('pt-BR') : '—'}</div>
            </div>
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                {item.document_url && (
                  <a href={item.document_url} target="_blank" rel="noopener noreferrer" title="Abrir anexo" className="text-gray-600 hover:text-blue-600">
...
export default function ContasPagar() {
  ...
  const sortIndicator = (field) => {
    if (sortBy !== field) return <span className="inline-block align-middle opacity-40"><ChevronUp className="w-3 h-3" /></span>;
    return sortDir === 'asc' ? <ChevronUp className="w-3 h-3 inline-block align-middle" /> : <ChevronDown className="w-3 h-3 inline-block align-middle" />;
  };

  const apiOrderBy = (sb) => sb;

  const paymentMethodSuggestions = useMemo(() => ['PIX','Cartão','Dinheiro','Transferência','Boleto'], []);
  ...
  const costCenterMap = useMemo(() => {
    const map = {};
    for (const c of costCenters) map[c.id] = c.name;
    return map;
  }, [costCenters]);
  ...
  const mergedPaymentMethods = useMemo(() => {
    const seen = new Set();
    const out = [];
    for (const m of paymentMethodSuggestions) { const t = m.trim(); if (t && !seen.has(t)) { seen.add(t); out.push(t); } }
  ...
  const summaryVisual = useMemo(() => {
    const total = items.reduce((sum, it) => sum + (parseFloat(it.amount) || 0), 0);
    const paidAmount = items.filter(it => (it.status || '').toLowerCase() === 'paid').reduce((sum, it) => sum + (parseFloat(it.amount) || 0), 0);
    const pendingAmount = items.filter(it => (it.status || '').toLowerCase() !== 'paid').reduce((sum, it) => sum + (parseFloat(it.amount) || 0), 0);
    const overdueAmount = items.filter(it => isOverdue(it)).reduce((sum, it) => sum + (parseFloat(it.amount) || 0), 0);
    ...
    return { total, paidAmount, pendingAmount, overdueAmount, paidCount, pendingCount, overdueCount };
  }, [items, isOverdue]);

  // 📄 Pagination: 30 items per page
  const { items: paginatedItems, pageNum, totalPages, nextPage, prevPage, goToPage } = usePagination(sortedItems, 30);
  ...
  const parseNfSummaryFromNotes = useCallback((notes) => {
    ...
    try {
      ...
      if (itemsBlockIdx >= 0) {
        ...
        if (subtotalMatch) res.subtotal = Number(subtotalMatch[1]);
        const taxesLine = block.match(/Impostos\s*\(NF\):([^|]+)\|\s*Total=([0-9]+(?:\.[0-9]{1,2})?)/);
        if (taxesLine) {
          ...
          const parseTax = (label) => {
            const m = taxesStr.match(new RegExp(label + "\\s*[^=]*=\\s*([0-9]+(?:\\.[0-9]{1,2})?)"));
            if (m) map[label] = Number(m[1]);
          };
          ['IR', 'CSLL', 'PIS/COFINS', 'ISS', 'ICMS'].forEach(parseTax);
          res.taxes = map;
        }
        const totalAfterMatch = block.match(/Total\s*\(após\s*impostos[^)]*\):\s*([0-9]+(?:\.[0-9]{1,2})?)/);
        if (totalAfterMatch) res.totalAfter = Number(totalAfterMatch[1]);
      }
    } catch {}
    return res;
  }, []);
  ...
  const saveFavorite = () => {
    ...
    setFavorites(next);
    try { localStorage.setItem('ap_filter_favorites', JSON.stringify(next)); } catch {}
  ...
  const loadBills = useCallback(async ({ clinicId: cId, vendor, paymentMethod, start, end, search, status, amountMin, amountMax, categoryId, searchDateIso, orderBy, orderDir, autoSelect = false } = {}) => {
    if (!cId) return;
    setLoading(true);
    try {
      ...
      if (autoSelect) setAutoSelectNext(false);
    } catch (error) {
        ...
        <div className="flex gap-2 items-center">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onSelect={() => setCostCenterDialogOpen(true)}>Alterar centro de custo</DropdownMenuItem>
              <DropdownMenuItem
                onSelect={() => {
                  const ids = Array.from(selected);
                  const rows = items.filter(it => ids.includes(it.id));
                  const headers = ['Descrição','Fornecedor','Vencimento','Status','Valor','ID'];
                  const toCsvRow = (r) => [
                    r.description || '',
                    r.vendor_name || '',
                    r.due_date || '',
                  ...
                  const csv = [headers.join(',') , ...rows.map(r => '"' + toCsvRow(r).join('","') + '"')].join('\n');
                  const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
                  const url = URL.createObjectURL(blob);
                  const a = document.createElement('a');
                  a.href = url; a.download = `contas_pagar_export_${Date.now()}.csv`; a.click();
                  URL.revokeObjectURL(url);
                }}
              >Gerar comprovante (CSV)</DropdownMenuItem>
              <DropdownMenuItem
                onSelect={async () => {
                  const ids = Array.from(selected);
                  ...
                  try {
                    ...
                    toast({ title: 'Falha ao atualizar', description: String(e.message || e), variant: 'destructive' });
                  }
                  triggerLoad(false);
                }}
              >Marcar como paga</DropdownMenuItem>
              ...
              <DropdownMenuItem
                onSelect={async () => {
                  ...
                  try {
                    ...
                    toast({ title: 'Falha ao excluir', description: String(e.message || e), variant: 'destructive' });
                  }
                  triggerLoad(false);
                }}
              >Excluir selecionados</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      }
    >
          ...
          <Card className="p-4 bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{currency.format(summaryVisual.total)}</p>
          ...
          <Card className="p-4 bg-gradient-to-br from-green-50 to-green-100 border-green-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{currency.format(summaryVisual.paidAmount)}</p>
          ...
          <Card className="p-4 bg-gradient-to-br from-orange-50 to-orange-100 border-orange-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">{currency.format(summaryVisual.pendingAmount)}</p>
          ...
          <Card className="p-4 bg-gradient-to-br from-red-50 to-red-100 border-red-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">{currency.format(summaryVisual.overdueAmount)}</p>
      ...
      <Card className="p-4 w-full">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4 pb-4 border-b">
              <div>
                ...
                {vendorOptions.length > 0 ? (
                  <select className="w-full border rounded h-8 px-2 text-sm" value={vendorFilter} onChange={(e) => setVendorFilter(e.target.value)}>
                  ...
                  <Input className="h-8 text-sm" placeholder="Fornecedor" value={vendorFilter} onChange={(e) => setVendorFilter(e.target.value)} />
                )}
              </div>
              <div>
                <Label className="text-xs text-gray-600">Centro de custo</Label>
                {costCenters.length > 0 ? (
                  <select className="w-full border rounded h-8 px-2 text-sm" value={costCenterFilter} onChange={(e) => setCostCenterFilter(e.target.value)}>
                    <option value="">Todos</option>
                    {costCenters
                  ...
                  <Input className="h-8 text-sm" placeholder="Centro de custo" value={costCenterFilter} onChange={(e) => setCostCenterFilter(e.target.value)} />
                )}
              </div>
              <div>
                <Label className="text-xs text-gray-600">Método</Label>
                <select className="w-full border rounded h-8 px-2 text-sm" value={paymentMethodFilter} onChange={(e) => setPaymentMethodFilter(e.target.value)}>
                  <option value="">Todos</option>
              ...
              <div>
                ...
                <select className="w-full border rounded h-8 px-2 text-sm" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
                  <option value="">Todos</option>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4 pb-4 border-b">
              <div>
                ...
                <Input className="h-8 text-sm" type="date" value={startFilter} onChange={(e) => setStartFilter(e.target.value)} />
              </div>
              <div>
                <Label className="text-xs text-gray-600">Vencimento até</Label>
                <Input className="h-8 text-sm" type="date" value={endFilter} onChange={(e) => setEndFilter(e.target.value)} />
              </div>
              <div>
                <Label className="text-xs text-gray-600">Valor (mín)</Label>
                <Input className="h-8 text-sm" type="number" step="0.01" value={amountMinFilter} onChange={(e) => setAmountMinFilter(e.target.value)} />
              </div>
              <div>
                <Label className="text-xs text-gray-600">Valor (máx)</Label>
                <Input className="h-8 text-sm" type="number" step="0.01" value={amountMaxFilter} onChange={(e) => setAmountMaxFilter(e.target.value)} />
              </div>
            </div>

            {/* BARRA DE AÇÕES */}
            <div className="flex gap-2">
              <Button size="sm" className="bg-blue-600" onClick={() => triggerLoad(autoSelectNext)} disabled={loading}>{loading?"Filtrando...":"Pesquisar"}</Button>
              <Button size="sm" variant="outline" onClick={() => {
                ...
                loadBills({ clinicId, vendor: '', paymentMethod: '', start: null, end: null, search: '', status: null, amountMin: null, amountMax: null, categoryId: null, orderBy: apiOrderBy(sortBy), orderDir: sortDir, autoSelect: false });
              }}>Limpar</Button>
              <Button size="sm" variant="ghost" className="ml-auto"><Download className="w-4 h-4 mr-2" />Exportar</Button>
            </div>
          </div>
        ) : (
          <div className="flex gap-2 items-center">
            <div className="relative flex-1">
              ...
              <Input placeholder="Buscar fornecedor, descrição..." className="pl-9" value={searchFilter} onChange={(e) => setSearchFilter(e.target.value)} />
            </div>
            <Button size="sm" onClick={() => triggerLoad(false)} disabled={loading}>Buscar</Button>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-2 items-center mb-2">
          <Button size="sm"
            variant="outline"
            onClick={() => { setFavName(selectedFavorite || ''); setOverwriteKey(selectedFavorite || ''); setSaveFavDialogOpen(true); }}
            title="Salva os filtros atuais como favorito"
          >Salvar favorito</Button>
          <select className="border rounded h-8 px-2 text-sm" value={selectedFavorite} onChange={(e) => setSelectedFavorite(e.target.value)}>
            <option value="">Meus favoritos</option>
            {favorites.map(f => <option key={f.name} value={f.name}>{f.name}</option>)}
          </select>
          <Button size="sm" variant="outline" onClick={applyFavorite} disabled={!selectedFavorite} title="Carrega o filtro salvo e aplica na lista">Carregar favorito</Button>
          <Button size="sm"
            variant="outline"
            onClick={() => {
              if (!selectedFavorite) { toast({ variant: 'destructive', title: 'Selecione um favorito' }); return; }
              ...
              if (list.some((f, i) => f.name === trimmed && i !== idx)) {
                const ok = window.confirm('Já existe favorito com esse nome. Deseja sobrescrever?');
                if (!ok) return;
                list = list.filter((f, i) => !(f.name === trimmed && i !== idx));
              }
              list[idx] = { ...list[idx], name: trimmed };
              setFavorites(list);
              try { localStorage.setItem('ap_filter_favorites', JSON.stringify(list)); } catch {}
              setSelectedFavorite(trimmed);
              toast({ title: 'Favorito renomeado', description: `${selectedFavorite} → ${trimmed}` });
            }}
        ...
        {startFilter && endFilter && (
          <div className="text-xs text-gray-500 mb-4">Período: {fmtBR(startFilter)} → {fmtBR(endFilter)}</div>
        )}
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          >Este mês</Button>
          <Button variant="outline" onClick={() => { setStartFilter(''); setEndFilter(''); }}>Limpar período</Button>
        </div>
        ...
        <div className="overflow-x-visible">
          <table className="w-full text-sm table-fixed">
            <thead className="bg-gray-50 sticky top-0 z-10">
              <tr>
                <th className="text-left px-3 py-2 w-[26%] cursor-pointer" onClick={() => toggleSort('description')}>Descrição {sortIndicator('description')}</th>
                <th className="text-left px-3 py-2 w-[20%] cursor-pointer" onClick={() => toggleSort('vendor_name')}>Fornecedor {sortIndicator('vendor_name')}</th>
                <th className="text-left px-3 py-2 w-[14%] hidden md:table-cell cursor-pointer" onClick={() => toggleSort('category_name')}>Centro de custo {sortIndicator('category_name')}</th>
                <th className="text-left px-3 py-2 w-[12%] cursor-pointer whitespace-nowrap" onClick={() => toggleSort('due_date')}>Vencimento {sortIndicator('due_date')}</th>
                <th className="text-right px-3 py-2 w-[12%] cursor-pointer whitespace-nowrap" onClick={() => toggleSort('amount')}>Valor {sortIndicator('amount')}</th>
                <th className="text-left px-3 py-2 w-[10%]">Status</th>
                <th className="px-3 py-2 w-[6%]">
                  <span className="inline-flex items-center justify-center gap-2 w-full">Ações
                    <Checkbox
                      className="h-4 w-4"
                      checked={paginatedItems.length > 0 && paginatedItems.every(it => selected.has(it.id))}
                      onCheckedChange={(v) => {
                        if (v) setSelected(new Set(paginatedItems.map(it => it.id)));
            ...
            <tbody>
              {sortedItems.length === 0 ? (
                <tr>
                  <td colSpan="6" className="text-center py-10 text-gray-500">Nenhuma conta registrada.</td>
                </tr>
              ) : (
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between p-4 border-t">
              <div className="text-sm text-gray-600">
                Página <span className="font-semibold">{pageNum + 1}</span> de <span className="font-semibold">{totalPages}</span> 
                ({sortedItems.length} contas)
              </div>
              <div className="flex gap-2">
                <Button 
                  size="sm" 
      ...
      <Dialog open={costCenterDialogOpen} onOpenChange={setCostCenterDialogOpen}>
        <DialogContent className="app-dialog-shell app-dialog-shell--compact">
          ...
          <div className="space-y-3">
            ...
            {costCenters.length > 0 ? (
              <select className="w-full border rounded h-9 px-2 text-sm" value={costCenterId} onChange={(e) => setCostCenterId(e.target.value)}>
              ...
              <Input value={costCenterId} onChange={(e) => setCostCenterId(e.target.value)} placeholder="UUID do centro de custo" />
            )}
          </div>
          <DialogFooter>
            ...
            <Button
              onClick={async () => {
                const ids = Array.from(selected);
                if (ids.length === 0 || !costCenterId) { setCostCenterDialogOpen(false); return; }
                try {
                  const { updated } = await updateAPBulk(ids, { category_id: costCenterId });
                  toast({ title: 'Centro de custo atualizado', description: `${updated} conta(s) atualizada(s).` });
                  
                  // Invalidar cache
                  refreshMetadata();
                } catch (e) {
                  toast({ title: 'Falha ao atualizar centro de custo', description: String(e.message || e), variant: 'destructive' });
                }
        ...
        <DialogContent className="app-dialog-shell app-dialog-shell--compact">
          ...
          <div className="space-y-3">
            <div>
              ...
              <select className="w-full border rounded h-9 px-2 text-sm" value={overwriteKey} onChange={(e) => { setOverwriteKey(e.target.value); setFavName(e.target.value || ''); }}>
            ...
            <div>
              ...
              <Input value={favName} onChange={(e) => setFavName(e.target.value)} placeholder="Ex.: Vencidos do mês" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setSaveFavDialogOpen(false)}>Cancelar</Button>
            <Button
              onClick={() => {
                if (!overwriteKey) { toast({ variant: 'destructive', title: 'Selecione um favorito para sobrescrever' }); return; }
                const fav = {
                  name: overwriteKey,
                  vendorFilter,
                  paymentMethodFilter,
                ...
                toast({ title: 'Favorito sobrescrito', description: `Atualizado: ${overwriteKey}` });
                setSaveFavDialogOpen(false);
              }}
            >Sobrescrever selecionado</Button>
            <Button
              onClick={() => {
                const name = (favName || '').trim();
                if (!name) { toast({ variant: 'destructive', title: 'Informe um nome' }); return; }
                let allow = true;
                if (favorites.some(f => f.name === name)) {
                  allow = window.confirm('Já existe um favorito com este nome. Deseja sobrescrever?');
                }
                if (!allow) return;
                const fav = {
                  name,
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className="app-dialog-shell app-dialog-shell--compact">
          ...
          {viewItem && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              ...
              <div>
                ...
                <div>{viewItem.issue_date ? new Date(viewItem.issue_date).toLocaleDateString('pt-BR') : '—'}</div>
              </div>
              <div>
                ...
                <div className="break-words">
                  ...
                  {viewItem.document_url && (
                    <div className="mt-1">
                      <a href={viewItem.document_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">Abrir anexo</a>
                    </div>
                  )}
                </div>
              </div>
              <div>
                ...
                <div>{viewItem.due_date ? new Date(viewItem.due_date).toLocaleDateString('pt-BR') : '—'}</div>
              </div>
              <div>
                  ...
                  <div className="md:col-span-2 border rounded-md p-3 bg-gray-50 space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                      ...
                      <div>
                        ...
                        <div className="font-medium">{nf.subtotal !== null ? nf.subtotal.toLocaleString('pt-BR',{style:'currency',currency:'BRL'}) : '—'}</div>
                      </div>
                      <div>
                        <div className="text-xs text-gray-600">Total impostos</div>
                        <div className="font-medium">{nf.taxesTotal !== null ? nf.taxesTotal.toLocaleString('pt-BR',{style:'currency',currency:'BRL'}) : '—'}</div>
                      </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-2 text-xs">
                      {['IR','CSLL','PIS/COFINS','ISS','ICMS'].map(k => (
                        <div key={k} className="bg-white rounded p-2 border">
                          ...
                          <div className="font-medium">{typeof nf.taxes[k] === 'number' ? nf.taxes[k].toLocaleString('pt-BR',{style:'currency',currency:'BRL'}) : '—'}</div>
                        </div>
                      ))}
                    </div>
                    <div className="text-sm">Total após impostos: <strong>{(nf.totalAfter ?? Number(viewItem.amount || 0)).toLocaleString('pt-BR',{style:'currency',currency:'BRL'})}</strong></div>
                  </div>
                );
              })()}
            </div>
          ...
          <DialogFooter>
            <Button onClick={() => { if (viewItem) navigate(`/clinica/financeiro/pagar/${viewItem.id}/editar`); }}>Editar</Button>
