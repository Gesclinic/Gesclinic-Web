...
export default function Hierarquia(){
  ...
  useEffect(() => { (async ()=>{
    if (!clinicId) return; setLoading(true);
    const { data } = await supabase.from('cost_centers').select('*').eq('clinic_id', clinicId).order('name');
  ...
  const tree = useMemo(() => {
    ...
    for (const a of map.values()){
      if (a.parent_id && map.has(a.parent_id)) map.get(a.parent_id).children.push(a); else roots.push(a);
    }
    return roots;
  }, [accounts]);
  ...
  const openEdit = (node) => {
    ...
    setSearchParams(prev => { const p = new URLSearchParams(prev); p.set('edit', node.id); return p; });
  };
  ...
  const Row = ({ node, level=0 }) => (
    <div className="flex items-center justify-between py-1" style={{ paddingLeft: `${level*16}px` }}>
      ...
      <div className="flex items-center gap-1">
        ...
        <Button size="sm" variant="ghost" onClick={()=>window.alert('Subcentro em breve')}>Subcentro</Button>
        <Button size="sm" variant="outline" onClick={()=>window.alert('Resultados em breve')}>Ver resultados</Button>
      </div>
    </div>
  );
  ...
  return (
    <div className="border rounded-md p-3">
      {loading ? (<div className="text-sm text-gray-600">Carregando...</div>) : (
        tree.length === 0 ? (<div className="text-sm text-gray-600">Nenhum centro cadastrado.</div>) : (
          tree.map(root => (
      ...
      <Dialog open={!!editing} onOpenChange={(v)=>{ if(!v) setEditing(null); }}>
        <DialogContent className="app-dialog-shell app-dialog-shell--compact">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={()=>{ setEditing(null); setSearchParams(prev => { const p = new URLSearchParams(prev); p.delete('edit'); return p; }); navigate(-1); }}>Cancelar</Button>
