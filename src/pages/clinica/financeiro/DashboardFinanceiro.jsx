...
export default function DashboardFinanceiro() {
  ...
  const { data: cachedKpi, loading: kpiLoading } = useDataCache({
    ...
    fetcher: async () => {
      const { data, error } = await supabase.rpc('cashflow_summary', { p_clinic_id: clinicId, p_start: period.start, p_end: period.end });
        ...
        <TabsContent value="financeiro">
          ...
          <div className="grid md:grid-cols-3 gap-4 mt-4">
            <Card className="p-6">
              ...
              <p className="text-2xl font-bold text-green-700 mt-2">{kpi.entradas.toLocaleString('pt-BR',{style:'currency',currency:'BRL'})}</p>
            </Card>
            <Card className="p-6">
              <h3 className="text-gray-600 text-sm">Saídas do mês</h3>
              <p className="text-2xl font-bold text-red-600 mt-2">{kpi.saidas.toLocaleString('pt-BR',{style:'currency',currency:'BRL'})}</p>
            </Card>
            <Card className="p-6">
              <h3 className="text-gray-600 text-sm">Saldo Atual</h3>
              <p className="text-2xl font-bold text-blue-600 mt-2">{kpi.saldo_final.toLocaleString('pt-BR',{style:'currency',currency:'BRL'})}</p>
            </Card>
          </div>
