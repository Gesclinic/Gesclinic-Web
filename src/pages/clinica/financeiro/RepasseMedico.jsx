...
export default function RepasseMedico() {
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    const { error: rpcError } = await supabase.rpc('generate_doctor_commissions_v2', {
  ...
  const totalGross = commissions.reduce((acc, r) => acc + (r.gross_amount || 0), 0);
  const totalNet = commissions.reduce((acc, r) => acc + (r.net_amount || 0), 0);
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2 mb-4 items-end">
            ...
            <Select onValueChange={setMode} value={mode}>
              ...
              <SelectContent>
                <SelectItem value="atendido">Por Atendimentos Realizados (Produção)</SelectItem>
                <SelectItem value="recebido">Por Pagamentos Recebidos (Caixa)</SelectItem>
              </SelectContent>
            </Select>
            ...
            <Button variant="outline" onClick={exportExcel} disabled={commissions.length === 0}>
          ...
          <Table>
            ...
            <TableBody>
              ...
              {!loading &&
                commissions.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell className="font-medium">{r.professional?.name || 'N/A'}</TableCell>
                    <TableCell className="text-center">{r.total_services}</TableCell>
                    ...
                    <TableCell className="text-center">{r.commission_percent}%</TableCell>
            ...
            <TableFooter>
              <TableRow className="bg-muted/50 font-bold">
                ...
                <TableCell className="text-right">R$ {totalGross.toFixed(2)}</TableCell>
                <TableCell colSpan={3}></TableCell>
                <TableCell className="text-right">R$ {totalNet.toFixed(2)}</TableCell>
