...
export default function EditarConta() {
    ...
    (async () => {
      try {
        ...
        setForm({
          ...
          installments:
            typeof row.installments !== 'undefined' && row.installments !== null
          ...
          linked_revenue:
            typeof row.linked_revenue !== 'undefined' ? String(row.linked_revenue) : '',
        });
        setIsInstallment(typeof row.installments !== 'undefined' && Number(row.installments) > 1);
        setFormItems(
          (items || []).map((it) => ({
            productId: it.stock_item_id || null,
            name: it.name,
            qty: it.qty,
            unit: it.unit_value,
          })),
  ...
  useEffect(() => {
    if (appliedVendorDefaults) {
      return;
    }
    if (!clinicId || !form.vendor_name) {
      return;
    }
    const missingMethod = !form.payment_method;
    const missingCategory = !form.category_id;
    if (!missingMethod && !missingCategory) {
      return;
    }
    (async () => {
      try {
    ...
    try {
      ...
      try {
        if (saved) {
          ...
          if (
            typeof saved.category_id !== 'undefined' &&
            String(saved.category_id || '') !== String(desiredCat || '')
          ) {
            toast({
              title: 'Categoria não atualizada',
              description: 'O centro de custo não foi alterado (restrição de esquema/FK).',
              variant: 'destructive',
            });
          }
          if (
            attachmentFile &&
            docUrl &&
            (!saved.document_url || String(saved.document_url) !== String(docUrl))
          ) {
            toast({
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            <div>
              ...
              <div className="text-base font-semibold text-gray-900">
                {Number(form.amount || 0).toLocaleString('pt-BR', {
                  style: 'currency',
                  currency: 'BRL',
                })}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div className="md:col-span-2 flex items-end">
                <div className="text-xs text-gray-600">
                  Valor automático por parcela e vencimento mensal serão gerados (edição não cria
                  novas linhas).
                </div>
              </div>
            </div>
