...
const AccountDialog = ({ open, onOpenChange, account, onSave, parentOptions }) => {
  ...
  useEffect(() => {
    if (open) { // Only update when dialog is truly open
      if (account) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="app-dialog-shell app-dialog-shell--compact">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div><Label>Conta Pai (opcional)</Label>
...
export default function PlanoContas() {
  ...
  const loadAccounts = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { data, error } = await supabase.from('account_plans').select('*').eq('clinic_id', clinicId).order('name');
    ...
    (async () => {
      try {
        ...
        for (const row of data || []) {
          ...
          if (!cid) continue;
          if (!map[cid]) map[cid] = { count: 0, lastDue: null };
          map[cid].count += 1;
          const cur = row.due_date ? new Date(row.due_date) : null;
          if (cur && (!map[cid].lastDue || cur > map[cid].lastDue)) map[cid].lastDue = cur;
        }
        setUsageInfo(map);
      } catch (e) {
  ...
  const handleSave = async (payload) => {
    try {
      ...
      if(dialog.account?.id) {
        ({ error } = await supabase.from('account_plans').update({...payload, clinic_id: clinicId}).eq('id', dialog.account.id));
      } else {
        ({ error } = await supabase.from('account_plans').insert({...payload, clinic_id: clinicId}));
      }
      if (error) throw error;
  ...
  const handleDelete = async (id) => {
    if (accounts.some(a => a.parent_id === id)) {
        toast({ variant: 'destructive', title: 'Erro ao excluir', description: 'Não é possível excluir uma conta que possui sub-contas.' });
        return;
    }
  ...
  const startInlineEdit = (acc) => {
    setInlineEdits(prev => ({
      ...prev,
      [acc.id]: { name: acc.name || '', type: acc.type || 'receita', parent_id: acc.parent_id || NONE }
    }));
  };

  const cancelInlineEdit = (id) => {
    setInlineEdits(prev => { const { [id]: _, ...rest } = prev; return rest; });
  };
  ...
  const saveInlineEdit = async (acc) => {
    ...
    if (newParent && (acc.children || []).length > 0) {
      toast({ variant: 'destructive', title: 'Estrutura inválida', description: 'Uma conta com sub-contas não pode virar filha (limite de 2 níveis).' });
      return;
    }
  ...
  const AccountRow = ({ acc, level = 0 }) => (
    <>
      <tr className="border-b hover:bg-muted/40">
        <td className="p-2" style={{ paddingLeft: `${0.5 + level * 1.5}rem` }}>
          {editMode && inlineEdits[acc.id] ? (
            <Input className="h-8 text-sm" value={inlineEdits[acc.id].name} onChange={e => setInlineEdits(prev => ({ ...prev, [acc.id]: { ...prev[acc.id], name: e.target.value } }))} />
          ) : (
        ...
        <td className="p-2">
          {editMode && inlineEdits[acc.id] ? (
            <Select value={inlineEdits[acc.id].type} onValueChange={v => setInlineEdits(prev => ({ ...prev, [acc.id]: { ...prev[acc.id], type: v } }))}>
        ...
        <td className="p-2">
          {editMode && inlineEdits[acc.id] ? (
            <Select value={inlineEdits[acc.id].parent_id ?? NONE} onValueChange={v => setInlineEdits(prev => ({ ...prev, [acc.id]: { ...prev[acc.id], parent_id: v } }))}>
        ...
        <td className="p-2 whitespace-nowrap">
          {(() => {
            ...
            return <span>{count} conta(s) • último: {last}</span>;
          })()}
        </td>
        <td className="p-2">
            <div className="flex gap-1">
                {!editMode || !inlineEdits[acc.id] ? (
                  <>
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setDialog({open: true, account: acc})}>
                      <Edit className="w-4 h-4" />
                    </Button>
                    {editMode && (
                      <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => startInlineEdit(acc)} title="Editar estrutura">
                        <Edit className="w-4 h-4" />
                      </Button>
                    )}
                    <Button variant="ghost" size="icon" className="h-7 w-7 hover:bg-destructive/10" onClick={() => handleDelete(acc.id)}>
                      <Trash2 className="w-4 h-4 text-destructive" />
                    </Button>
                  </>
                ) : (
                  <>
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => saveInlineEdit(acc)} title="Salvar">
                      <Save className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => cancelInlineEdit(acc.id)} title="Cancelar">
                      <X className="w-4 h-4" />
                    </Button>
                  </>
                )}
      ...
      {acc.children.map(child => <AccountRow key={child.id} acc={child} level={level + 1} />)}
    </>
  );
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          <Button variant={editMode ? 'default' : 'outline'} onClick={() => setEditMode(v => !v)}>{editMode ? 'Sair do modo edição' : 'Modo edição'}</Button>
          ...
          <Button variant="outline" onClick={async()=>{ try { await applyDefaultAccountPlan(clinicId); toast({ title:'Estrutura padrão aplicada!' }); loadAccounts(); } catch(e){ toast({ variant:'destructive', title:'Falha ao aplicar estrutura', description: e.message }); } }}>
            <Wand2 className="w-4 h-4 mr-2"/> Estrutura Padrão
          </Button>
          <Button variant="destructive" onClick={async()=>{
            if (!window.confirm('Tem certeza que deseja resetar a estrutura? Isso apagará o plano atual e desvinculará categorias das contas a pagar desta clínica.')) return;
            try { await resetAccountPlan(clinicId); toast({ title:'Plano de contas resetado!' }); loadAccounts(); } catch(e){ toast({ variant:'destructive', title:'Falha ao resetar', description: e.message }); }
          }}>Resetar Estrutura</Button>
          <Button onClick={() => setDialog({open: true, account: null})}><PlusCircle className="w-4 h-4 mr-2"/> Nova Conta</Button>
        </div>
      </div>
      <Card>
        <CardHeader>
        ...
        <CardContent>
          {loading ? (<p>Carregando...</p>) : (
            <table className="w-full text-sm">
              ...
              <tbody>
                {structuredAccounts.map(acc => <AccountRow key={acc.id} acc={acc} />)}
              </tbody>
            </table>
          )}
        </CardContent>
