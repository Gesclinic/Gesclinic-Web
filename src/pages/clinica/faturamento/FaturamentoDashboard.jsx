  ...
  const loadBatches = async (batchId, newStatus) => {
    if (batchId && newStatus) {
      setBatches((prevBatches) =>
        prevBatches.map((b) => (b.id === batchId ? { ...b, status: newStatus } : b)),
      );
      return;
    }
    ...
    try {
      ...
      if (filters.payer) {
        query = query.ilike('payer.name', `%${filters.payer}%`);
      }
                  ...
                  batches.map((b) => (
                    <tr key={b.id} className="border-b hover:bg-muted/30 transition-colors">
                      ...
                      <td className="p-3 text-right font-mono">
                        {b.total_valor
                          ? `R$ ${Number(b.total_valor).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`
                          : '—'}
                      </td>
                      <td className="p-3 text-center capitalize">
                        <span
                          className={`px-2 py-1 text-xs font-semibold rounded-full ${b.status === 'aberto' ? 'bg-blue-100 text-blue-800' : b.status === 'fechado' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'}`}
                        >
                          {b.status || 'Aberto'}
                        </span>
                      </td>
