...
export default function RepasseRegrasPage() {
  ...
  const carregarDados = async () => {
    try {
      ...
      const [revenueRes, repasseRes, profsList, svcosList] = await Promise.all([
        supabase.from('revenue_rules').select('*').eq('clinic_id', clinicId),
        supabase.from('repasse_config').select('*').eq('clinic_id', clinicId),
  ...
  const handleToggleRegra = async (regraId, ativo) => {
    try {
      ...
      const [resRevenue, resRepasse] = await Promise.all([
        supabase.from('revenue_rules').update({ active: !ativo }).eq('id', regraId),
        supabase.from('repasse_config').update({ ativo: !ativo }).eq('id', regraId),
      ]);
  ...
  const handleDeletarRegra = async (regraId) => {
    ...
    try {
      ...
      const [resRevenue, resRepasse] = await Promise.all([
        supabase.from('revenue_rules').delete().eq('id', regraId),
        supabase.from('repasse_config').delete().eq('id', regraId),
      ]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <p className="text-sm text-blue-800">
          Abaixo estão todas as regras de repasse cadastradas. Você pode editar ou remover cada
          regra. Para criar novas regras, clique em <strong>Nova Regra</strong> abaixo.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Gerenciamento de regras de repasse para profissionais</CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <tbody>
                  {regras.map((regra) => (
                    <tr key={regra.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4">
                        <span
                          className={`px-2 py-1 text-xs rounded cursor-pointer transition-all hover:opacity-75 ${
                            regra.active
