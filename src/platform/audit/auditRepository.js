...
export class PostgresAuditRepository {
  ...
  async readLatestByTenant(tenantId, limit = 100) {
    try {
      const { data, error } = await this.supabase.from('platform.audit_events').select('*').eq('tenant_id', tenantId).order('created_at', { ascending: false }).limit(limit)
