...
export async function createHealthInsurance(clinicId, data) {
  const {
    ...
    max_file_size_mb,
    max_guides_per_submission,
    requires_manual_confirmation,
    support_email,
    support_phone,
    support_hours,
    enable_rps_generation,
  ...
  const insertData = {
    ...
    tiss_enabled: tiss_enabled === true,
    submission_method: submission_method?.trim() || 'HTTP',
    tiss_endpoint: tiss_endpoint?.trim() || null,
    tiss_username: tiss_username?.trim() || null,
    tiss_password: tiss_password?.trim() || null,
    tiss_response_email: tiss_response_email?.trim() || null,
    // Portal XML Integration fields
    portal_username: portal_username?.trim() || null,
    portal_password: portal_password?.trim() || null,
    portal_webhook_url: portal_webhook_url?.trim() || null,
    portal_api_key: portal_api_key?.trim() || null,
    certificate_path: certificate_path?.trim() || null,
    certificate_password: certificate_password?.trim() || null,
    use_certificate: use_certificate === true,
    submission_format: submission_format?.trim() || 'xml',
    response_format: response_format?.trim() || 'xml',
    use_compression: use_compression === true,
    max_daily_submissions: parseInt(max_daily_submissions) || 100,
    max_file_size_mb: parseInt(max_file_size_mb) || 50,
    max_guides_per_submission: parseInt(max_guides_per_submission) || 500,
    requires_manual_confirmation: requires_manual_confirmation === true,
    support_email: support_email?.trim() || null,
    support_phone: support_phone?.trim() || null,
    support_hours: support_hours?.trim() || '08:00-18:00',
    enable_rps_generation: enable_rps_generation === true,
    portal_connection_status: portal_connection_status?.trim() || 'untested',
    active: true,
  };
