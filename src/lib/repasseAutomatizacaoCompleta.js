...
export async function pipelineProcessamentoManual(clinicId, dataInicio, dataFim) {
  try {
    ...
    const respProcessarTransferencias = confirm(
      `Deseja processar ${repassos.length} transferências agora?`,
    );
...
async function registrarExecucaoPipeline(dados) {
  try {
    const { error } = await supabase.from('repasse_pipeline_log').insert([
      {
...
export function setupAutomatizacaoEmTempoReal(clinicId) {
  ...
  const intervaloDiario = setInterval(async () => {
    const hoje = new Date();
    if (hoje.getDate() === 1 && hoje.getHours() === 1) {
      // Executa 01:00 do 1º dia
      try {
