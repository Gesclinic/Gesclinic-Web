...
const isStackDepthErr = (err) =>
  err?.code === "54001" ||
  err?.message?.toLowerCase?.().includes("stack depth");
...
export async function listClinicMembers(
  ...
  if (!enrichWithEmail || rows.length === 0) return rows;
  ...
  return rows.map((r, i) => {
    const res = results[i];
    if (res.status === "fulfilled") return { ...r, email: res.value?.data ?? null };
...
export async function upsertMemberByEmail({ email, clinicId, role }) {
  ...
  if (!safeEmail) throw new Error("email é obrigatório");
  ...
  if (error) throw new Error(error.message ?? "Falha ao vincular usuário");
  return data;
}
...
export async function updateMemberRole({ userId, clinicId, role }) {
  ...
  if (error) throw new Error(error.message ?? "Falha ao atualizar papel");
  return data;
}
...
export async function removeMember({ userId, clinicId }) {
  ...
  if (error) throw new Error(error.message ?? "Falha ao remover vínculo");
  return true;
}