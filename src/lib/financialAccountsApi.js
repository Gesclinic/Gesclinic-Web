...
export const financialAccountsApi = {
  ...
  async calculateDREForPeriod(clinicId, startDate, endDate) {
    ...
    transactions.forEach(t => {
      ...
      if (t.type === 'revenue') {
        ...
        } else if (t.category === 'rent' || t.category === 'utilities' || t.category === 'maintenance') {
    ...
    dre.totalDespesas = dre.despesasAdmin + dre.despesasClinica + dre.despesasComercial + dre.despesasFinanceira;
    ...
    dre.lucroOperacionalPct = dre.receitaLiquida > 0 ? (dre.lucroOperacional / dre.receitaLiquida) * 100 : 0;

    dre.lucroLiquido = dre.lucroOperacional;
    dre.lucroLiquidoPct = dre.receitaLiquida > 0 ? (dre.lucroLiquido / dre.receitaLiquida) * 100 : 0;
  ...
  async getDRELines(clinicId, startDate, endDate) {
    ...
    return [
      ...
      { label: 'Receita Líquida', value: dre.receitaLiquida, type: 'receita', level: 1, bold: true },
      { label: 'Custos Diretos', value: -dre.custos, type: 'custo', level: 2 },
      { label: 'Margem Bruta', value: dre.margemBruta, type: 'receita', level: 1, bold: true, pct: dre.margemBrutaPct },
      ...
      { label: 'EBITDA', value: dre.ebitda, type: 'receita', level: 1, bold: true, pct: dre.ebitdaPct },
      { label: 'Depreciação e Amortização', value: -dre.depreciacao, type: 'ajuste', level: 2 },
      { label: 'Lucro Operacional', value: dre.lucroOperacional, type: 'receita', level: 1, bold: true, pct: dre.lucroOperacionalPct },
      { label: 'Lucro Líquido', value: dre.lucroLiquido, type: 'receita', level: 1, bold: true, pct: dre.lucroLiquidoPct }
    ];
  },
