...
export function validateTISSDataCompleteness(guideData) {
  ...
  if (guideData.appointment?.requires_authorization && !guideData.appointment?.authorization_number) {
...
export function generateTISSXML(guideData) {
  ...
  const guideNumber = appointment?.guide_number || `${formatDate(new Date())}-${Math.random().toString(36).substr(2, 9)}`;
...
export async function submitTISSGuide(guideId, clinicId) {
  try {
    ...
    // 7. Log de auditoria
    await logTISSAudit(clinicId, guideId, "SUBMITTED", `XML enviado. ID: ${submissionId}`, xmlContent);
