...
export const statusToCanonical = (status) => {
  ...
  if (['agendado', 'scheduled'].includes(s)) {
    return 'scheduled';
  }
  if (['confirmado', 'confirmed'].includes(s)) {
    return 'confirmed';
  }
  if (['presente', 'present'].includes(s)) {
    return 'present';
  }
  if (['em-consultorio', 'in-office'].includes(s)) {
    return 'in_office';
  }
  if (['atendido', 'attended', 'finished'].includes(s)) {
    return 'attended';
  }
  if (['cancelado', 'cancelled'].includes(s)) {
    return 'cancelled';
  }
  if (['faltou', 'no-show'].includes(s)) {
    return 'no_show';
  }
