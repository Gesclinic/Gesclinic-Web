 ...
 */
export async function getAgendaIndicatorsByDateRange(
  clinicId,
  startDate,
  endDate
) {
...
export function generateAlerts(indicators) {
  ...
  if (
    indicators.meta_dia &&
    indicators.receita_estimada < indicators.meta_dia * 0.7
  ) {
    alerts.push({
      type: "warning",
      severity: "medium",
...
export function getHealthStatus(indicators) {
  ...
  if (ocupacao < 40 && receita < 50) return "critical";
  if (taxa_faltas > 20) return "critical";

  // Aviso: algo está fora
  if (ocupacao < 50) return "warning";
  if (receita < 70) return "warning";
  if (taxa_faltas > 15) return "warning";
