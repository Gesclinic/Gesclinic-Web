...
export const listAppointmentsRange = async (
  clinicId,
  from,
  to,
  freeText,
  ...
  try {
    const { data, error } = await supabase.rpc('list_appointments_enhanced', {
      ...
      p_professional_id: (professionalId && professionalId !== 'todos' && professionalId !== '') 
        ? professionalId 
        : null,
      p_status: null
    });
    ...
    if (from instanceof Date) query = query.gte("start_time", from.toISOString());
    if (to instanceof Date) query = query.lt("end_time", to.toISOString());
    ...
    const enrichedData = await Promise.all((data || []).map(async (appointment) => {
      ...
      if (appointment.patient_id) {
        try {
          ...
          if (patient) {
            let nomeLimpo = patient.full_name
              ? String(patient.full_name).replace(
