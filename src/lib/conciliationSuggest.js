...
export function suggestForStatement({ description, type }) {
  ...
  if (desc.includes('TARIFA')) {
    planoContas = 'Tarifas Bancárias';
  } else if (desc.includes('PIX')) {
    planoContas = type === 'credit' ? 'Receita Operacional' : 'Transferências';
  } else if (desc.includes('TED')) {
    planoContas = type === 'debit' ? 'Contas a Pagar' : 'Transferências';
  } else if (desc.includes('SALDO INICIAL')) {
    planoContas = 'Ajustes/Saldo Inicial';
  }
  // Tipo de lançamento
  let tipoLancamento = '';
  if (type === 'credit') {
  ...
  // Categoria extra
  let categoria = '';
  if (desc.includes('UNIMED')) {
    categoria = 'Convênios';
  }
  // Sugerir ignorar
  let sugerirIgnorar = false;
  if (desc.includes('SALDO INICIAL') || desc.includes('TRANSFERÊNCIA INTERNA')) {
    sugerirIgnorar = true;
  }
  return {
    planoContas,
