  ...
  try {
    ...
    if (error) {
      ...
      if (error.message?.includes('start_time')) {
        throw new Error(
          "❌ ERRO CRÍTICO: Coluna 'start_time' não existe. Use 'startTime' no payload.",
        );
      }
    ...
    logAppointmentAudit({
      ...
      console.warn('[appointmentsApi.createAppointment] Aviso ao logar auditoria:', err),
    );
  ...
  try {
    ...
    if (updates.updated_at && updates.updated_at !== currentUpdatedAt) {
      ...
      const error = new Error(
        'Este agendamento foi atualizado por outro usuário. Recarregue e tente novamente.',
      );
    ...
    if (selectError) {
      console.error(
        '[appointmentsApi.updateAppointment] ❌ Erro ao buscar resultado:',
        selectError,
      );
    ...
    if (updates.status && updates.status !== oldStatus) {
      console.log(
        `[appointmentsApi.updateAppointment] 📝 Status mudou: ${oldStatus} → ${updates.status}`,
      );
      ...
      if (
        ...
        console.log(
          '[appointmentsApi.updateAppointment] ⚡ AUTO-TRIGGER: Processamento financeiro iniciado',
        );
        try {
          setTimeout(async () => {
            await finalizeAppointmentWithFinancials(id);
            console.log(
              '[appointmentsApi.updateAppointment] ✅ AUTO-TRIGGER: Processamento financeiro concluído',
            );
          }, 100);
        } catch (finErr) {
          console.warn(
            '[appointmentsApi.updateAppointment] ⚠️ AUTO-TRIGGER: Erro no processamento financeiro:',
            finErr.message,
          );
        }
      }
