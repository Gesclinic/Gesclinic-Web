...
export async function detalheRepasse(repasseId) {
  const { data, error } = await supabase.from('repasse_medico').select('*').eq('id', repasseId).single();
  if (error) throw error;
  return data;
}
...
export async function ajustarRepasse({ repasseId, valorAjuste, motivo, usuarioId }) {
  const { data, error } = await supabase.from('repasse_ajuste').insert([
    { repasse_id: repasseId, valor_ajuste: valorAjuste, motivo, usuario_id: usuarioId }
  ]);
  if (error) throw error;
  return data;
}
...
export async function liberarRepasseParaPagamento(repasseId) {
  ...
  const { data, error } = await supabase.rpc('liberar_repasse_pagamento', { p_repasse_id: repasseId });
  if (error) throw error;
