...
export async function listPacientes(searchTerm = "", clinicId = null) {
  ...
  if (clinicId) {
    ...
    console.log("🔍 ClinicId é UUID válido?", /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(clinicId));
...
export async function deletePacienteMedia(pacienteId, fileId, fileName) {
  ...
  if (storageError && storageError.statusCode !== '404') {
     console.warn("Storage deletion might have failed, but DB record is removed:", storageError.message);
  }
}