...
export async function listResources(clinicId, options = {}) {
  ...
  let query = supabase
    .from('resources')
    .select(
      'id, code, name, description, type, category, is_consumable, requires_maintenance, active',
    )
  ...
  if (error) {
    throw new Error(`Falha ao listar recursos: ${error.message}`);
  }
  return data ?? [];
}
...
export async function getResource(resourceId, clinicId) {
  ...
  if (error) {
    throw new Error(`Falha ao obter recurso: ${error.message}`);
  }
  return data;
}
...
export async function updateResource(resourceId, clinicId, updates) {
  ...
  if (error) {
    throw new Error(`Falha ao atualizar recurso: ${error.message}`);
  }
  return data;
}
...
export async function deactivateResource(resourceId, clinicId) {
  ...
  if (error) {
    throw new Error(`Falha ao desativar recurso: ${error.message}`);
  }
...
export async function listRoomResources(roomId, clinicId) {
  ...
  if (error) {
    throw new Error(`Falha ao listar recursos da sala: ${error.message}`);
  }
  return data ?? [];
}
 ...
 * @returns {Promise<Object>}
 */
export async function allocateResourceToRoom(roomId, resourceId, clinicId, options = {}) {
...
export async function deallocateResourceFromRoom(roomId, resourceId, clinicId) {
  ...
  if (error) {
    throw new Error(`Falha ao remover alocação: ${error.message}`);
  }
