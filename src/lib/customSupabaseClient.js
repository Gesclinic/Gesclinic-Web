...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  ...
  console.error("VITE_SUPABASE_ANON_KEY:", SUPABASE_ANON_KEY ? "✓ Configurado" : "✗ Não configurado");
...
if (!supabaseClient) {
  ...
  supabaseClient.from("clinics").select("count", { count: "exact" }).limit(1)
    .then(() => {
...
console.log("🔍 Tokens no localStorage:", {
  "gesclinic-auth-token": localStorage?.getItem("gesclinic-auth-token") ? "✓ Existe" : "❌ Não existe",
  "sb-gvdkdjyupktlflwurike-auth-token": localStorage?.getItem("sb-gvdkdjyupktlflwurike-auth-token") ? "✓ Existe" : "❌ Não existe"
});
