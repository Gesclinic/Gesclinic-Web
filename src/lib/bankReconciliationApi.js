...
export const bankReconciliationApi = {
        ...
        .insert([
          {
            ...
            status: Math.abs(parseFloat(bankBalance) - parseFloat(systemBalance)) < 0.01 ? 'reconciled' : 'needs_adjustment'
          }
        ])
        .select()
        .single();
  ...
  async listReconciliations(clinicId, filters = {}) {
    try {
      ...
      if (filters.from_date) query = query.gte('bank_statement_date', filters.from_date);
      if (filters.to_date) query = query.lte('bank_statement_date', filters.to_date);
