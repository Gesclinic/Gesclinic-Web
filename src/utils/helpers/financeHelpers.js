...
export function parseSearchGeneral(txt) {
  if (!txt) {
    return { clean: '', statusText: null, amountEq: null, dateIso: null };
  }
  let s = String(txt);
  let statusText = null;
  let amountEq = null;
  ...
  if (statusText) {
    ...
    if (['pago', 'paga', 'paid', 'quitado'].includes(v)) {
      statusText = 'paid';
    } else if (['aberto', 'em aberto', 'open', 'pendente'].includes(v)) {
      statusText = 'open';
    } else if (['parcial', 'partial'].includes(v)) {
      statusText = 'partial';
    } else if (['cancelado', 'cancelada', 'canceled'].includes(v)) {
      statusText = 'canceled';
    } else if (['agendada', 'agendado', 'scheduled', 'programada'].includes(v)) {
      statusText = 'scheduled';
    } else if (['vencida', 'overdue'].includes(v)) {
      statusText = 'overdue';
    }
  }
  const mNum = s.match(
    /(?:R\$\s*)?(?<![0-9.,])([0-9]{1,3}(?:[.,][0-9]{3})*(?:[.,][0-9]{1,2})?)(?![0-9.,])/i,
  );
  if (mNum) {
...
export function parcelLabel(it) {
  const n =
    typeof it.installments !== 'undefined' && it.installments !== null
      ? Number(it.installments)
      : null;
