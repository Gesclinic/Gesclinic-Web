...
export default function LaudoTemplatesManager() {
  ...
  const resolvedClinic = useMemo(
    () => ({
      ...clinicDetails,
      address_line: [clinicDetails?.address, clinicDetails?.city, clinicDetails?.state]
        .filter(Boolean)
        .join(' - '),
      contact_line: [clinicDetails?.phone, clinicDetails?.email, clinicDetails?.cnpj]
        .filter(Boolean)
        .join(' • '),
    }),
    [clinicDetails],
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        toast({
          title: 'Erro',
          description: 'Falha ao carregar os modelos de laudo.',
          variant: 'destructive',
        });
      } finally {
        setLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      await (form.id
        ? updatePatientLaudoTemplate(form.id, payload)
        : createPatientLaudoTemplate(payload));
      const refreshed = await listPatientLaudoTemplates(clinic?.id || clinicId || null);
      setTemplates(refreshed || []);
      toast({
        title: form.id ? 'Modelo atualizado' : 'Modelo criado',
        description: 'O modelo de laudo foi salvo com sucesso.',
      });
      if (!form.id) {
        resetForm();
      }
  ...
  const handleUploadLogo = async (event) => {
    ...
    try {
      ...
      toast({
        ...
        description: 'O logo foi vinculado ao timbrado deste modelo.',
      });
    } catch (error) {
      console.error('Erro ao carregar logo:', error);
      toast({
        title: 'Erro',
        description: error.message || 'Falha ao carregar o logo.',
        variant: 'destructive',
      });
    } finally {
                ...
                >
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Badge variant="outline" className="border-slate-300 bg-white text-slate-700">
                      {LAUDO_TYPE_OPTIONS.find((option) => option.value === template.laudo_type)
                        ?.label || template.laudo_type}
                    </Badge>
                    {template.is_default ? (
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Defina conteúdo padrão, timbrado e logo institucional.
              </p>
            </div>
            {form.id ? (
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_360px]">
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  <Label>Tipo</Label>
                  <Select
                    value={form.laudo_type}
                    onValueChange={(value) => setForm((prev) => ({ ...prev, laudo_type: value }))}
                  >
                    <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setForm((prev) => ({ ...prev, title_template: e.target.value }))}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setForm((prev) => ({ ...prev, content_template: e.target.value }))
                  }
                />
              </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 space-y-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      Configure os dados institucionais exibidos no laudo.
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      disabled={uploadingLogo || !(clinic?.id || clinicId)}
                    >
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input
                      value={form.letterhead.clinic_display_name}
                      onChange={(e) =>
                        setForm((prev) => ({
                          ...prev,
                          letterhead: { ...prev.letterhead, clinic_display_name: e.target.value },
                        }))
                      }
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      value={form.letterhead.clinic_contact_line}
                      onChange={(e) =>
                        setForm((prev) => ({
                          ...prev,
                          letterhead: { ...prev.letterhead, clinic_contact_line: e.target.value },
                        }))
                      }
                    />
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    rows={3}
                    value={form.letterhead.clinic_address_line}
                    onChange={(e) =>
                      setForm((prev) => ({
                        ...prev,
                        letterhead: { ...prev.letterhead, clinic_address_line: e.target.value },
                      }))
                    }
                  />
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input
                      value={form.letterhead.professional_display_name}
                      onChange={(e) =>
                        setForm((prev) => ({
                          ...prev,
                          letterhead: {
                            ...prev.letterhead,
                            professional_display_name: e.target.value,
                          },
                        }))
                      }
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      value={form.letterhead.professional_title_line}
                      onChange={(e) =>
                        setForm((prev) => ({
                          ...prev,
                          letterhead: {
                            ...prev.letterhead,
                            professional_title_line: e.target.value,
                          },
                        }))
                      }
                    />
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    rows={3}
                    value={form.letterhead.professional_address_line}
                    onChange={(e) =>
                      setForm((prev) => ({
                        ...prev,
                        letterhead: {
                          ...prev.letterhead,
                          professional_address_line: e.target.value,
                        },
                      }))
                    }
                  />
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input
                      value={form.letterhead.professional_contact_line}
                      onChange={(e) =>
                        setForm((prev) => ({
                          ...prev,
                          letterhead: {
                            ...prev.letterhead,
                            professional_contact_line: e.target.value,
                          },
                        }))
                      }
                    />
                  ...
                  <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        Marque para destacar este modelo como preferencial.
                      </p>
                    </div>
                    <Switch
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  O logo e as linhas abaixo serão exibidos no laudo, no preview e na impressão.
                </p>
              </div>
              <LaudoLetterhead letterhead={form.letterhead} />
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <p className="text-base font-semibold text-slate-900">
                  {form.title_template || 'Título do laudo'}
                </p>
                <div className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-700">
                  {form.content_template || 'Conteúdo do modelo.'}
                </div>
              </div>
            </div>
          </div>
