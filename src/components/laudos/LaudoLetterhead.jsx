...
export default function LaudoLetterhead({ letterhead = {}, compact = false, section = 'all' }) {
  ...
  if ((!showClinic || !clinicLines.length) && (!showProfessional || !professionalLines.length) && (!showClinic || !logoUrl)) {
    return null;
  }
      ...
      <div className="flex items-start gap-4">
        {showClinic && logoUrl ? (
          <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
            <img src={logoUrl} alt="Logo da clínica" className="max-h-full max-w-full object-contain" />
          </div>
        ) : null}
          ...
          {showClinic && clinicLines.length ? (
            <div>
              ...
              {clinicLines.slice(1).map((line) => (
                <p key={`clinic-${line}`} className="text-sm leading-5 text-slate-600">{line}</p>
              ))}
            </div>
          ) : null}

          {showProfessional && professionalLines.length ? (
            <div className={`${showClinic && clinicLines.length ? 'mt-3 border-t border-slate-200 pt-3' : ''}`}>
              <p className="text-sm font-semibold text-slate-900">{professionalLines[0]}</p>
              {professionalLines.slice(1).map((line) => (
                <p key={`professional-${line}`} className="text-sm leading-5 text-slate-600">{line}</p>
              ))}
            </div>
          ) : null}
        </div>
