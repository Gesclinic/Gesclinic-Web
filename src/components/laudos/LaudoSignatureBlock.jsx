    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm font-semibold text-slate-900">
            {normalized.signed_by_name || 'Responsável não informado'}
          </p>
          {normalized.certificate_id ? (
            <p className="mt-1 text-xs text-slate-600">Certificado: {normalized.certificate_id}</p>
          ) : null}
          {normalized.signature_hash ? (
            <p className="mt-1 break-all text-[11px] text-slate-500">
              Hash SHA-256: {normalized.signature_hash}
            </p>
          ) : null}
        </div>
      ...
      {normalized.visual_signature_data_url ? (
        <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3">
          <img
            src={normalized.visual_signature_data_url}
