...
function ProfessionalConveniosTab({ profesionalId, clinicId, submitting }) {
  ...
  const handleEditPayer = (assignment) => {
    ...
    const selectedPrices = servicePrices.filter(
      sp => sp.payer_id === assignment.payer_id
    );
  ...
  const handleAddService = async (serviceId) => {
    try {
      ...
      if (insertError) {
        ...
        setError(
          insertError.message ||
          "Erro ao adicionar serviço. Verifique se o convênio é válido."
        );
        return;
      }
  ...
  const handleRemoveService = async (servicePriceId) => {
    try {
      ...
      setSelectedServices(
        selectedServices.filter(sp => sp.id !== servicePriceId)
      );
    } catch (err) {
  ...
  const handleUpdateServicePrice = async (servicePriceId, newPrice) => {
    try {
      ...
      setSelectedServices(
        selectedServices.map(sp =>
          sp.id === servicePriceId ? { ...sp, price: newPrice } : sp
        )
      );
    } catch (err) {
  ...
  const handleUpdateServiceSchedule = (servicePriceId, dayOfWeek, startTime, endTime, duration = 30) => {
  ...
  const handleDeleteSchedulePeriod = (servicePriceId, dayOfWeek, periodId) => {
    setServiceSchedules(prev => {
      const periods = (prev[servicePriceId]?.[dayOfWeek]?.periods || []).filter(p => p.id !== periodId);
      return {
        ...prev,
        [servicePriceId]: {
          ...(prev[servicePriceId] || {}),
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const detailsMsg = schedulesCount > 0 
        ...
        {!addingPayer ? (
          <div className="bg-white border border-gray-200 rounded-lg p-5">
            <div className="border-b pb-3 mb-4">
              ...
              <p className="text-sm text-gray-600 mt-1">Vincular um novo convênio ao profissional</p>
            </div>
            <Button
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-3">
            <div className="border-b pb-3 mb-4">
              ...
              <p className="text-sm text-gray-600 mt-1">Selecione um convênio para vincular ao profissional</p>
            </div>
          <div>
            ...
            <div className="p-6 border-b sticky top-0 bg-white z-10">
              <div className="flex justify-between items-start mb-4">
                <h3 className="text-lg font-semibold">
                  Configurar Regras - {editingPayerId ? payers.find(p => p.id === editingPayerId)?.name : "Convênio"}
                </h3>
                <button
              ...
              {activeModalTab === "servicos" && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-gray-600">Configure preço e horários disponíveis para cada serviço neste convênio</p>
                    <select
                      onChange={(e) => {
                        if (e.target.value) {
                      ...
                      <div className="space-y-2 max-h-96 overflow-y-auto">
                        {selectedServices.map((servicePrice) => {
                          ...
                          return (
                            ...
                            >
                              <div className="p-3 flex items-center justify-between cursor-pointer hover:bg-gray-100 transition"
                                onClick={() => setExpandedServiceId(isExpanded ? null : servicePrice.id)}>
                                <div className="flex-1">
                                  ...
                                  <div className="flex items-center gap-2 mt-1">
                                    ...
                                    {basePrices[servicePrice.service_id] !== undefined && basePrices[servicePrice.service_id] !== servicePrice.price && (
                                      <div className="ml-2 inline-flex items-center gap-1 px-2 py-1 bg-orange-100 text-orange-700 rounded text-xs font-medium">
                                        <span title={`Preço base: R$ ${parseFloat(basePrices[servicePrice.service_id] || 0).toFixed(2)}`}>
                                          ✏️ Negociação
                                        </span>
                                      </div>
                                    )}
                                    {basePrices[servicePrice.service_id] === servicePrice.price && basePrices[servicePrice.service_id] !== undefined && (
                                      <div className="ml-2 inline-flex items-center gap-1 px-2 py-1 bg-green-100 text-green-700 rounded text-xs font-medium">
                                        <span title="Preço padrão do convênio">
                                          ✅ Padrão
                                        </span>
                                      </div>
                                    )}
                                  ...
                                  {basePrices[servicePrice.service_id] !== undefined && basePrices[servicePrice.service_id] !== servicePrice.price && (
                                    <p className="text-xs text-orange-600 mt-1">
                                      Preço base: <strong>R$ {parseFloat(basePrices[servicePrice.service_id] || 0).toFixed(2)}</strong> 
                                      <span className="ml-2 text-gray-600">
                                        Diferença: <strong>{((servicePrice.price - basePrices[servicePrice.service_id]) > 0 ? '+' : '')}R$ {(servicePrice.price - basePrices[servicePrice.service_id]).toFixed(2)}</strong>
                                      </span>
                                    </p>
                                  )}
                                </div>
                              ...
                              {isExpanded && (
                                <div className="border-t bg-white p-4 space-y-3">
                                  <p className="text-xs font-medium text-gray-700">Horários de Atendimento deste Serviço</p>
                                  <div className="space-y-3">
                                    {daysOfWeek.map(day => {
                                      const periods = serviceSchedules[servicePrice.id]?.[day.id]?.periods || [];
                                      const isDeleted = serviceSchedules[servicePrice.id]?.[day.id] === null;
                                      
                                      return (
                                        <div key={day.id} className={`p-3 rounded border transition ${isDeleted ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-200'}`}>
                                          <div className="flex items-center justify-between mb-2">
                                            ...
                                            {isDeleted && (
                                              <button
                                                onClick={() => handleAddSchedulePeriod(servicePrice.id, day.id)}
                                            ...
                                            <>
                                              {periods.length === 0 ? (
                                                <div className="flex items-center justify-between">
                                                  <span className="text-xs text-gray-600 italic">Sem períodos configurados</span>
                                                  <button
                                                    onClick={() => handleAddSchedulePeriod(servicePrice.id, day.id)}
                                                ...
                                                <div className="space-y-2">
                                                  {periods.map((period, idx) => {
                                                    const appointmentCount = calculateAppointmentCount(
                                                      period.start_time,
                                                      period.end_time,
                                                      period.duration
                                                    );
                                                    ...
                                                    return (
                                                      <div key={period.id} className="p-3 bg-white border border-gray-200 rounded space-y-3">
                                                        <div className="flex justify-between items-start">
                                                          <span className="font-semibold text-sm text-gray-900">Período {idx + 1}</span>
                                                          <button
                                                            onClick={() => handleDeleteSchedulePeriod(servicePrice.id, day.id, period.id)}
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                              <div className="flex-1">
                                                                <input
                                                                  ...
                                                                  onChange={(e) => handleUpdateSchedulePeriod(
                                                                    servicePrice.id,
                                                                    day.id,
                                                                    period.id,
                                                                    'start_time',
