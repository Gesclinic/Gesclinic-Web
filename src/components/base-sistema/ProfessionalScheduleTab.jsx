...
export function ProfessionalScheduleTab({ 
  ...
  useEffect(() => {
    if (onFormStateChange) {
      onFormStateChange({
        ...
        hasUnsavedData: !!editingScheduleId || newSchedule.unit_name || newSchedule.room_id || newSchedule.day_of_week !== "1"
      });
    }
  ...
  const getScheduleData = () => ({
    ...
    allowed_health_insurances: newSchedule.allowed_health_insurances.length > 0 ? newSchedule.allowed_health_insurances : null,
  });
  ...
  const handleAddSchedule = async () => {
    if (!newSchedule.unit_name || !newSchedule.room_id || !newSchedule.day_of_week || !newSchedule.start_time || !newSchedule.end_time) {
    ...
    if (!newSchedule.id) {
      ...
      if (isDuplicate) {
        const dayName = DAYS_OF_WEEK.find(d => d.value === newSchedule.day_of_week)?.label || "este dia";
        const msg = (
          `🚫 CONFLITO DE HORÁRIO!\n\n` +
  ...
  const handleDeleteSchedule = async (scheduleId) => {
    ...
    const dayLabel = DAYS_OF_WEEK.find(d => d.value === String(schedule.day_of_week))?.label || "Dia";

    if (!window.confirm(`Remover horário de ${dayLabel} (${schedule.start_time} - ${schedule.end_time})?`)) {
      return;
    }
      ...
      <div style={{flex: 1, overflowY: "auto"}}>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-5">
          <div className="border-b pb-3 mb-4 flex items-center justify-between">
            <h4 className="text-lg font-bold text-gray-900">{editingScheduleId ? "✏️ Editar Horário" : "➕ Novo Horário de Atendimento"}</h4>
            {editingScheduleId && (
              <button
                type="button"
                onClick={resetForm}
        ...
        <div className="flex gap-2 items-end mt-3">
          ...
          <div className="flex-1 min-w-[110px]">
            ...
            <input
              ...
              onChange={(e) => setNewSchedule({ ...newSchedule, duration_minutes: parseInt(e.target.value) || 30 })}
              className="w-full px-3 py-2 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              disabled={submitting}
            />
          </div>
              ...
              <div className="absolute top-full left-0 right-0 mt-1 border rounded-lg bg-white shadow-lg z-50">
                ...
                {healthInsurances.length > 0 && (
                  <div className="border-b px-3 py-2 flex gap-2 bg-gray-50">
                    <button
                      type="button"
                      onClick={() => setNewSchedule({ ...newSchedule, allowed_health_insurances: healthInsurances.map(h => h.id) })}
                    ...
                    <button
                      type="button"
                      onClick={() => setNewSchedule({ ...newSchedule, allowed_health_insurances: [] })}
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setNewSchedule({
                                  ...newSchedule,
                                  allowed_health_insurances: [...newSchedule.allowed_health_insurances, insurance.id]
                                });
                              } else {
                                setNewSchedule({
                                  ...newSchedule,
                                  allowed_health_insurances: newSchedule.allowed_health_insurances.filter(id => id !== insurance.id)
                                });
                              }
                            }}
        ...
        <div className="flex gap-2 items-end mt-3">
          ...
          <div className="flex items-center gap-1 pb-0.5">
            <input
              ...
              disabled={submitting}
            />
            <label htmlFor="active-schedule" className="text-xs font-medium text-gray-700 cursor-pointer whitespace-nowrap">
              Ativo
            </label>
          </div>
          ...
          <tbody>
            {schedules && schedules.length > 0 ? (
              schedules.map((schedule) => {
                const dayLabel = DAYS_OF_WEEK.find(d => d.value === String(schedule.day_of_week))?.label || schedule.day_of_week;
                const roomName = rooms.find(r => r.id === schedule.room_id)?.name || "Não definida";
                return (
                  <tr key={schedule.id} className="border-b hover:bg-gray-50">
                    <td className="p-2 font-medium text-purple-700">{schedule.unit_name || "-"}</td>
