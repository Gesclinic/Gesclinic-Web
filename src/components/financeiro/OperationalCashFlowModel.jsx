...
export function buildOperationalModel(consolidation, accounts = [], returnTo = '', accountingMode = 'realized', options = {}) {
  ...
  function addMovementToTree(movement) {
    ...
    const periodPath = periodicity === 'daily'
      ? []
...
function ControlGroup({ label, value, options, onChange, className = '' }) {
  return (
    <div className={`min-w-0 rounded-lg border border-slate-200 bg-white p-1.5 shadow-sm dark:border-slate-700 dark:bg-slate-900 ${className}`}>
    ...
    <Card className="mb-6 overflow-hidden border-slate-200 bg-white p-0 shadow-sm dark:border-slate-700 dark:bg-slate-950">
      ...
      <div className="mb-3 rounded-xl border border-slate-200 bg-slate-100/70 p-1.5 shadow-inner dark:border-slate-700 dark:bg-slate-900/70">
        <div className="flex flex-wrap gap-2">
          <ControlGroup className="flex-[1_1_190px]" label="Visualizacao" value={tablePeriodicity} options={PERIODICITY_OPTIONS} onChange={setPeriodicity} />
          <ControlGroup className="flex-[1.25_1_245px]" label="Cenario" value={tableScenario} options={SCENARIO_OPTIONS} onChange={setScenario} />
          <ControlGroup className="flex-[1.15_1_225px]" label="Exibir" value={tableDisplay} options={DISPLAY_OPTIONS} onChange={setDisplay} />
          <ControlGroup
            className="flex-[0.9_1_170px]"
            label="Nivel"
            value={viewDepth}
            options={[
            ...
            onChange={setDepth}
          />
          <div className="min-w-0 flex-[0.9_1_172px] rounded-lg border border-slate-200 bg-white p-1 shadow-sm dark:border-slate-700 dark:bg-slate-900">
          ...
          <div className="flex min-w-[112px] flex-[0_0_118px] flex-col justify-end gap-1.5 rounded-lg border border-slate-200 bg-white p-1.5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <button
              type="button"
              onClick={expandAll}
              className="inline-flex h-7 w-full items-center justify-center gap-1 rounded-md border border-slate-300 bg-white px-2 text-[11px] font-semibold text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-white"
            >
