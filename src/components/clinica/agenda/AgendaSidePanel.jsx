    ...
    const loadWaitingQueue = async () => {
      try {
        ...
        const { data, error } = await supabase
          ...
          .lt(
            'start_time',
            format(new Date(selectedDate.getTime() + 24 * 60 * 60 * 1000), 'yyyy-MM-dd'),
          )
          .order('start_time');
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      combined.forEach((item) => {
        if (
          !uniquePatients[item.patient_id] ||
          new Date(item.start_time) < new Date(uniquePatients[item.patient_id].start_time)
        ) {
          ...
          <Card>
            ...
            <CardContent className="pt-0 space-y-2">
              {waitingQueue.length === 0 ? (
                <p className="text-sm text-gray-500 text-center py-4">Nenhum paciente na fila</p>
              ) : (
                  ...
                  {searchResults.map((result) => (
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center justify-between">
                          <p className="text-sm font-medium truncate">{result.patient_name}</p>
                        ...
                        <p className="text-xs text-gray-500 truncate">
                          <b>Serviço:</b> {result.service_name || '-'}
                          <br />
                          <b>Convênio:</b> {result.payer_name || 'Particular'}
                          <br />
