...
export default function AgendaTable({ appointments = [], onClickSlot, loading, isProfessional, onStatusChange, selectedDate }) {
      ...
      <table className="w-full text-sm">
        <thead className="bg-gray-50 text-gray-600 font-semibold border-b">
          <tr>
            ...
            {!isProfessional && <th className="p-3 text-left hidden lg:table-cell">Profissional</th>}
        ...
        <tbody>
          {appointmentsForDay.map((apt) => (
            ...
            >
                <td className="p-3 font-medium">{apt.start_time ? (() => {
                  try {
              ...
              <td className="p-3 hidden xl:table-cell">R$ {apt.price?.toFixed(2).replace('.', ',') || "0,00"}</td>
              <td className="p-3">
                <div className="flex items-center gap-2">
                  <Badge variant="outline" className={`${statusColors[apt.status] || "bg-gray-100 text-gray-800"} relative`}>
                  ...
                  {editingStatusId === apt.id && selectedStatus && (
                    <button
                      ...
                      onClick={e => {
                        ...
                        if (onStatusChange) onStatusChange(apt.id, selectedStatus);
                      }}
                    >Salvar</button>
                  )}
                </div>
              </td>
              {!isProfessional && <td className="p-3 hidden lg:table-cell">{apt.professional_name || "-"}</td>}
