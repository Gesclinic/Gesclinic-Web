...
import { Clock, User, Phone, UserCheck, AlertCircle, RefreshCw } from 'lucide-react';
import { APPOINTMENT_STATUS, getStatusColor, getStatusLabel } from '@/constants/appointmentStatus';
...
export default function WaitingQueue({
  ...
  const groupedPatients = {
    [APPOINTMENT_STATUS.NA_RECEPCAO]: waitingPatients.filter(
      (p) => p.status === APPOINTMENT_STATUS.NA_RECEPCAO,
    ),
    [APPOINTMENT_STATUS.NO_GUICHE]: waitingPatients.filter(
      (p) => p.status === APPOINTMENT_STATUS.NO_GUICHE,
    ),
    [APPOINTMENT_STATUS.PRESENTE]: waitingPatients.filter(
      (p) => p.status === APPOINTMENT_STATUS.PRESENTE,
    ),
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="secondary">{waitingPatients.length} paciente(s)</Badge>
        </div>

        <Button variant="outline" size="sm" onClick={fetchWaitingPatients} disabled={loading}>
...
function PatientCard({ patient, onCall, showCallButton }) {
  ...
  return (
    <div
      className={`p-3 rounded-lg border-l-4 ${getPriorityColor(patient.priority, patient.arrival_time)}`}
    >
