...
export default function StockRequestDialog({ open, onOpenChange, onSubmit, clinicId }) {
  ...
  useEffect(() => {
    if (!open) {
      setForm({ date: "", locationId: "", location_name: "", requested_by: "", purpose: "", notes: "", products: [] });
  ...
  const addLine = () => {
    if (!temp.itemId || !(parseFloat(temp.qty) > 0)) return;
    const line = { itemId: temp.itemId, product: temp.product, qty: temp.qty, item_note: temp.item_note };
    if (editingIndex !== null) {
      const copy = [...form.products];
      copy[editingIndex] = line;
      setForm({ ...form, products: copy });
      setEditing(null);
    } else setForm({ ...form, products: [...form.products, line] });
    setTemp({ itemId: "", product: "", qty: "", item_note: "" });
  };

  const removeLine = (idx) => {
    setForm({ ...form, products: form.products.filter((_, i) => i !== idx) });
    if (editingIndex === idx) setEditing(null);
  };

  const submit = (e) => {
    e.preventDefault();
    if (!form.locationId) return alert("Selecione o local");
    if (!form.purpose) return alert("Selecione a finalidade");
    if (!form.products.length) return alert("Adicione pelo menos um item");
    onSubmit(form);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="app-dialog-shell app-dialog-shell--content app-dialog-shell--wide">
        ...
        {/* CONTENT */}
        <form id="request-form" onSubmit={submit} className="flex-1 overflow-y-auto px-6 pt-6 pb-2 space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="md:col-span-2 space-y-2">
                ...
                <LocationSelect
                  ...
                  onChange={(d) => setForm({ ...form, locationId: d.locationId, location_name: d.location })}
                  hideLabel
                />
              </div>
            </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6 space-y-5 mb-5">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Produtos solicitados ({form.products.length} item{form.products.length !== 1 ? 's' : ''})</p>
              </div>
            </div>
            ...
            <div className="space-y-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
              <div className="space-y-2">
                <label className="block text-sm font-medium text-gray-700">Buscar produto por nome ou código</label>
                <ProductSelect
                  clinicId={clinicId}
                  value={temp.product}
                  itemId={temp.itemId}
              ...
              <div className="space-y-2">
                <label className="block text-sm font-medium text-gray-700">Observação do item (opcional)</label>
                <input
                  type="text"
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {form.products.map((p, idx) => (
                      <tr key={idx} className="hover:bg-gray-50 transition">
                        ...
                        <td className="px-4 py-3 text-right font-medium text-gray-900">{parseFloat(p.qty).toFixed(2)}</td>
