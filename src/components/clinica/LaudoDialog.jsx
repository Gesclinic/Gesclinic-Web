...
export default function LaudoDialog({
  ...
  const [signature, setSignature] = useState(() => normalizeLaudoSignature({}, professionalInfo?.name || ''));

  const defaultLetterhead = useMemo(() => normalizeLetterhead({
  ...
  useEffect(() => {
    if (open) {
      if (initialData?.id) {
        ...
        setLaudoType(initialData?.laudo_type ?? initialData?.metadata?.template_type ?? 'personalizado');
        setStatus(initialData?.status ?? 'rascunho');
        setDataExame((initialData?.exam_date ?? initialData?.data_exame ?? '')?.split('T')[0] ?? '');
        setPortalVisible(Boolean(initialData?.portal_visible || initialData?.status === 'publicado'));
        ...
        setTemplateSections(normalizeTemplateSections(initialData?.metadata?.template_sections, getLaudoTypeConfig(initialData?.laudo_type ?? initialData?.metadata?.template_type ?? 'personalizado').sections));
        setSignature(normalizeLaudoSignature(initialData?.metadata?.signature || {}, initialData?.professional_name || professionalInfo?.name || ''));
      ...
      setTemplateSearch('');
      setSaveAsDefault(false);
      setTemplateSections(normalizeTemplateSections(starterTemplate.metadata?.template_sections, getLaudoTypeConfig('avaliacao_clinica').sections));
      setSignature(normalizeLaudoSignature({}, professionalInfo?.name || defaultLetterhead.professional_display_name || ''));
  ...
  const searchableTemplates = useMemo(() => {
    ...
    if (!normalizedSearch) return sortedTemplates;
  ...
  const handleTemplateSelectionChange = useCallback((value) => {
  ...
  const handleApplyTemplate = useCallback(() => {
    ...
    setTemplateSections(normalizeTemplateSections(selectedTemplate?.metadata?.template_sections, builtInTemplate.metadata?.template_sections || getLaudoTypeConfig(laudoType).sections));
  ...
  const handleUseSavedTemplate = useCallback((template) => {
    ...
    setCorpo(resolved.content);
    setLetterhead(resolved.letterhead);
    setTemplateSections(normalizeTemplateSections(template.metadata?.template_sections, getLaudoTypeConfig(template.laudo_type || laudoType).sections));
    setTemplateMetadata((prev) => ({
      ...prev,
      ...(template.metadata || {}),
  ...
  const handleTemplateSectionChange = useCallback((index, value) => {
    setTemplateSections((prev) => prev.map((section, sectionIndex) => sectionIndex === index ? value : section));
  }, []);
  ...
  const handleApplySectionsToContent = useCallback(() => {
    const nextSections = normalizeTemplateSections(templateSections, getLaudoTypeConfig(laudoType).sections);
  ...
  const handleSaveTemplateClick = useCallback(async (mode = 'update') => {
    const shouldCreateNew = mode === 'new' || selectedTemplateId === 'builtin';
    const normalizedSections = normalizeTemplateSections(templateSections, getLaudoTypeConfig(laudoType).sections);
    const payload = {
  ...
  }, [selectedTemplateId, templateName, titulo, laudoType, corpo, letterhead, templateMetadata, onSaveTemplate, saveAsDefault, templateSections]);
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    const normalizedStatus = publishInPortal
      ...
      : status === 'publicado'
        ? 'assinado'
        : status;
    const normalizedSignature = normalizeLaudoSignature(signature, letterhead.professional_display_name || professionalInfo?.name || '');
    ...
    if (shouldPersistSignature) {
      const signaturePayload = buildLaudoSignaturePayload({
        ...
        professionalName: normalizedSignature.signed_by_name || letterhead.professional_display_name || professionalInfo?.name || '',
        letterhead,
      });
      ...
      resolvedSignature = {
        ...
        signed_at: ['assinado', 'publicado'].includes(normalizedStatus)
          ? initialData?.metadata?.signature?.signed_at || initialData?.signed_at || new Date().toISOString()
    ...
    const payload = {
      ...
      portal_published_at: publishInPortal
        ...
        : null,
      signed_at: normalizedStatus === 'assinado' || normalizedStatus === 'publicado'
      ...
      metadata: {
        ...
        ...templateMetadata,
        template_type: laudoType,
        template_sections: normalizeTemplateSections(templateSections, getLaudoTypeConfig(laudoType).sections),
      ...
      <DialogContent className="app-dialog-shell app-dialog-shell--content overflow-hidden p-0">
        <DialogHeader className="border-b border-slate-200 px-6 pb-4 pt-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <DialogTitle className="text-xl font-semibold text-slate-900">{isEdit ? 'Editar Laudo' : 'Novo Laudo'}</DialogTitle>
              <DialogDescription>
                Estruture o laudo clínico com mais conforto. Você pode manter em rascunho e publicar depois no fluxo do paciente.
              </DialogDescription>
            </div>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                  <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:col-span-2">
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Estrutura recomendada</p>
                    <p className="mt-2 text-sm text-slate-700">
                      {selectedTypeConfig.description || 'Registre achados, conclusão e orientações de forma clara para a equipe e para a futura liberação ao paciente.'}
                    </p>
                  </div>
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
                    <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Status atual</p>
                  ...
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        Escreva o laudo completo, com linguagem apropriada para entrega ao paciente e consulta futura.
                      </p>
                    </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500">Edite os tópicos do modelo e aplique a estrutura no conteúdo quando quiser.</p>
                    </div>
                    <div className="flex gap-2">
                      <Button type="button" variant="outline" onClick={handleAddTemplateSection}>
                        Adicionar tópico
                      </Button>
                      <Button type="button" variant="outline" onClick={handleApplySectionsToContent}>
                        Aplicar estrutura
                      </Button>
                    </div>
                  </div>
                  <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
                    {templateSections.map((section, index) => (
                      <div key={`${index}-${section}`} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500">Salve a assinatura visual e o hash do documento para auditoria.</p>
                    </div>
                  </div>
                  <div className="mt-4">
            ...
            <aside className="min-h-0 overflow-y-auto border-t border-slate-200 bg-slate-50 px-6 py-6 xl:border-l xl:border-t-0">
              <div className="space-y-6">
                  ...
                  <div className="mt-4 space-y-4">
                    {highlightedTemplates.length > 0 ? (
                      <div className="space-y-2">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-slate-500">Clique para carregar um modelo salvo do mesmo tipo.</p>
                        </div>
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center justify-between gap-2">
                                  <p className="text-sm font-medium text-slate-900 line-clamp-1">{template.name}</p>
                                  <div className="flex items-center gap-1">
                                    {template.id === mostRecentTemplateId ? (
                                      <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-amber-700">Recente</span>
                                    ) : null}
                                    {template.is_default ? (
                                      <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-emerald-700">Padrão</span>
                                    ) : null}
                                  </div>
                                </div>
                                <p className="mt-1 text-xs text-slate-500 line-clamp-2">{template.title_template || 'Modelo de laudo salvo'}</p>
                              </button>
                            );
                          })}
                        </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className="bg-white"
                      />
                      <Select value={selectedTemplateId} onValueChange={handleTemplateSelectionChange}>
                      ...
                      <div className="flex gap-2">
                        <Button type="button" variant="outline" className="flex-1" onClick={handleApplyTemplate} disabled={selectedTemplateId === 'builtin' && !laudoType}>
                          Aplicar modelo
                        </Button>
                        <Button type="button" variant="ghost" className="px-3 text-slate-600" onClick={() => handleTemplateSelectionChange('builtin')}>
                          Limpar
                        </Button>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-3">
                        <div>
                          <p className="text-sm font-medium text-slate-900">Marcar como modelo padrão</p>
                          <p className="text-xs text-slate-500">Deixa este modelo em destaque para futuras seleções.</p>
                        </div>
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                      ...
                      <p className="mt-1 text-xs text-slate-500">Salve este conteúdo para reutilizar depois e já deixe o modelo selecionado.</p>
                      <div className="mt-3 flex flex-col gap-2">
                        {selectedTemplateId !== 'builtin' ? (
                          <Button type="button" variant="outline" onClick={() => handleSaveTemplateClick('update')} disabled={savingTemplate || !titulo.trim()}>
                            {savingTemplate ? 'Salvando...' : 'Atualizar modelo selecionado'}
                          </Button>
                        ) : null}
                        <Button type="button" onClick={() => handleSaveTemplateClick('new')} disabled={savingTemplate || !titulo.trim()}>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  ...
                  <div className="mt-4 space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input id="clinic_display_name" value={letterhead.clinic_display_name} onChange={(e) => handleLetterheadFieldChange('clinic_display_name', e.target.value)} className="bg-white" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="clinic_address_line">Endereço / timbrado da clínica</Label>
                      <Textarea id="clinic_address_line" value={letterhead.clinic_address_line} onChange={(e) => handleLetterheadFieldChange('clinic_address_line', e.target.value)} rows={3} className="bg-white" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="clinic_contact_line">Contato da clínica</Label>
                      <Input id="clinic_contact_line" value={letterhead.clinic_contact_line} onChange={(e) => handleLetterheadFieldChange('clinic_contact_line', e.target.value)} className="bg-white" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="professional_display_name">Nome do médico</Label>
                      <Input id="professional_display_name" value={letterhead.professional_display_name} onChange={(e) => handleLetterheadFieldChange('professional_display_name', e.target.value)} className="bg-white" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="professional_title_line">CRM / especialidade</Label>
                      <Input id="professional_title_line" value={letterhead.professional_title_line} onChange={(e) => handleLetterheadFieldChange('professional_title_line', e.target.value)} className="bg-white" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="professional_address_line">Endereço do médico</Label>
                      <Textarea id="professional_address_line" value={letterhead.professional_address_line} onChange={(e) => handleLetterheadFieldChange('professional_address_line', e.target.value)} rows={3} className="bg-white" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="professional_contact_line">Contato do médico</Label>
                      <Input id="professional_contact_line" value={letterhead.professional_contact_line} onChange={(e) => handleLetterheadFieldChange('professional_contact_line', e.target.value)} className="bg-white" />
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  ...
                  <div className="mt-4">
                    <LaudoSignaturePad signature={signature} onChange={setSignature} disabled={loading} />
                    {signatureError && (
                ...
                <div className="rounded-2xl border border-sky-200 bg-sky-50 p-4">
                  ...
                  <p className="mt-2 text-sm text-sky-800">
                    Este laudo poderá entrar no fluxo de assinatura, liberação e publicação para acesso do paciente com login e senha.
                  </p>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  <h3 className="text-sm font-semibold text-slate-900">Orientação de preenchimento</h3>
                  <ul className="mt-3 space-y-2 text-sm text-slate-600">
