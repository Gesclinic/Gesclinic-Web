...
export default function ProfessionalDialog({ open, onOpenChange, onSubmit, initialData }) {
      ...
      if (isEdit && initialData?.id) {
        ...
        const mappedPayers = payersData.map((p) => {
          ...
          return {
            payer_id: p.id,
            name: p.name,
            is_restricted: restriction ? restriction.is_restricted : false,
          };
        });
        ...
        setProfessionalPayers(
          payersData.map((p) => ({ payer_id: p.id, name: p.name, is_restricted: false })),
        );
    ...
    if (existing) {
      setProfessionalServices(
        professionalServices.map((s) =>
          s.service_id === serviceId ? { ...s, is_active: !s.is_active } : s,
        ),
      );
    } else {
      setProfessionalServices([
  ...
  const handlePayerRestrictionToggle = (payerId) => {
    setProfessionalPayers(
      professionalPayers.map((p) =>
        p.payer_id === payerId ? { ...p, is_restricted: !p.is_restricted } : p,
      ),
    );
  };
                ...
                <TabsContent value="personal" className="space-y-4 outline-none">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        name="user_id"
                        value={selectValue(form.user_id)}
                        onValueChange={(v) => setForm((f) => ({ ...f, user_id: fromSelect(v) }))}
                      >
                        <SelectTrigger>
                ...
                <TabsContent value="services" className="space-y-6 outline-none">
                    ...
                    <div className="max-h-60 overflow-y-auto border rounded-md p-2 space-y-2">
                      {services.map((service) => (
                        <div key={service.id} className="flex items-center justify-between">
                          ...
                          <Checkbox
                            checked={
                              professionalServices.find((s) => s.service_id === service.id)
                                ?.is_active ?? false
                            }
                  ...
                  <div>
                    <h3 className="font-semibold mb-2">
                      Convênios Atendidos (marcar para restringir)
                    </h3>
                    <div className="max-h-60 overflow-y-auto border rounded-md p-2 space-y-2">
                ...
                <TabsContent value="schedule" className="space-y-4 outline-none">
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(c) => setForm((f) => ({ ...f, has_schedule: c }))}
                    />
                    <Label htmlFor="has_schedule">
                      Possui agenda de atendimentos nesta unidade
                    </Label>
                  </div>
                  {form.has_schedule && (
                    <>
                      <div className="space-y-2">
                        ...
                        <Textarea
                          ...
                          onChange={handleChange}
                          placeholder="Ex: Atende somente na parte da manhã."
                        />
                      </div>
                      <div className="space-y-2">
                          ...
                          {professionalSchedules.map((schedule, index) => (
                            <div key={index} className="grid grid-cols-5 gap-2 items-center">
                              <Select
                                ...
                                onValueChange={(v) =>
                                  handleScheduleChange(index, 'weekday', Number(v))
                                }
                              >
                                <SelectTrigger>
                                  <SelectValue />
