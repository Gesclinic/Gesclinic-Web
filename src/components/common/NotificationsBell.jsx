...
export default function NotificationsBell() {
  ...
  const markAllRead = async () => {
    if (!clinicId || !user?.id || notifications.length === 0) return;
    const unreadIds = notifications.filter(n => !n.is_read).map(n => n.id);
    if (unreadIds.length === 0) return;

    const toInsert = unreadIds.map(id => ({ notification_id: id, user_id: user.id }));
    
    await supabase.from("notifications_read").upsert(toInsert, { onConflict: 'notification_id,user_id' });
    setUnreadCount(0);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'notifications_read' }, payload => {
        fetchUnreadCount();
         if(popoverOpen) loadNotifications();
      })
      .subscribe();
        ...
        <PopoverContent className="w-96 p-0" align="end">
          ...
          <ScrollArea className="h-[400px]">
            ...
            {loading ? (
              ...
              notifications.map((n) => (
                ...
                >
                  <p className={`font-semibold ${!n.is_read ? 'text-blue-800 dark:text-blue-300' : ''}`}>{n.title}</p>
          ...
          <div className="flex justify-center p-2 bg-gray-50 dark:bg-gray-800/50">
            <Button
              ...
              onClick={() => { setPanelOpen(true); setPopoverOpen(false); }}
            >
