  ...
  return (
    <form className="p-2 border rounded mb-2" onSubmit={handleSubmit}>
      <div className="mb-2">
        ...
        <input className="border px-2 py-1 w-full" value={motivo} onChange={e => setMotivo(e.target.value)} required />
      </div>
      <div className="mb-2">
        <label className="block text-xs font-medium">Início</label>
        <input type="datetime-local" className="border px-2 py-1 w-full" value={inicio} onChange={e => setInicio(e.target.value)} required />
      </div>
      <div className="mb-2">
        <label className="block text-xs font-medium">Fim</label>
        <input type="datetime-local" className="border px-2 py-1 w-full" value={fim} onChange={e => setFim(e.target.value)} required />
      </div>
      <button className="bg-blue-600 text-white px-4 py-1 rounded" type="submit">{initial ? 'Salvar edição' : 'Indisponibilizar'}</button>
    </form>
  );
}
